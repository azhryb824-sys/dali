    ...
    const result=await db.transaction(async tx=>{
      const[document]=await tx.insert(companyDocuments).values({
        ...
        metadataJson:JSON.stringify({
          ...
          servicePeriod:payment.servicePeriod,issueDate,details:invoiceDetails,absenceDeductionHalalas,amountHalalas:netAmountHalalas,subtotalHalalas:netSubtotalHalalas,
          vatHalalas:netVatHalalas,vatRateBps:payment.vatRateBps,netSubtotalHalalas,netVatHalalas,netAmountHalalas,templateVersion:"letterhead-v4-english-invoice",
      ...
      const[updated]=await tx.update(contractPaymentSchedules).set({status:"invoiced",invoiceDocumentId:document.id,financialRecordId:financial.id,invoicedBy:access.user.email,invoicedAt:new Date().toISOString(),updatedAt:new Date().toISOString()}).where(and(eq(contractPaymentSchedules.id,payment.id),eq(contractPaymentSchedules.status,"referred"))).returning();
...
export async function PATCH(request:Request){
  ...
  if(action==="reschedule"){
    ...
    const[updated]=await db.update(contractPaymentSchedules).set({dueDate,status:dueDate<=now.slice(0,10)?"due":"scheduled",servicePeriod:payment.billingBasis==="monthly_salary"?dueDate.slice(0,7):payment.servicePeriod,updatedAt:now}).where(and(eq(contractPaymentSchedules.id,payment.id),inArray(contractPaymentSchedules.status,["scheduled","due"]))).returning();
