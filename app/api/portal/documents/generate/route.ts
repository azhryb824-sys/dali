...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (quoteVersionId && (!sourceQuote?.approvedBy || !["approved", "sent", "accepted"].includes(sourceQuote.status))) return Response.json({ error: "لا يمكن إنشاء عقد إلا من عرض سعر معتمد" }, { status: 409 });
    ...
    if (conversionMode === "as_is" && sourceQuote?.commercialTermsJson) {
      ...
      for (const spec of commercialTextFields) {
        if (spec.name === "endDate" && seasonType === "regular") continue;
        if (approvedTerms[spec.name] && approvedTerms[spec.name] !== submittedTerms[spec.name]) return Response.json({ error: `${spec.label} لا يطابق العرض المعتمد؛ عدّل إصدار العرض واعتمده أولًا` }, { status: 409 });
      }
      if (approvedTerms.showPaymentSchedule !== showPaymentSchedule) return Response.json({ error: "إظهار جدول الدفعات يجب أن يطابق العرض المعتمد" }, { status: 409 });
    ...
    if (sourceOpportunity?.sourceRequestId) {
      ...
      if (!approvedRequest?.approvedBy || approvedRequest.approvalStatus !== "approved") return Response.json({ error: "طلب عرض السعر المصدر غير معتمد" }, { status: 409 });
    }
