...
import { chartOfAccounts, financialRecords, journalEntries, workforceContracts } from "@/db/schema";
...
export async function POST(request:Request){
  ...
  try{
    ...
    const account=(code:string)=>accounts.find(item=>item.code===code);
    let debitCode="";let creditCode="";let description=record.description;
    ...
    else if(record.category==="receipt_voucher"){debitCode="1200";creditCode="1300";description=`تحصيل من عميل — ${record.description}`;}
    else if(isExpense){debitCode=record.category==="worker_expense"?"5100":"5200";creditCode="2100";description=`إثبات مصروف مستحق — ${record.description}`;}
    else return jsonNoStore({error:"هذه الحركة تُرحّل من وحدتها المتخصصة ولا تقبل قيدًا يدويًا هنا"},{status:409});
    const debit=account(debitCode);const credit=account(creditCode);const taxPayable=account("2300");const taxRecoverable=account("1400");if(!debit||!credit||(record.vatHalalas>0&&isRevenue&&!taxPayable)||(record.vatHalalas>0&&isExpense&&!taxRecoverable))return jsonNoStore({error:"يجب تهيئة دليل الحسابات والضريبة قبل إنشاء القيد"},{status:409});
    const subtotal=record.subtotalHalalas||record.amountHalalas-record.vatHalalas;const lines=isRevenue?[{accountId:debit.id,debitHalalas:record.amountHalalas,description},{accountId:credit.id,creditHalalas:subtotal,description:`إيراد قبل الضريبة — ${record.description}`}]:isExpense?[{accountId:debit.id,debitHalalas:subtotal,description:`مصروف قبل الضريبة — ${record.description}`},{accountId:credit.id,creditHalalas:record.amountHalalas,description}]:[{accountId:debit.id,debitHalalas:record.amountHalalas,description},{accountId:credit.id,creditHalalas:record.amountHalalas,description}];if(record.vatHalalas>0&&isRevenue&&taxPayable)lines.push({accountId:taxPayable.id,creditHalalas:record.vatHalalas,description:`ضريبة مخرجات — ${record.description}`});if(record.vatHalalas>0&&isExpense&&taxRecoverable)lines.push({accountId:taxRecoverable.id,debitHalalas:record.vatHalalas,description:`ضريبة مدخلات — ${record.description}`});
    ...
    const [updated]=await db.update(financialRecords).set({journalEntryId:journal.entry.id,postingStatus:"draft",updatedAt:new Date().toISOString()}).where(eq(financialRecords.id,record.id)).returning();
    await auditPortalAction({actorEmail:access.user.email,action:"financial-journal-created",entityType:"financial-record",entityId:record.id,before:record,after:updated});
    return jsonNoStore({record:updated,journal:journal.entry},{status:201});
