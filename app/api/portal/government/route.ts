...
export async function GET(request:Request){const access=await permitted();if(!access)return jsonNoStore({error:"غير مصرح"},{status:403});const correlationId=requestCorrelationId(request);try{const db=getDb();const[sites,payments,obligations,staff,banks]=await Promise.all([db.select().from(governmentSites).where(eq(governmentSites.status,"active")).orderBy(governmentSites.name),db.select().from(governmentPaymentRequests).orderBy(desc(governmentPaymentRequests.createdAt)).limit(500),db.select().from(complianceObligations).orderBy(complianceObligations.expiryDate).limit(1000),db.select().from(employees).where(isNull(employees.archivedAt)).orderBy(employees.fullName).limit(2000),db.select().from(bankAccounts).where(eq(bankAccounts.status,"active")).orderBy(bankAccounts.bankName)]);const renewalItems=staff.flatMap(employee=>[...(employee.residencyType==="resident"?[{key:`employee-${employee.id}-iqama`,employeeId:employee.id,employeeNumber:employee.employeeNumber,employeeName:employee.fullName,kind:"iqama",label:"تجديد الإقامة",expiryDate:employee.iqamaExpiry,actionUrl:"https://muqeem.sa/",platform:"مقيم"},{key:`employee-${employee.id}-work-permit`,employeeId:employee.id,employeeNumber:employee.employeeNumber,employeeName:employee.fullName,kind:"work_permit",label:"تجديد رخصة العمل",expiryDate:employee.workPermitExpiry,actionUrl:"https://www.qiwa.sa/",platform:"قوى"}]:[]),...(employee.contractType==="fixed_term"?[{key:`employee-${employee.id}-contract`,employeeId:employee.id,employeeNumber:employee.employeeNumber,employeeName:employee.fullName,kind:"employment_contract",label:"تجديد عقد العمل",expiryDate:employee.contractEndDate,actionUrl:"https://www.qiwa.sa/",platform:"قوى"}]:[])]).filter(item=>item.expiryDate).sort((a,b)=>String(a.expiryDate).localeCompare(String(b.expiryDate)));return jsonNoStore({banks,sites:sites.map(site=>({...site,usernameEnvelope:undefined,passwordEnvelope:undefined,hasUsername:Boolean(site.usernameEnvelope),hasPassword:Boolean(site.passwordEnvelope)})),payments,obligations,renewalItems,canReveal:owner(access),canPay:owner(access),canWrite:owner(access)||await hasPortalPermission(access,"government","write")});}catch(error){console.error("government-affairs-load-failed",{correlationId,error});return jsonNoStore({error:"تعذر تحميل العلاقات الحكومية بسبب عدم توافق قاعدة البيانات",correlationId},{status:500});}}
