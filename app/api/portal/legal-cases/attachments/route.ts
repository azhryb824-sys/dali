...
import { legalCaseAttachments, legalRecords, portalActivity } from "@/db/schema";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [attachment] = await db.insert(legalCaseAttachments).values({ legalRecordId, title, fileName, storageKey, contentType: file.type, sizeBytes: validation.bytes.byteLength, validationStatus: "signature-validated", validationDetails: validation.validationDetails, createdBy: actor.user.email }).returning();
    await db.insert(portalActivity).values({ actorEmail: actor.user.email, action: "legal-case-attachment-uploaded", entityType: "legal-record", entityId: String(legalRecordId), afterJson: JSON.stringify({ attachmentId: attachment.id, title }) });
