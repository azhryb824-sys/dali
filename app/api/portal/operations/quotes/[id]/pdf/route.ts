...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const bytes = await generateIssuedPdf({
    ...
    title: commercial.title || opportunity.title,
    titleEn: commercial.titleEn,
    detailsEn: commercial.detailsEn,
    contractDirection: commercial.contractDirection,
    ...
    accommodationParty: quote.accommodationParty === "not_applicable" ? "لا ينطبق" : quote.accommodationParty === "dali" ? "توفره دالي" : quote.accommodationParty === "counterparty" ? "يوفره الطرف الثاني" : undefined,
    transportParty: quote.transportParty === "not_applicable" ? "لا ينطبق" : quote.transportParty === "dali" ? "توفره دالي" : quote.transportParty === "counterparty" ? "يوفره الطرف الآخر" : undefined,
    paymentSchedule,
