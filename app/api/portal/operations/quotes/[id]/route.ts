...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!access || !(await hasPortalPermission(access, "workforce", "write"))) return jsonNoStore({ error: "غير مصرح بتعديل عرض السعر" }, { status: 403 });
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  if (rejectCrossSiteRequest(request)) return jsonNoStore({ error: "مصدر الطلب غير مسموح" }, { status: 403 });
  const access = await requirePortalApiRole(["admin", "manager", "employee"]);
  if (!access || !(await hasPortalPermission(access, "workforce", "write"))) return jsonNoStore({ error: "غير مصرح بحذف عرض السعر" }, { status: 403 });
  const id = Number((await context.params).id);
  const db = getDb();
  const quote = await db.query.quoteVersions.findFirst({ where: eq(quoteVersions.id, id) });
  if (!quote) return jsonNoStore({ error: "عرض السعر غير موجود" }, { status: 404 });
