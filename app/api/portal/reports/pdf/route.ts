...
export async function GET(request:Request){
  const access=await requirePortalApiRole(["admin","manager","employee"]);if(!access||!(await hasPortalPermission(access,"reports","export")))return Response.json({error:"تصدير التقارير يتطلب صلاحية مخصصة"},{status:403});
