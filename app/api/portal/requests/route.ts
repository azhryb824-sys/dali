import { decideQuoteRequest, WorkflowError } from "@/lib/quote-request-workflow";
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!Number.isInteger(id) || id < 1 || (!["approve", "reject", "request-changes"].includes(String(payload.action)) && !allowedStatuses.has(status)) || !Number.isInteger(version) || version < 1) {
    ...
    if (["approve", "reject", "request-changes"].includes(String(payload.action))) {
      try {
        const updated = await decideQuoteRequest(access, id, version, payload.action === "approve" ? "approved" : payload.action === "reject" ? "rejected" : "changes_requested", typeof payload.reason === "string" ? payload.reason.trim().slice(0, 1000) : "");
        return Response.json({ request: updated });
      } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "تعذر اتخاذ القرار" }, { status: error instanceof WorkflowError ? error.status : 500 }); }
    }
