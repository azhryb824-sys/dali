...
export async function GET(request: Request) {
  ...
  const [canSearchWorkforce, canSearchFinance, canSearchContracts, canSearchVideo, canSearchEmployees, canSearchLegal, canSearchOperations, canSearchConstruction, canSearchGovernment, canSearchWebsite] = await Promise.all([
    hasPortalPermission(access, "workforce", "read"),
    hasPortalPermission(access, "finance", "read"),
    hasPortalPermission(access, "contracts", "read"),
    hasPortalPermission(access, "video", "read"),
    hasPortalPermission(access, "employees", "read"),
    ...
    hasPortalPermission(access, "government", "read"),
    hasPortalPermission(access, "website", "read"),
  ]);
  const canSearchConversations = canAccessPortalConversations(access);
  const canSearchDocuments = canAccessPortalDocuments(access);
  ...
  try {
    const [requestRows, workerRows, contractRows, paymentRows, conversationRows, interviewRows, employeeRows, financeRows, legalRows, lawyerRows, documentRows, clientRows, opportunityRows, quoteRows, orderRows, sheetRows, planRows, privacyRows, userRows, constructionOpportunityRows, constructionProjectRows, constructionRecordRows, governmentSiteRows, governmentPaymentRows, taskRows, letterRows] = await Promise.all([
      ...
      canSearchContracts ? db.select().from(contractPaymentSchedules).where(like(contractPaymentSchedules.title, pattern)).orderBy(desc(contractPaymentSchedules.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchConversations ? db.select().from(visitorConversations).where(or(like(visitorConversations.trackingCode, pattern), like(visitorConversations.visitorName, pattern), like(visitorConversations.visitorMobile, pattern), like(visitorConversations.subject, pattern))).orderBy(desc(visitorConversations.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchVideo ? db.select().from(videoInterviews).where(or(like(videoInterviews.referenceCode, pattern), like(videoInterviews.assignedTo, pattern), like(videoInterviews.status, pattern))).orderBy(desc(videoInterviews.updatedAt)).limit(6) : Promise.resolve([]),
      ...
      canSearchOperations ? db.select().from(salesOpportunities).where(or(like(salesOpportunities.opportunityCode, pattern), like(salesOpportunities.title, pattern))).orderBy(desc(salesOpportunities.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchContracts ? db.select().from(quoteVersions).where(like(quoteVersions.quoteCode, pattern)).orderBy(desc(quoteVersions.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchOperations ? db.select().from(workOrders).where(or(like(workOrders.workOrderCode, pattern), like(workOrders.title, pattern), like(workOrders.workSite, pattern))).orderBy(desc(workOrders.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchOperations ? db.select().from(timesheets).where(like(timesheets.timesheetCode, pattern)).orderBy(desc(timesheets.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchOperations ? db.select().from(capacityPlans).where(or(like(capacityPlans.planCode, pattern), like(capacityPlans.seasonName, pattern), like(capacityPlans.profession, pattern), like(capacityPlans.location, pattern))).orderBy(desc(capacityPlans.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchLegal ? db.select().from(dataSubjectRequests).where(or(like(dataSubjectRequests.trackingCode, pattern), like(dataSubjectRequests.fullName, pattern), like(dataSubjectRequests.email, pattern))).orderBy(desc(dataSubjectRequests.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchUsers ? db.select().from(portalUsers).where(or(like(portalUsers.displayName, pattern), like(portalUsers.email, pattern), like(portalUsers.requestedJobTitle, pattern), like(portalUsers.requestReason, pattern))).orderBy(desc(portalUsers.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchConstruction ? db.select().from(constructionOpportunities).where(or(like(constructionOpportunities.opportunityCode, pattern), like(constructionOpportunities.title, pattern), like(constructionOpportunities.clientName, pattern), like(constructionOpportunities.projectType, pattern))).orderBy(desc(constructionOpportunities.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchConstruction ? db.select().from(constructionProjects).where(or(like(constructionProjects.projectCode, pattern), like(constructionProjects.title, pattern), like(constructionProjects.clientName, pattern), like(constructionProjects.projectType, pattern), like(constructionProjects.costCenterCode, pattern))).orderBy(desc(constructionProjects.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchConstruction ? db.select().from(constructionRecords).where(or(like(constructionRecords.recordCode, pattern), like(constructionRecords.title, pattern), like(constructionRecords.description, pattern), like(constructionRecords.responsibleEmail, pattern))).orderBy(desc(constructionRecords.updatedAt)).limit(8) : Promise.resolve([]),
      canSearchGovernment ? db.select().from(governmentSites).where(or(like(governmentSites.name, pattern), like(governmentSites.accountReference, pattern))).orderBy(desc(governmentSites.updatedAt)).limit(6) : Promise.resolve([]),
      canSearchGovernment ? db.select().from(governmentPaymentRequests).where(or(like(governmentPaymentRequests.referenceCode, pattern), like(governmentPaymentRequests.serviceName, pattern), like(governmentPaymentRequests.sadadNumber, pattern), like(governmentPaymentRequests.billerNumber, pattern))).orderBy(desc(governmentPaymentRequests.updatedAt)).limit(6) : Promise.resolve([]),
      db.select().from(portalTasks).where(and(eq(portalTasks.createdBy, access.user.email), or(like(portalTasks.title, pattern), like(portalTasks.description, pattern)))).orderBy(desc(portalTasks.updatedAt)).limit(8),
      canSearchContracts ? db.select().from(officialLetters).where(or(like(officialLetters.referenceCode, pattern), like(officialLetters.subject, pattern), like(officialLetters.recipient, pattern))).orderBy(desc(officialLetters.updatedAt)).limit(6) : Promise.resolve([]),
    ]);
    ...
    const results: Result[] = [
      ...
      ...contractRows.map((item) => ({ key: `contract-${item.id}`, kind: "contract", id: item.id, view: "contractual-documents", title: `${item.referenceCode} · ${item.clientName}`, meta: `عقد · ${item.workSite}`, searchValue: item.referenceCode })),
      ...paymentRows.map((item) => ({ key: `contract-payment-${item.id}`, kind: "contract-payment", id: item.id, view: "contractual-documents", title: item.title, meta: `دفعة عقد · ${item.status} · ${item.dueDate}`, searchValue: item.title })),
      ...
      ...interviewRows.map((item) => ({ key: `video-interview-${item.id}`, kind: "video-interview", id: 0, stringId: item.id, view: "overview", title: item.referenceCode, meta: `مقابلة مرئية · ${item.status} · ${item.assignedTo || "بانتظار التعيين"}`, searchValue: item.referenceCode })),
      ...
      ...quoteRows.map((item) => ({ key: `quote-${item.id}`, kind: "quote", id: item.id, view: "contractual-documents", title: item.quoteCode, meta: `عرض سعر · ${item.status}`, searchValue: item.quoteCode })),
      ...
      ...taskRows.map((item) => ({ key: `task-${item.id}`, kind: "task", id: item.id, view: "tasks", title: item.title, meta: `مهمة · ${item.status}`, searchValue: item.title })),
