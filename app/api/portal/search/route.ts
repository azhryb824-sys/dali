...
import { clients, companyDocuments, constructionOpportunities, constructionProjects, constructionRecords, contractPaymentSchedules, employees, financialRecords, legalRecords, quoteVersions, salesOpportunities, timesheets, visitorConversations, workers, workforceContracts, workforceRequests, workOrders, capacityPlans, dataSubjectRequests, portalUsers } from "@/db/schema";
...
export async function GET(request: Request) {
  ...
  const db = getDb();
  try {
    const [requestRows, workerRows, contractRows, paymentRows, conversationRows, employeeRows, financeRows, legalRows, documentRows, clientRows, opportunityRows, quoteRows, orderRows, sheetRows, planRows, privacyRows, userRows, constructionOpportunityRows, constructionProjectRows, constructionRecordRows] = await Promise.all([
