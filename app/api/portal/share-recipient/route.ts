...
export async function GET(request: Request) {
  ...
  if (!access || !canAccessPortalDocuments(access) || !canSharePortalDocuments(access)) return jsonNoStore({ error: "غير مصرح بمشاركة المستندات" }, { status: 403 });
  ...
  if ((source.quoteId || source.contractId) && !await hasPortalPermission(access, "contracts", "read")) return jsonNoStore({ error: "غير مصرح بعرض بيانات العميل" }, { status: 403 });
