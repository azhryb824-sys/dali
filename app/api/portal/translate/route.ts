...
export async function POST(request:Request){
  ...
  try{
    const response=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({q:values,source:"ar",target:"en",format:"text",api_key:process.env.LIBRETRANSLATE_API_KEY||undefined}),signal:controller.signal,cache:"no-store"});
