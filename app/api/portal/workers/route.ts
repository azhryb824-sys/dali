...
export async function POST(request: Request) {
  ...
  try {
    ...
    const ajirContractStatus = "not_applicable";
    ...
    if (!sponsorshipType || (sponsorshipType === "other" && sponsorName.length < 2)) {
      return Response.json({ error: "حدد جهة كفالة العامل، واكتب اسم الكفيل عندما تكون الكفالة على جهة أخرى" }, { status: 400 });
    }
