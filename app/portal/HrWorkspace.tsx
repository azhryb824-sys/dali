...
type HrDocument={id:number;employeeId:number;documentType:string;documentNumber:string|null;expiryDate:string|null;status:string};type Leave={id:number;employeeId:number;leaveType:string;startDate:string;endDate:string;days:number;status:string;reason:string|null};type Attendance={id:number;employeeId:number;attendanceDate:string;status:string;lateMinutes:number;overtimeMinutes:number};type PortalUser={email:string;displayName:string;status:string};
type HrData = { employees: Employee[]; movements: Movement[]; runs: PayrollRun[]; items: PayrollItem[];documents:HrDocument[];leaves:Leave[];attendance:Attendance[];users:PortalUser[] };
...
export default function HrWorkspace({ canWrite, isAdmin }: { canWrite: boolean; isAdmin: boolean }) {
  ...
  return <section className="hr-workspace">
    ...
    {canWrite && <div className="hr-forms">
      ...
      <details><summary>ربط الملف الوظيفي والمستخدم</summary><form onSubmit={event=>submitExtended(event,"employee-profile")}><select name="employeeId" required defaultValue=""><option value="" disabled>اختر الموظف</option>{activeStaff.map(item=><option key={item.id} value={item.id}>{item.fullName}</option>)}</select><select name="portalUserEmail" defaultValue=""><option value="">بدون حساب مستخدم</option>{data.users.filter(user=>user.status==="active").map(user=><option key={user.email} value={user.email}>{user.displayName} · {user.email}</option>)}</select><select name="managerId" defaultValue=""><option value="">بدون مدير مباشر</option>{activeStaff.map(item=><option key={item.id} value={item.id}>{item.fullName}</option>)}</select><input name="workLocation" placeholder="موقع العمل"/><select name="employmentType"><option value="full_time">دوام كامل</option><option value="part_time">دوام جزئي</option><option value="temporary">مؤقت</option></select><select name="contractType"><option value="fixed_term">محدد المدة</option><option value="indefinite">غير محدد المدة</option></select><input name="gosiNumber" placeholder="رقم التأمينات"/><button>حفظ وربط الملف</button></form></details>
      <details><summary>طلب إجازة</summary><form onSubmit={event=>submitExtended(event,"leave-request")}><select name="employeeId" required>{activeStaff.map(item=><option key={item.id} value={item.id}>{item.fullName}</option>)}</select><select name="leaveType"><option value="annual">سنوية</option><option value="sick">مرضية</option><option value="unpaid">بدون راتب</option><option value="emergency">اضطرارية</option></select><input name="startDate" type="date" required/><input name="endDate" type="date" required/><input className="wide" name="reason" placeholder="سبب الطلب"/><button>إرسال للاعتماد</button></form></details>
      <details><summary>وثيقة موظف</summary><form onSubmit={event=>submitExtended(event,"document")}><select name="employeeId" required>{activeStaff.map(item=><option key={item.id} value={item.id}>{item.fullName}</option>)}</select><select name="documentType"><option value="employment_contract">عقد العمل</option><option value="national_id">الهوية / الإقامة</option><option value="insurance">التأمين الطبي</option><option value="certificate">شهادة مهنية</option></select><input name="documentNumber" placeholder="رقم الوثيقة"/><input name="expiryDate" type="date"/><input className="wide" name="notes" placeholder="ملاحظات"/><button>حفظ الوثيقة</button></form></details>
      <details><summary>الحضور والانصراف</summary><form onSubmit={event=>submitExtended(event,"attendance")}><select name="employeeId" required>{activeStaff.map(item=><option key={item.id} value={item.id}>{item.fullName}</option>)}</select><input name="attendanceDate" type="date" required/><select name="status"><option value="present">حاضر</option><option value="absent">غائب</option><option value="remote">عمل عن بعد</option><option value="leave">إجازة</option><option value="sick">مرضي</option></select><input name="checkInAt" type="time"/><input name="checkOutAt" type="time"/><input name="lateMinutes" type="number" min="0" placeholder="دقائق التأخير"/><input name="overtimeMinutes" type="number" min="0" placeholder="دقائق إضافية"/><button>حفظ سجل اليوم</button></form></details>
    </div>}
    <section className="hr-people-grid"><article className="panel"><div className="panel-head"><div><h2>دليل الموظفين الموحّد</h2><p>الهيكل الإداري والحساب والصلاحيات والبيانات المالية في ملف واحد</p></div></div>{data.employees.map(item=><div className="employee-profile-row" key={item.id}><b>{item.fullName}<small>{item.employeeNumber} · {item.jobTitle}</small></b><span>{item.department}<small>{item.workLocation||"الموقع غير محدد"}</small></span><span>{item.portalUserEmail?"مرتبط بالنظام":"غير مرتبط"}<small>{item.employmentType}</small></span><strong>{money(item.baseSalaryHalalas+item.housingAllowanceHalalas+item.transportAllowanceHalalas+item.otherAllowanceHalalas)}</strong></div>)}</article><article className="panel"><div className="panel-head"><div><h2>مركز الموافقات والتنبيهات</h2><p>لا تُخصم الإجازة إلا بعد الاعتماد</p></div></div>{pendingLeaves.map(leave=>{const employee=data.employees.find(item=>item.id===leave.employeeId);return <div className="leave-approval" key={leave.id}><b>{employee?.fullName}<small>{leave.startDate} — {leave.endDate} · {leave.days} أيام</small></b>{canWrite&&<span><button onClick={()=>void action("PATCH",{action:"leave-decision",leaveId:leave.id,decision:"approved"},`leave-${leave.id}`)}>اعتماد</button><button className="danger" onClick={()=>void action("PATCH",{action:"leave-decision",leaveId:leave.id,decision:"rejected"},`leave-${leave.id}`)}>رفض</button></span>}</div>})}{!pendingLeaves.length&&<p className="empty-operational">لا توجد طلبات معلّقة.</p>}</article></section>
