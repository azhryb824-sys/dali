...
function QuoteForm({ data, busy, onCreate }: { data: OperationsData; busy: string; onCreate: CreateOperation }) {
  ...
  type Line={key:string;profession:string;quantity:number;durationMonths:number;unitPrice:number;notes:string};
  const presets:Record<string,Omit<Line,"key">>={"توريد وتشغيل القوى العاملة":{profession:"عامل حسب المهنة المطلوبة",quantity:1,durationMonths:12,unitPrice:0,notes:"يشمل نطاق التوريد والتشغيل المتفق عليه"},"التشغيل والصيانة":{profession:"باقة تشغيل وصيانة",quantity:1,durationMonths:12,unitPrice:0,notes:"زيارات دورية وتقارير أداء واستجابة للأعطال"},"النظافة وإدارة المرافق":{profession:"خدمة نظافة وإدارة مرافق",quantity:1,durationMonths:12,unitPrice:0,notes:"وفق المساحات ومستوى الخدمة والمواد المعتمدة"},"المقاولات والأعمال الإنشائية":{profession:"بند أعمال إنشائية",quantity:1,durationMonths:1,unitPrice:0,notes:"وفق المخططات وجداول الكميات المعتمدة"},"خدمات موسمية للحج ورمضان":{profession:"خدمة موسمية",quantity:1,durationMonths:1,unitPrice:0,notes:"يشمل التجهيز والإشراف خلال الموسم"},"خدمة أخرى":{profession:"الخدمة المطلوبة",quantity:1,durationMonths:1,unitPrice:0,notes:""}};
  const [items,setItems]=useState<Line[]>([{key:"line-1",...presets[activity]}]);
  const changeActivity=(value:string)=>{setActivity(value);setItems([{key:`line-${Date.now()}`,...presets[value]}]);};
  ...
  return <FormDetails title="إنشاء عرض سعر احترافي"><form className="professional-quote-form" onSubmit={submit((form) => void onCreate("create-quote",form,{items:items.map(item=>({profession:item.profession,quantity:item.quantity,durationMonths:item.durationMonths,unitPrice:item.unitPrice,notes:item.notes}))}))}>
    <div className="quote-form-section span-two"><strong>1. العميل ونوع الطلب</strong><p>يتغير قالب البنود والوصف تلقائيًا حسب النشاط المحدد.</p></div>
    ...
    <label>نوع نشاط العرض<select name="activityLabel" value={activity} onChange={(event)=>changeActivity(event.target.value)}>{Object.keys(presets).map(option=><option key={option}>{option}</option>)}</select></label>
    ...
    <div className="quote-form-section span-two"><strong>2. جدول البنود والكميات والأسعار</strong><p>أضف كل خدمة كبند مستقل؛ يحسب النظام الإجمالي تلقائيًا.</p></div>
    <div className="quote-line-builder span-two">{items.map((item,index)=><article key={item.key}><header><strong>البند {index+1}</strong>{items.length>1&&<button type="button" onClick={()=>setItems(rows=>rows.filter(row=>row.key!==item.key))}>حذف</button>}</header><label className="quote-line-name">وصف الخدمة<input required value={item.profession} onChange={e=>update(item.key,"profession",e.target.value)}/></label><label>الكمية<input required type="number" min="1" value={item.quantity} onChange={e=>update(item.key,"quantity",Number(e.target.value))}/></label><label>المدة بالأشهر<input required type="number" min="1" value={item.durationMonths} onChange={e=>update(item.key,"durationMonths",Number(e.target.value))}/></label><label>سعر الوحدة الشهري<input required type="number" min="0.01" step="0.01" value={item.unitPrice||""} onChange={e=>update(item.key,"unitPrice",Number(e.target.value))}/></label><label className="quote-line-notes">نطاق البند وملاحظاته<input value={item.notes} onChange={e=>update(item.key,"notes",e.target.value)}/></label><b>{money(item.quantity*item.durationMonths*item.unitPrice*100)}</b></article>)}<button className="quote-add-line" type="button" onClick={()=>setItems(rows=>[...rows,{key:`line-${Date.now()}`,...presets[activity]}])}>+ إضافة بند آخر</button></div>
    <div className="quote-form-section span-two"><strong>3. التسعير والشروط</strong><p>الإجمالي الأولي للبنود: {new Intl.NumberFormat("ar-SA",{style:"currency",currency:"SAR"}).format(subtotal)}</p></div>
    <label>الخصم بالريال<input name="discount" type="number" min="0" step="0.01" defaultValue="0"/></label><label>ضريبة القيمة المضافة<select name="vatRate" defaultValue="15"><option value="0">بدون ضريبة</option><option value="15">ضريبة 15%</option></select></label><label className="span-two">شروط الدفع<textarea name="terms" required rows={3} placeholder="مثال: 50% مقدم، و50% عند بدء الخدمة"/></label><label className="span-two">الافتراضات والاستثناءات ومتطلبات العميل<textarea name="assumptions" rows={3} placeholder={`اكتب التفاصيل الخاصة بنشاط: ${activity}`}/></label><button className="quote-submit span-two" disabled={busy === "create-quote"}>{busy === "create-quote"?"جارٍ إنشاء العرض...":"إنشاء عرض السعر وحفظ PDF"}</button>
  </form></FormDetails>;
}
