...
export default function OperationsWorkspace({ canWrite, isAdmin, initialTab = "crm", initialQuery = "" }: { canWrite: boolean; isAdmin: boolean; initialTab?: OperationsTab; initialQuery?: string }) {
  ...
  return <>
    ...
    {tab === "capacity" && <OperationsSection title="تخطيط السعة للمواسم والحج" count={data.capacityPlans.length} form={canWrite ? <CapacityForm busy={busy} onCreate={create}/> : null}>
...
function lines(value: FormDataEntryValue | null, kind: "quote" | "requirements") { return String(value || "").split("\n").map((line) => line.trim()).filter(Boolean).map((line) => { const parts = line.split("|").map((item) => item.trim()); return kind === "quote" ? { profession: parts[0], quantity: Number(parts[1]), durationMonths: Number(parts[2]), unitPrice: Number(parts[3]) } : { profession: parts[0], requiredCount: Number(parts[1]), shiftName: parts[2] || "", startTime: parts[3] || "", endTime: parts[4] || "" }; }); }
...
function QuoteForm({ data, busy, onCreate }: { data: OperationsData; busy: string; onCreate: CreateOperation }) { return <FormDetails title="عرض سعر"><form onSubmit={submit((form) => { const fd = new FormData(form); void onCreate("create-quote",form,{items:lines(fd.get("itemLines"),"quote")}); })}><select name="opportunityId" required defaultValue=""><option value="" disabled>الفرصة</option>{data.opportunities.filter((item) => !["won","lost"].includes(item.stage)).map((item) => <option value={item.id} key={item.id}>{item.opportunityCode} · {item.title}</option>)}</select><input name="issueDate" type="date" required/><input name="validUntil" type="date" required/><textarea name="itemLines" required rows={4} placeholder="كل بند في سطر: المهنة | العدد | الأشهر | سعر العامل الشهري"/><input name="discount" type="number" min="0" step="0.01" placeholder="الخصم بالريال"/><textarea name="terms" rows={2} placeholder="الشروط"/><button disabled={busy === "create-quote"}>إنشاء العرض</button></form></FormDetails>; }
