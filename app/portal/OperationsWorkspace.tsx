...
function QuoteForm({ data, busy, onCreate }: { data: OperationsData; busy: string; onCreate: CreateOperation }) {
  ...
  const payloadItems=items.map(item=>({profession:item.profession,quantity:item.quantity,durationMonths:item.durationMonths,unitPrice:item.unitPrice,notes:[item.nationality&&`الجنسية: ${item.nationality}`,item.unit&&`الوحدة: ${item.unit}`,item.shift&&`الوردية/الخدمة: ${item.shift}`,item.notes].filter(Boolean).join(" | ")}));
  return <FormDetails title="إنشاء عرض سعر حسب نوع الطلب"><form className={`professional-quote-form quote-${activity}`} onSubmit={submit((form) => void onCreate("create-quote",form,{items:payloadItems}))}>
    ...
    <div className="quote-form-section span-two"><strong>3. التسعير والشروط</strong>{activity!=="workforce"?<p>الإجمالي الأولي للبنود: {new Intl.NumberFormat("ar-SA",{style:"currency",currency:"SAR"}).format(subtotal)}</p>:<p>يعرض المستند عدد العمال لكل مهنة وراتب العامل دون إجماليات تجميعية.</p>}</div>
    {activity!=="workforce"&&<><label>الخصم بالريال<input name="discount" type="number" min="0" step="0.01" defaultValue="0"/></label><label>ضريبة القيمة المضافة<select name="vatRate" defaultValue="15"><option value="0">بدون ضريبة</option><option value="15">ضريبة 15%</option></select></label></>}<label className="span-two">شروط الدفع<textarea name="terms" required rows={3} placeholder="مثال: دفعة مقدمة ثم دفعات مرحلية مرتبطة بالإنجاز"/></label><label className="span-two">نطاق العرض والافتراضات والاستثناءات<textarea name="assumptions" rows={3} placeholder={`تفاصيل خاصة بمسار ${activities[activity].label}`}/></label><button className="quote-submit span-two" disabled={busy === "create-quote"}>{busy === "create-quote"?"جارٍ إنشاء العرض...":"إنشاء عرض السعر وحفظ PDF"}</button>
  </form></FormDetails>;
}
