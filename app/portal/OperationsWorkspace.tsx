...
export default function OperationsWorkspace({ canWrite, isAdmin, isOwner, initialTab = "crm", initialQuery = "", onCreateContract, allowedTabs, embedded = false }: { canWrite: boolean; isAdmin: boolean; isOwner: boolean; initialTab?: OperationsTab; initialQuery?: string; onCreateContract: (quoteId?:number) => void; allowedTabs?: OperationsTab[]; embedded?: boolean }) {
  ...
  async function editQuote(quote: Quote) { setEditingQuote(quote); }

  async function saveQuoteEdit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!editingQuote) return;
    const fd=new FormData(event.currentTarget),issueDate=String(fd.get("issueDate")||""),validUntil=String(fd.get("validUntil")||"");
    setBusy(`edit-quote-${editingQuote.id}`); setNotice("");
    try { const response=await fetch(`/api/portal/operations/quotes/${editingQuote.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({issueDate,validUntil})}); const result=await readApiJson(response) as {error?:string}; if(!response.ok)throw new Error(result.error||"تعذّر تعديل عرض السعر"); setEditingQuote(null); setNotice("تم تعديل عرض السعر وإعادته للمسودة لاعتماده مجددًا."); await load(); }
    catch(error){setNotice(error instanceof Error?error.message:"تعذّر تعديل العرض")} finally{setBusy("")}
  }
...
function QuoteForm({ data, busy, onCreate, embedded = false }: { data: OperationsData; busy: string; onCreate: CreateOperation; embedded?: boolean }) {
  ...
  const changeSeasonType=(value:"regular"|"ramadan"|"hajj")=>{setSeasonType(value);if(activity==="workforce"&&value==="regular")setItems(rows=>rows.map(row=>({...row,durationMonths:12})))};
  const update=(key:string,field:keyof Omit<Line,"key">,value:string|number)=>setItems(rows=>rows.map(row=>row.key===key?{...row,[field]:activity==="workforce"&&seasonType==="regular"&&field==="durationMonths"?12:value}:row));
  ...
  const form=<form className={`professional-quote-form quote-${activity} quote-${quantityMode}`} onSubmit={submit((form) => void onCreate("create-quote",form,{items:payloadItems,quantityMode,seasonType,paymentSchedule:seasonType==="regular"?[]:payments.map(({title,percentage,dueDate})=>({title,percentage,dueDate}))}))}>
    ...
    <label className="span-two">نوع المدة والفوترة<select value={seasonType} onChange={event=>changeSeasonType(event.target.value as "regular"|"ramadan"|"hajj")}><option value="regular">سنوي — 12 شهرًا، وأول دفعة بعد شهر من بداية العقد</option><option value="ramadan">موسم رمضان — دفعات بالنسب والتواريخ</option><option value="hajj">موسم الحج — دفعات بالنسب والتواريخ</option></select></label>
