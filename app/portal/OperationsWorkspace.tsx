...
export default function OperationsWorkspace({ canWrite, isAdmin, isOwner, initialTab = "crm", initialQuery = "", onCreateContract }: { canWrite: boolean; isAdmin: boolean; isOwner: boolean; initialTab?: OperationsTab; initialQuery?: string; onCreateContract: () => void }) {
  ...
  return <>
    ...
    {tab === "quotes" && <OperationsSection title="عروض الأسعار والإصدارات" count={data.quotes.length} form={canWrite ? <QuoteForm data={data} busy={busy} onCreate={create}/> : null}>
      <div className="operations-list wide">{data.quotes.filter(includes).map((quote) => <div key={quote.id}><span className="record-code">{quote.quoteCode}<small>v{quote.versionNumber}</small></span><p><strong>{data.opportunities.find((item) => item.id === quote.opportunityId)?.title || "فرصة"}</strong><small>{fmt(quote.issueDate)} · صالح حتى {fmt(quote.validUntil)} · {data.quoteItems.filter((item) => item.quoteVersionId === quote.id).length} بنود</small></p><b>{quote.quantityMode==="open"?"عدد مفتوح":money(quote.totalHalalas)}</b><span className={`workflow-status ${quote.status}`}>{quote.status}</span>{quote.approvedBy ? <a className="quote-pdf" href={`/api/portal/operations/quotes/${quote.id}/pdf`}>تنزيل PDF</a> : <span className="quote-pdf locked">PDF بعد اعتماد المالك أو مشرف النظام</span>}{canWrite && ["draft","rejected"].includes(quote.status) && <button className="quote-revision" disabled={busy === `edit-quote-${quote.id}`} onClick={() => void editQuote(quote)}>تعديل</button>}{canWrite && ["draft","rejected"].includes(quote.status) && <button className="quote-revision danger-action" disabled={busy === `delete-quote-${quote.id}`} onClick={() => void deleteQuote(quote)}>حذف</button>}{canWrite && !["accepted","superseded"].includes(quote.status) && <button className="quote-revision" disabled={busy === `revision-${quote.id}`} onClick={() => void createQuoteRevision(quote)}>نسخة جديدة</button>}{canWrite && quote.status === "draft" && <button className="admin-primary quote-approve" disabled={busy === `transition-quote-${quote.id}`} onClick={() => void transition("transition-quote", quote, "pending_approval")}>إرسال للاعتماد</button>}{canWrite && isOwner && quote.status === "pending_approval" && <button className="admin-primary quote-approve" disabled={busy === `transition-quote-${quote.id}`} onClick={() => void transition("transition-quote", quote, "approved")}>{busy === `transition-quote-${quote.id}` ? "جارٍ الاعتماد..." : "اعتماد عرض السعر"}</button>}{canWrite && <TransitionSelect busy={busy === `transition-quote-${quote.id}`} value={quote.status} options={(quoteNext[quote.status] || []).filter(status => status !== "approved" && status !== "pending_approval")} onChange={(status) => void transition("transition-quote", quote, status)}/>}</div>)}</div>
    </OperationsSection>}
...
function QuoteForm({ data, busy, onCreate }: { data: OperationsData; busy: string; onCreate: CreateOperation }) {
  ...
  type Line={key:string;profession:string;quantity:number;durationMonths:number;unitPrice:number;notes:string;nature:string;nationality:string;unit:string;shift:string};
  const activities:Record<Activity,{label:string;hint:string;seed:Omit<Line,"key">}>={
    workforce:{label:"توريد العمالة",hint:"المهنة والجنسية والعدد وراتب العامل والمدة والوردية",seed:{profession:workforceProfessions[0].label,quantity:1,durationMonths:12,unitPrice:0,notes:"",nature:"عمالة",nationality:"أي جنسية",unit:"عامل/شهر",shift:"وردية واحدة"}},
    construction:{label:"المقاولات",hint:"بند الأعمال والوحدة والكمية ونطاق التنفيذ",seed:{profession:"بند أعمال إنشائية",quantity:1,durationMonths:1,unitPrice:0,notes:"",nature:"مقاولات",nationality:"",unit:"م²",shift:""}},
    maintenance:{label:"التشغيل والصيانة",hint:"الأصل أو النظام ومستوى الخدمة والزيارات",seed:{profession:"خدمة تشغيل وصيانة",quantity:1,durationMonths:12,unitPrice:0,notes:"استجابة للأعطال وتقارير أداء دورية",nature:"تشغيل وصيانة",nationality:"",unit:"شهر",shift:"حسب اتفاقية مستوى الخدمة"}},
    seasonal:{label:"الخدمات الموسمية",hint:"الموقع والموسم والعدد وفترة التشغيل",seed:{profession:"خدمة موسمية",quantity:1,durationMonths:1,unitPrice:0,notes:"يشمل التجهيز والإشراف خلال الموسم",nature:"موسمي",nationality:"أي جنسية",unit:"عامل/موسم",shift:"حسب الخطة التشغيلية"}},
  };
  ...
  const payloadItems=items.map(item=>({profession:item.profession,quantity:quantityMode==="open"?0:item.quantity,durationMonths:item.durationMonths,unitPrice:item.unitPrice,notes:[item.nationality&&`الجنسية: ${item.nationality}`,item.unit&&`الوحدة: ${item.unit}`,item.shift&&`الوردية/الخدمة: ${item.shift}`,item.notes].filter(Boolean).join(" | ")}));
  return <FormDetails title="إنشاء عرض سعر حسب نوع الطلب"><form className={`professional-quote-form quote-${activity} quote-${quantityMode}`} onSubmit={submit((form) => void onCreate("create-quote",form,{items:payloadItems,quantityMode}))}>
