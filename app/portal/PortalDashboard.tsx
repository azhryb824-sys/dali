...
type WorkerRecord = {
  ...
  mobile: string | null; iban: string | null; bankName: string | null; monthlySalaryHalalas: number; isCompanySponsored: boolean; archivedAt: string | null; beneficiaryName: string | null; clientSite: string; assignmentStartDate: string | null;
  ...
  async function createWorker(form: HTMLFormElement) {
    ...
    try {
      ...
      for(let index=0;index<count;index++){
        ...
        for(const name of ["workerNumber","iqamaNumber","fullName","mobile","iban","bankName","monthlySalary","isCompanySponsored","iqamaExpiry","medicalInsuranceExpiry","photo","iqamaDocument","ibanCertificate"])payload.set(name,source.get(`${name}:${index}`) as FormDataEntryValue);
...
function WorkerModal({ busy, onClose, onSubmit }: { busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  const [sponsoredWorkers,setSponsoredWorkers]=useState<Record<number,boolean>>({});
