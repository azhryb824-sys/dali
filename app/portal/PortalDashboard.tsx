...
function CreateUserModal({ busy, onClose, onSubmit }: { busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  return <div className="modal-layer"><button className="drawer-backdrop" aria-label="إغلاق نافذة إضافة مستخدم" onClick={onClose}/><section className="record-modal create-user-modal" role="dialog" aria-modal="true" aria-label="إضافة مستخدم"><div className="drawer-head"><div><span>إدارة المستخدمين</span><h2>إضافة مستخدم جديد</h2></div><button onClick={onClose} aria-label="إغلاق"><Icon name="close"/></button></div><form onSubmit={submit}>
    ...
    <label>الصلاحية العليا<select name="functionalRole" defaultValue=""><option value="">دون صلاحية عليا إضافية</option><option value="system_owner">مالك النظام — جميع الصلاحيات</option><option value="system_admin">مشرف النظام — جميع الصلاحيات والاعتماد</option></select></label>
...
function DocumentTable({ documents, canShare, busy, onShare }: { documents: CompanyDocument[]; canShare: boolean; busy: string | null; onShare: (id: number) => Promise<void> }) {
  ...
  return <div className="management-table-wrap"><table className="management-table documents-table"><thead><tr><th>المرجع</th><th>المستند</th><th>التصنيف</th><th>الجهة</th><th>الانتهاء</th><th>الحجم</th><th>الإجراءات</th></tr></thead><tbody>{documents.map((item) => { const days = daysUntil(item.expiryDate); const expiryClass = days < 0 ? "expired" : days <= 30 ? "expiring" : ""; return <tr key={item.id}><td dir="ltr"><b>{item.referenceCode}</b></td><td><strong>{item.title}</strong><small>{item.source === "generated" ? issuedTypeLabels[item.documentType || ""] || "PDF صادر" : item.fileName}{item.lockedUntil ? ` · حجز نظامي حتى ${formatDate(item.lockedUntil)}` : item.retentionUntil ? ` · مراجعة الاحتفاظ ${formatDate(item.retentionUntil)}` : ""}</small></td><td><span className={`doc-kind ${item.source === "generated" ? "generated" : ""}`}>{item.source === "generated" ? "صادر من النظام" : documentCategoryLabels[item.category] || item.category}</span></td><td>{item.counterparty || "—"}</td><td><span className={`expiry-pill ${expiryClass}`}>{expiryText(item.expiryDate)}</span></td><td>{formatBytes(item.sizeBytes)}</td><td><div className="document-actions"><PdfDownloadButton href={`/api/portal/documents/${item.id}?inline=1`} title={item.title} inline icon="documents" label="معاينة"/><PdfDownloadButton href={`/api/portal/documents/${item.id}`} title={item.title} icon="download" label="تنزيل"/>{canShare && <button disabled={busy === `share-${item.id}`} onClick={() => void onShare(item.id)} aria-label={`مشاركة ${item.title}`}><Icon name="share"/><span>{busy === `share-${item.id}` ? "جارٍ الإنشاء" : "مشاركة"}</span></button>}</div></td></tr>; })}</tbody></table></div>;
}
