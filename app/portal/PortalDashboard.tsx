...
import { matchesWorkforceProfession, requirementsForProfession, workforceNationalities, workforceProfessions } from "@/lib/workforce-requirements";
...
type WorkforceRequest = {
  ...
  quotationItemsJson: string | null;
  quotationTermsJson: string | null;
...
type WorkerRecord = {
  ...
  sponsorshipType: string;
  sponsorName: string | null;
  ajirContractStatus: string;
  archivedAt: string | null;
  beneficiaryName: string | null;
  clientSite: string;
  assignmentStartDate: string | null;
  iqamaExpiry: string | null;
  medicalInsuranceExpiry: string | null;
  status: string;
  createdAt: string;
  updatedAt: string;
};
...
const requestStatuses: Record<RequestStatus, { label: string; className: string }> = {
  ...
  reviewing: { label: "قيد المراجعة", className: "status-reviewing" },
  contacted: { label: "تم التواصل", className: "status-contacted" },
...
const functionalRoleLabels: Record<string, string> = {
  ...
  client_consultant: "ممثل العميل أو الاستشاري",
  subcontractor: "مقاول باطن",
  accountant: "المحاسب",
  lawyer: "محامي",
  legal_supervisor: "محامي مشرف",
...
const departmentLabels: Record<PortalDepartment, string> = {
  ...
  workforce: "شؤون العمالة",
  construction: "المقاولات والمشروعات",
  general: "صلاحية عامة",
};
const financeLabels: Record<string, string> = {
  worker_salary: "راتب عامل",
  worker_advance: "سلفة عامل",
  worker_deduction: "خصم عامل",
  worker_violation: "مخالفة عامل",
  worker_expense: "مصروف عمالة",
  workforce_invoice: "فاتورة عمالة",
  receipt_voucher: "سند قبض",
  payment_voucher: "سند صرف",
  progress_claim: "مستخلص عمالة",
  invoice: "فاتورة عميل",
  expense: "مصروف",
  payroll: "مسير رواتب",
  advance: "عُهدة أو سلفة",
};
const workforceExpenseLabels: Record<string, string> = {
  accommodation: "السكن",
  transportation: "النقل",
  iqama_visa: "الإقامة والتأشيرات",
  medical_insurance: "التأمين الطبي",
  tickets: "التذاكر",
  uniforms_safety: "الزي ومعدات السلامة",
  training_certificates: "التدريب والشهادات",
  treatment: "العلاج",
  meals: "الإعاشة والوجبات",
  other: "مصروف آخر",
};
...
function legalFileSummary(value: string | null) {
  if (!value) return null;
  try {
    const file = JSON.parse(value) as {
      documents?: unknown[];
      payments?: unknown[];
      finances?: unknown[];
      workers?: unknown[];
      contract?: { referenceCode?: string };
    };
    return {
      documents: file.documents?.length || 0,
      payments: file.payments?.length || 0,
      finances: file.finances?.length || 0,
      workers: file.workers?.length || 0,
      referenceCode: file.contract?.referenceCode || "",
    };
  } catch {
    return null;
  }
  ...
  const paths: Record<IconName, React.ReactNode> = {
    ...
    employees: (
      <>
        ...
        <path d="M2.5 21c.7-4 2.8-6 6.5-6s5.8 2 6.5 6M16 4.5a4 4 0 0 1 0 7.5M17 15c2.5.5 3.9 2.4 4.5 6" />
      </>
    ),
    finance: (
      <>
    ...
    legal: (
      <>
        <path d="M12 3v18M6 6h12M7 6l-4 7h8L7 6ZM17 6l-4 7h8l-4-7ZM7 21h10" />
      </>
    ),
    workforce: (
      <>
    ...
    users: (
      <>
        ...
        <path d="M1.5 21c.5-4 2.5-6 6.5-6s6 2 6.5 6M14 15c4.8-.2 7.4 1.8 8.5 6" />
      </>
    ),
    search: (
      <>
    ...
    stamp: (
      <>
        <path d="M8 13h8M9 13c0-3 1-4 1-7a2 2 0 0 1 4 0c0 3 1 4 1 7M5 13h14v4H5zM7 21h10" />
      </>
    ),
    mail: (
      <>
...
export default function PortalDashboard({
  ...
  initialLegal,
  initialWorkers,
  initialWorkerAttachments,
  initialDocuments,
  initialAssets,
  initialContracts,
  initialContractProfessions,
  initialContractAssignments,
  initialConversations,
  initialConversationMessages,
  initialBusinessHours,
  initialChatAutomation,
  ...
  initialFinance: FinanceRecord[];
  initialLegal: LegalRecord[];
  initialWorkers: WorkerRecord[];
  initialWorkerAttachments: WorkerAttachment[];
  ...
  initialContractAssignments: ContractAssignment[];
  initialConversations: VisitorConversation[];
  initialConversationMessages: VisitorMessage[];
  ...
  useEffect(() => {
    let timer = 0;
    const refreshWithoutReload = () => {
      window.clearTimeout(timer);
      timer = window.setTimeout(() => router.refresh(), 300);
    };
    window.addEventListener("dali-server-changes", refreshWithoutReload);
    return () => {
      window.clearTimeout(timer);
      window.removeEventListener("dali-server-changes", refreshWithoutReload);
    };
  }, [router]);
  ...
  const hasPermission = (permission: string) => currentUser.role === "admin" || (functionalAdmin && ["contracts.read", "contracts.write"].includes(permission)) || currentUser.functionalPermissions.includes("*") || currentUser.functionalPermissions.includes(permission);
  ...
  useEffect(() => {
    ...
    const endForIdle = () => {
      router.replace("/api/portal/session/end?returnTo=%2Fportal&reason=idle-timeout");
    };
    const endForNightCutoff = () => {
      router.replace("/api/portal/session/end?returnTo=%2Fportal&reason=attendance-20h-idle-cutoff");
    };
    const registerActivity = () => {
      ...
      lastTouch = Date.now();
      void fetch("/api/portal/session/touch", {
        method: "POST",
        cache: "no-store",
      })
        .then((response) => {
  ...
  const requestCounts = useMemo(
    () => ({
      ...
      new: requests.filter((item) => safeRequestStatus(item.status) === "new").length,
  ...
  async function sendConversationReply(conversationId: string, form: HTMLFormElement) {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر إرسال الرد.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function updateConversationStatus(conversationId: string, status: "waiting" | "open" | "closed") {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر تحديث المحادثة.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function saveBusinessHours(form: HTMLFormElement) {
    ...
    const config = {
      ...
      closedDates: String(data.get("closedDates") || "")
        .split(/[\s,،]+/)
        .filter(Boolean),
      specialOpenDates: String(data.get("specialOpenDates") || "")
        .split(/[\s,،]+/)
        .filter(Boolean),
    };
    const automation = {
      ...
      rules: chatAutomation.rules.map((rule) => ({
        ...
        keywords: String(data.get(`ruleKeywords:${rule.id}`) || "")
          .split(/[\n,،]+/)
          .map((item) => item.trim())
          .filter(Boolean),
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر حفظ الإعدادات.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function updateNotificationState(action: "read" | "read-all" | "dismiss", ids: number[] = []) {
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر تحديث الإشعارات.");
    }
  }
  ...
  async function sendRequestReply(requestId: number, form: HTMLFormElement) {
    ...
    try {
      ...
      const payload = {
        ...
        functionalRoles: formData.getAll("functionalRoles"),
      };
      ...
      const result = (await readApiJson(response)) as {
        reply?: WorkforceRequestReply | null;
  ...
  async function createRecord(entity: RecordEntity, form: HTMLFormElement) {
    ...
    try {
      ...
      const result = (await readApiJson(response)) as {
        record?: EmployeeRecord | FinanceRecord | LegalRecord | WorkerRecord;
        error?: string;
      };
      ...
      setModal(null);
      notify("تم حفظ السجل بنجاح.");
    } catch (error) {
      notify(error instanceof Error ? error.message : "تعذّر حفظ السجل.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function createEmployee(form: HTMLFormElement) {
    ...
    try {
      ...
      setModal(null);
      notify("تم إنشاء ملف الموظف وربطه بالمستخدم والموارد البشرية والرواتب.");
      void refreshNotifications(true);
    } catch (error) {
      notify(error instanceof Error ? error.message : "تعذّر إنشاء ملف الموظف");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function deleteEmployee(id: number) {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذر حذف الموظف");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function createWorker(form: HTMLFormElement) {
    ...
    const createdWorkers: WorkerRecord[] = [];
    const createdAttachments: WorkerAttachment[] = [];
    try {
      const source = new FormData(form);
      const count = Math.max(1, Number(source.get("workerCount") || 1));
      for (let index = 0; index < count; index++) {
        ...
        payload.set("nationality", source.get("nationality") as FormDataEntryValue);
        payload.set("profession", source.get("profession") as FormDataEntryValue);
        ...
        createdWorkers.push(result.worker);
        createdAttachments.push(...result.attachments);
      ...
      setModal(null);
      notify(`تم إنشاء ${createdWorkers.length} ملف عامل وإرفاق صور الإقامة والمتطلبات.`);
    } catch (error) {
      notify(`${createdWorkers.length ? `تم حفظ ${createdWorkers.length} عامل بنجاح. ` : ""}${error instanceof Error ? error.message : "تعذّر إنشاء ملف العامل."}`);
    } finally {
      setBusy(null);
    }
  }
  ...
  async function uploadWorkerAttachment(workerId: number, form: HTMLFormElement) {
    ...
    const data = new FormData(form);
    data.set("workerId", String(workerId));
    try {
      const response = await fetch("/api/portal/workers/attachments", {
        method: "POST",
        body: data,
      });
      const result = (await readApiJson(response)) as {
      ...
      form.reset();
      notify("تمت إضافة الشهادة إلى ملف العامل.");
    } catch (error) {
      notify(error instanceof Error ? error.message : "تعذّر رفع الشهادة.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function updateRecordStatus(entity: RecordEntity, id: number, status: string) {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر تحديث السجل.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function updateRequestStatus(id: number, status: RequestStatus) {
    ...
    try {
      ...
      const response = await fetch("/api/portal/requests", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id, status, version: current?.version }),
      });
  ...
  async function createUser(form: HTMLFormElement) {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّرت إضافة المستخدم.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function uploadDocument(form: HTMLFormElement) {
    ...
    try {
      ...
      notify("تم رفع المستند وحفظه في مركز المستندات.");
    } catch (error) {
      notify(error instanceof Error ? error.message : "تعذّر رفع المستند.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function issueDocument(form: HTMLFormElement) {
    ...
    try {
      const response = await fetch("/api/portal/documents/generate", {
        method: "POST",
        body: data,
      });
      const result = (await readApiJson(response)) as {
        document?: CompanyDocument;
        contract?: WorkforceContract | null;
        professions?: ContractProfession[];
        assignments?: ContractAssignment[];
        workers?: WorkerRecord[];
        financialRecord?: FinanceRecord | null;
        capacity?: Array<{
          profession: string;
          requiredCount: number;
          ...
          registeredShortage: number;
          availableShortage: number;
      ...
      setDocuments((items) => [result.document as CompanyDocument, ...items]);
      if (result.contract) {
        setContracts((items) => [result.contract as WorkforceContract, ...items]);
        setContractualTab("contracts");
        setCommercialRevision(value => value + 1);
        changeView("contractual-documents");
        void refreshNotifications(true);
      }
      ...
      notify(error instanceof Error ? error.message : "تعذّر إصدار المستند.");
      throw error;
    } finally {
      setBusy(null);
    }
  ...
  async function assignWorkerToContract(contractId: number, contractProfessionId: number, workerId: number) {
    ...
    try {
      const response = await fetch(`/api/portal/contracts/${contractId}/workers`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ contractProfessionId, workerId }),
      });
      const result = (await readApiJson(response)) as {
  ...
  async function updateContractStatus(
    ...
    try {
      ...
      if (status === "approved" && !stampId) {
        ...
        const stampData = (await readApiJson(stampResponse)) as {
      ...
      const response = await fetch(`/api/portal/contracts/${contractId}/status`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ status, reason, reasonCode, stampId }),
      });
      const result = (await readApiJson(response)) as {
  ...
  async function recordContractAbsence(contractId: number, contractProfessionId: number, absenceDate: string, absenceEndDate: string, workerId: number | null, replacementWorkerId: number | null, absentCount: number, notes: string) {
    setBusy(`contract-absence-${contractId}`);
    try {
      const response = await fetch(`/api/portal/contracts/${contractId}/attendance`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  async function editContract(
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر تعديل العقد.");
      throw error;
    } finally {
      setBusy(null);
    }
  ...
  async function deleteContract(contract: WorkforceContract) {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "تعذّر حذف العقد.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function uploadAsset(slot: "stamp" | "signature", form: HTMLFormElement) {
    ...
    if (!file) {
      notify("اختر صورة الختم أو التوقيع أولاً.");
      return;
    }
    if (!["image/png", "image/jpeg"].includes(file.type) || file.size > 5 * 1024 * 1024) {
      notify("استخدم صورة PNG أو JPG لا تتجاوز 5 ميجابايت.");
      return;
    }
    setBusy(`asset-${slot}`);
    const data = new FormData(form);
    data.set("slot", slot);
    try {
      const response = await fetch("/api/portal/company-assets", {
        method: "POST",
        body: data,
      });
      const result = (await readApiJson(response)) as {
      ...
      form.reset();
      notify(slot === "stamp" ? "تم اعتماد ختم الشركة." : "تم اعتماد توقيع الشركة.");
    } catch (error) {
      notify(error instanceof Error ? error.message : "تعذّر حفظ الملف.");
    } finally {
      setBusy(null);
    }
  }
  ...
  const viewTitle: Record<View, string> = {
    ...
    finance: "الإدارة المالية",
    legal: "الشؤون القانونية",
    government: "العلاقات الحكومية والامتثال",
    workforce: "شؤون العمالة",
    operations: "المبيعات والتشغيل",
    representatives: "إدارة المناديب",
    construction: "المقاولات والمشروعات",
    conversations: "المحادثات المباشرة",
    "workforce-supervision": "إدارة الإشراف على العمالة",
    "contractual-documents": "العقود والعروض والخطابات",
    ...
    <main className="admin-shell">
      ...
      <aside className={`admin-sidebar ${menuOpen ? "sidebar-open" : ""}`}>
        ...
        <nav aria-label="أقسام النظام">
          ...
          <button className={view === "tasks" ? "active" : ""} onClick={() => changeView("tasks")}>
            <Icon name="check" />
          ...
          {canAccessConversations && (
            <button className={view === "conversations" ? "active" : ""} onClick={() => changeView("conversations")}>
              ...
              {(unreadConversationMessages > 0 || waitingConversations > 0) && <b>{Math.min(99, unreadConversationMessages || waitingConversations)}</b>}
            </button>
          )}
          {canAccess("employees") && (
            <button className={view === "employees" ? "active" : ""} onClick={() => changeView("employees")}>
              <Icon name="employees" />
          ...
          {canAccess("finance") && (
            <button className={view === "finance" ? "active" : ""} onClick={() => changeView("finance")}>
              <Icon name="finance" />
              <span>الإدارة المالية</span>
            </button>
          )}
          {canAccess("legal") && (
            <button className={view === "legal" ? "active" : ""} onClick={() => changeView("legal")}>
              <Icon name="legal" />
              <span>الشؤون القانونية</span>
              {legalAlerts > 0 && <b>{legalAlerts}</b>}
            </button>
          )}
          {canAccessGovernment && (
            <button className={view === "government" ? "active" : ""} onClick={() => changeView("government")}>
              <Icon name="website" />
              <span>العلاقات الحكومية</span>
            </button>
          )}
          {canAccess("workforce") && (
            <button className={view === "workforce" ? "active" : ""} onClick={() => changeView("workforce")}>
              <Icon name="workforce" />
              <span>شؤون العمالة</span>
              {requestCounts.new + workerAlerts + incompleteWorkerFiles > 0 && <b>{requestCounts.new + workerAlerts + incompleteWorkerFiles}</b>}
            </button>
          )}
          {canAccessOperations && (
            <button className={view === "operations" ? "active" : ""} onClick={() => changeView("operations")}>
              <Icon name="finance" />
          ...
          {canAccessConstruction && (
            <button className={view === "construction" ? "active" : ""} onClick={() => changeView("construction")}>
              <Icon name="legal" />
              <span>المقاولات والمشروعات</span>
            </button>
          )}
          {canAccessContracts && canAccess("workforce") && (
            <button className={view === "workforce-supervision" ? "active" : ""} onClick={() => changeView("workforce-supervision")}>
              <Icon name="workforce" />
              <span>إدارة الإشراف على العمالة</span>
            </button>
          )}
          {canAccessContracts && (
            <button className={view === "contractual-documents" ? "active" : ""} onClick={() => changeView("contractual-documents")}>
              <Icon name="documents" />
              <span>العقود والعروض والخطابات</span>
            </button>
          )}
          {canAccessDocuments && (
            <button className={view === "documents" ? "active" : ""} onClick={() => changeView("documents")}>
              <Icon name="documents" />
              <span>مستندات الشركة</span>
          ...
          {canAccessWebsite && (
            <button className={view === "website" ? "active" : ""} onClick={() => changeView("website")}>
              <Icon name="website" />
          ...
          {isRoot && (
            <button className={view === "users" ? "active" : ""} onClick={() => changeView("users")}>
              ...
              {users.some((item) => item.status === "pending") && <i />}
            </button>
          )}
        </nav>
        <div className="sidebar-foot">
          <div className="security-note">
            <span>✓</span>
            <p>
              <strong>اتصال محمي</strong>تُطبّق الصلاحيات من جهة الخادم.
            </p>
          </div>
      ...
      <section className="admin-workspace">
        <header className="admin-topbar">
          <button className="mobile-menu" aria-label="فتح القائمة" onClick={() => setMenuOpen(true)}>
            <Icon name="menu" />
          </button>
          <div className="topbar-title">
            <span>شركة دالي للتشغيل والصيانة</span>
          ...
          <div className="topbar-actions">
            <div className="notification-shell" ref={notificationShellRef}>
              <button className="notification" aria-label="فتح مركز الإشعارات" aria-expanded={notificationsOpen} onClick={() => setNotificationsOpen((open) => !open)}>
                <Icon name="bell" />
                {unreadNotifications > 0 && <b>{unreadNotifications > 99 ? "99+" : unreadNotifications}</b>}
              </button>
              {notificationsOpen && (
            ...
            <div className="user-menu">
              ...
              <p>
                ...
                <small>
                  {activeRoleLabel} · {departmentLabels[currentUser.department]}
                </small>
              </p>
            </div>
          </div>
        ...
        <div className="admin-content">
          ...
          {view === "overview" && (
            <>
              <div className="content-heading">
                <div>
                  <p className="admin-eyebrow">مركز العمليات</p>
              ...
              <section className="metric-grid module-metrics">
                {canAccess("employees") && (
                  <button onClick={() => changeView("employees")}>
                    <span className="metric-icon red">
                      <Icon name="employees" />
                    </span>
                    <p>
                      الموظفون<strong>{employees.length}</strong>
                    </p>
                    <small>{employees.filter(isCurrentEmployee).length} على رأس العمل</small>
                  </button>
                )}
                {canAccess("finance") && (
                  <button onClick={() => changeView("finance")}>
                    <span className="metric-icon navy">
                      <Icon name="finance" />
                    </span>
                    <p>
                      السجلات المالية<strong>{finance.length}</strong>
                    </p>
                    <small>{formatMoney(financialTotal)} إجمالي مسجّل</small>
                  </button>
                )}
                {canAccess("legal") && (
                  <button onClick={() => changeView("legal")}>
                    <span className="metric-icon sand">
                      <Icon name="legal" />
                    </span>
                    <p>
                      الملفات القانونية<strong>{legal.length}</strong>
                    </p>
                    <small>{legalAlerts} تنبيه خلال 45 يوماً</small>
                  </button>
                )}
                {canAccess("workforce") && (
                  <button onClick={() => changeView("workforce")}>
                    <span className="metric-icon green">
                      <Icon name="workforce" />
                    </span>
                    <p>
                      العمالة<strong>{workers.length}</strong>
                    </p>
                    <small>
                      {workers.filter((item) => item.status === "available").length} متاح · {activeBeneficiaries} جهة مستفيدة
                    </small>
                  </button>
                )}
                {canAccessConversations && (
                  <button onClick={() => changeView("conversations")}>
                    ...
                    <p>
                      المحادثات<strong>{conversations.length}</strong>
                    </p>
                    <small>
                      {waitingConversations} تنتظر الرد · {unreadConversationMessages} رسالة غير مقروءة
                    </small>
                  </button>
                )}
                {canAccessDocuments && (
                  <button onClick={() => changeView("documents")}>
                    <span className="metric-icon navy">
                      <Icon name="documents" />
                    </span>
                    <p>
                      مستندات الشركة<strong>{documents.length}</strong>
                    </p>
                    <small>{documentAlerts} تنبيه انتهاء خلال 30 يوماً أو مستند منتهٍ</small>
                  </button>
                )}
                ...
                {canAccessGovernment && (
                  <button className={employeeComplianceAlerts ? "metric-attention" : ""} onClick={() => changeView("government")}>
                    ...
                    <p>
                      العلاقات الحكومية
                      <strong>{employeeComplianceAlerts}</strong>
                    </p>
                    <small>{employeeComplianceAlerts ? "استحقاقات موظفين خلال أقل من 29 يومًا" : "لا توجد استحقاقات موظفين عاجلة"}</small>
                  </button>
                )}
              </section>
              ...
              {currentUser.role === "employee" && currentUser.department === "general" && currentUser.functionalRoles.length === 0 ? (
                <section className="employee-home">
                  ...
                  <h1>لم يُحدَّد قسمك بعد</h1>
                  <p>يرجى التواصل مع مدير النظام لإسناد حسابك إلى أحد الأقسام التشغيلية الأربعة.</p>
                ...
                <section className="overview-grid">
                  <article className="panel operations-panel">
                    <div className="panel-head">
                      <div>
                        <h2>الأقسام التشغيلية</h2>
                        <p>الوحدات المتاحة وفقاً لصلاحية حسابك</p>
                      </div>
                    </div>
                  ...
                  {currentUser.role !== "employee" && (
                    <article className="panel activity-panel">
                      <div className="panel-head">
                        <div>
                          <h2>سجل النشاط</h2>
                          <p>آخر التحديثات الإدارية</p>
                        </div>
                      </div>
                      <div className="activity-list">
                        {initialActivity.length === 0 ? (
                          <div className="empty-small">لا يوجد نشاط مسجّل بعد.</div>
                        ) : (
          ...
          {view === "employees" && canAccess("employees") && (
            <ModuleSection eyebrow="الموارد البشرية" title="إدارة الموظفين" description="ملفات الموظفين وحالتهم الوظيفية وبيانات الالتحاق." actionLabel="إضافة موظف" canWrite={canWrite} onAdd={() => setModal("employees")}>
              <section className="metric-grid compact-metrics">
                <Metric label="إجمالي الموظفين" value={employees.length} note="جميع الملفات المسجّلة" />
                <Metric label="على رأس العمل" value={employees.filter(isCurrentEmployee).length} note="ملفات وظيفية قائمة" />
                <Metric label="في إجازة" value={employees.filter((item) => item.status === "leave").length} note="إجازات حالية" />
          ...
          {view === "finance" && canAccess("finance") && (
            <ModuleSection eyebrow="مالية القوى العاملة" title="الإدارة المالية" description="رواتب العمالة والسلف والخصميات والمصروفات والفواتير والسندات المرتبطة بالعامل والعقد." actionLabel="إضافة حركة مالية" canWrite={canWrite} onAdd={() => setModal("finance")}>
              <section className="metric-grid compact-metrics finance-metrics">
                <Metric label="رواتب العمالة" value={formatMoney(payrollTotal)} note="إجمالي الرواتب المسجّلة" />
                <Metric label="السلف" value={formatMoney(advancesTotal)} note="سلف العمالة" />
                <Metric label="الخصميات" value={formatMoney(deductionsTotal)} note="خصميات مسجّلة" />
                <Metric label="مصروفات العمالة" value={formatMoney(workforceExpensesTotal)} note="سكن ونقل وإقامات وغيرها" />
                <Metric label="فواتير ومستخلصات" value={formatMoney(workforceInvoicesTotal)} note="مرتبطة بعقود العمالة" />
                <Metric label="إجمالي الحركات" value={formatMoney(financialTotal)} note={`${finance.length} حركة مالية`} />
              </section>
              <PaymentManagementDashboard />
              {canWrite && <FinanceDocumentActions onIssue={openIssueDocument} />}
              <ManagementPanel query={query} setQuery={setQuery} placeholder="ابحث بالمرجع أو العامل أو العقد أو البيان">
                <FinanceTable records={finance} workers={workers} contracts={contracts} query={query} canWrite={canWrite} busy={busy} onStatus={(id, status) => updateRecordStatus("finance", id, status)} />
              </ManagementPanel>
          ...
          {view === "legal" && canAccess("legal") && (
            <ModuleSection eyebrow="العقود والامتثال" title="الشؤون القانونية" description="متابعة العقود والقضايا والتراخيص والتنبيهات النظامية." actionLabel="إضافة ملف قانوني" canWrite={canWrite && canManageLegalCases} onAdd={() => setModal("legal")}>
              ...
              <section className="metric-grid compact-metrics">
                ...
                <Metric label="قيد المراجعة" value={legal.filter((item) => item.status === "reviewing").length} note="ملفات مفتوحة" />
          ...
          {view === "workforce" && canAccess("workforce") && (
            <ModuleSection eyebrow="التشغيل الميداني" title="شؤون العمالة" description="إدارة ملفات العمال وتوزيعهم على المواقع ومتابعة طلبات العملاء." actionLabel="إضافة عامل" canWrite={canWrite} onAdd={() => setModal("workforce")}>
              <section className="metric-grid compact-metrics workforce-metrics">
                <Metric label="إجمالي العمالة" value={workers.length} note="كل الملفات المسجّلة" />
                <Metric label="مرتبطون بجهات" value={workers.filter((item) => item.status === "assigned").length} note={`${activeBeneficiaries} جهة مستفيدة حالياً`} />
              ...
              <section className="panel request-panel workforce-requests">
                <div className="panel-head">
                  <div>
                    <h2>طلبات القوى العاملة من الموقع</h2>
                    <p>الطلبات الواردة مباشرة من العملاء</p>
                  </div>
                  <span className="panel-count">{requestCounts.new} جديد</span>
                </div>
                <div className="table-tools">
                  <label className="search-box">
                    <Icon name="search" />
                    <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث بالاسم أو رقم الطلب" />
                  </label>
                  <div className="filter-tabs" role="group" aria-label="تصفية الطلبات">
                    {(["all", "new", "reviewing", "contacted", "closed"] as const).map((value) => (
                      <button key={value} className={requestFilter === value ? "active" : ""} onClick={() => setRequestFilter(value)}>
                        {value === "all" ? "الكل" : requestStatuses[value].label}
                      </button>
                    ))}
                  </div>
                </div>
                <RequestTable requests={visibleRequests} onSelect={setSelectedId} />
              </section>
            </ModuleSection>
          )}
            ...
            <ModuleSection eyebrow="التحكم في الوصول" title="المستخدمون والصلاحيات" description="اعتماد مسبب، وأقل صلاحية لازمة، وإبطال تلقائي للجلسات عند كل تغيير أمني." actionLabel="إضافة مستخدم" canWrite onAdd={() => setUserModal(true)}>
              <section className="panel users-panel">
                <div className="panel-head">
                  <div>
                    <h2>حسابات النظام</h2>
                    <p>{users.filter((item) => item.status === "pending").length} حساب بانتظار الاعتماد · لا توجد كلمات مرور محفوظة في النظام</p>
                  </div>
                </div>
      ...
      <form onSubmit={submit}>
        <div className="user-access-identity">
          ...
          <div className="user-identity">
            ...
            <time>آخر دخول: {user.lastLoginAt ? formatDate(user.lastLoginAt, true) : "لم يسجّل الدخول"}</time>
          </div>
          <span className={`access-request-state ${requestComplete ? "complete" : "incomplete"}`}>{requestComplete ? "طلب مكتمل" : "بيانات ناقصة"}</span>
        </div>
        {user.requestSubmittedAt && (
          <div className="user-request-context">
            <div>
              <span>المسمى</span>
              <strong>{user.requestedJobTitle || "غير محدد"}</strong>
            </div>
            <div>
              <span>القسم المطلوب</span>
              <strong>{departmentLabels[(user.requestedDepartment || "general") as PortalDepartment] || user.requestedDepartment}</strong>
            </div>
            <p>{user.requestReason}</p>
            <time>أُرسل {formatDate(user.requestSubmittedAt, true)}</time>
          </div>
        )}
        ...
        {!accessLocked && (
          <div className="user-access-decision">
            <label>
              ...
              <textarea value={reason} onChange={(event) => setReason(event.target.value)} required minLength={10} maxLength={1000} rows={2} placeholder="اكتب مبررًا واضحًا يظهر في سجل التدقيق." />
            </label>
            <button className="admin-primary" disabled={busy}>
              {busy ? "جارٍ الحفظ..." : "حفظ القرار الأمني"}
            </button>
          </div>
        )}
        ...
        {!self && passwordResetOpen && !protectedOwner && (
          <div className="user-password-reset" role="group" aria-label={`إعادة تعيين كلمة مرور ${user.displayName}`}>
            ...
            <label>
              كلمة المرور المؤقتة
              <input value={temporaryPassword} onChange={(event) => setTemporaryPassword(event.target.value)} type="password" minLength={12} maxLength={128} autoComplete="new-password" dir="ltr" placeholder="12 خانة: كبير وصغير ورقم ورمز" />
            </label>
            <label>
              تأكيد كلمة المرور المؤقتة
              <input value={temporaryPasswordConfirmation} onChange={(event) => setTemporaryPasswordConfirmation(event.target.value)} type="password" minLength={12} maxLength={128} autoComplete="new-password" dir="ltr" placeholder="أعد إدخال كلمة المرور المؤقتة" />
            </label>
            ...
            <button
              ...
              disabled={busy || !temporaryPasswordStrong || !temporaryPasswordsMatch}
              onClick={() =>
                void onResetPassword(user.email, temporaryPassword).then((saved) => {
                  if (saved) {
                    setTemporaryPassword("");
                    setTemporaryPasswordConfirmation("");
                    setPasswordResetOpen(false);
            ...
            >
              {busy ? "جارٍ الحفظ..." : "حفظ كلمة المرور المؤقتة"}
            </button>
...
function CreateUserModal({ roles, busy, onClose, onSubmit }: { roles: PortalRoleDefinition[]; busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  return (
    <div className="modal-layer">
      <button className="drawer-backdrop" aria-label="إغلاق نافذة إضافة مستخدم" onClick={onClose} />
      <section className="record-modal create-user-modal" role="dialog" aria-modal="true" aria-label="إضافة مستخدم">
        ...
        <form onSubmit={submit}>
          <label>
            الاسم الكامل
            <input name="displayName" required minLength={3} maxLength={160} autoComplete="name" />
          </label>
          <label>
            رقم الهوية
            <input name="identifier" required inputMode="numeric" pattern="[0-9]{10}" maxLength={10} dir="ltr" placeholder="10 أرقام" />
          </label>
          <label>
            البريد الإلكتروني
            <input name="email" type="email" required maxLength={254} dir="ltr" autoComplete="email" />
          </label>
          <label>
            كلمة المرور المؤقتة
            <input
          ...
          <fieldset className="quick-permission-profile span-two">
            ...
            {roles.map((role) => (
              <label key={role.roleKey}>
                <input type="checkbox" name="functionalRoles" value={role.roleKey} checked={selectedRoles.includes(role.roleKey)} onChange={() => toggleRole(role.roleKey)} />
                <span>
                  <b>{role.labelAr}</b>
                  <small>{role.description || "صلاحيات محددة حسب الدور"}</small>
                </span>
              </label>
            ))}
          </fieldset>
          ...
          <fieldset className="quick-permission-profile span-two">
            <legend>حزمة الصلاحيات السريعة</legend>
            <label>
              <input type="radio" name="permissionProfile" value="role_default" defaultChecked />
              <span>
                <b>صلاحيات الأدوار</b>
                <small>اتحاد صلاحيات جميع الأدوار المختارة.</small>
              </span>
            </label>
            <label>
              <input type="radio" name="permissionProfile" value="operator" />
              <span>
                <b>تنفيذ دون اعتماد</b>
                <small>إنشاء وتعديل دون اعتماد أو دفع أو ترحيل.</small>
              </span>
            </label>
            <label>
              <input type="radio" name="permissionProfile" value="read_only" />
              <span>
                <b>اطلاع فقط</b>
                <small>عرض البيانات دون أي تعديل.</small>
              </span>
            </label>
          </fieldset>
    ...
    <div className="global-search" ref={wrapRef}>
        ...
        <input
          ...
          onKeyDown={(event) => {
            ...
            if (event.key === "Enter" && visibleResults[0]) {
              event.preventDefault();
              onSelect(visibleResults[0]);
              setOpen(false);
            }
          }}
          placeholder="ابحث في جميع أقسام النظام..."
      ...
      {open && value.trim().length >= 2 && (
        <div className="global-search-results" id="global-search-results" role="listbox">
          {loading ? (
            <div className="global-search-empty">جارٍ البحث في السجلات المصرح بها...</div>
            ...
            <div className="global-search-empty">لا توجد نتائج مطابقة في الأقسام المتاحة لحسابك.</div>
          )}
        </div>
      )}
    </div>
...
function NotificationPopover({ notifications, onOpen, onReadAll, onViewAll }: { notifications: PortalNotification[]; onOpen: (item: PortalNotification) => void; onReadAll: () => void; onViewAll: () => void }) {
  ...
  return (
    <aside className="notification-popover" aria-label="الإشعارات الحديثة">
      <header>
        <div>
          <strong>الإشعارات</strong>
          <span>{unread} غير مقروء</span>
        </div>
        {unread > 0 && <button onClick={onReadAll}>تعليم الكل كمقروء</button>}
      </header>
      <div>
        {recent.length ? (
          recent.map((item) => (
            <button key={item.id} className={`${item.severity} ${item.readAt ? "read" : "unread"}`} onClick={() => onOpen(item)}>
          ...
          <div className="notification-empty">
            ...
            <span>ستظهر هنا الأحداث التي تحتاج إلى انتباهك.</span>
          </div>
        )}
      </div>
      <footer>
        <button onClick={onViewAll}>فتح مركز الإشعارات ←</button>
      </footer>
    </aside>
  );
}

function NotificationCenterView({ notifications, onOpen, onRead, onDismiss, onReadAll, onRefresh }: { notifications: PortalNotification[]; onOpen: (item: PortalNotification) => void; onRead: (id: number) => void; onDismiss: (id: number) => void; onReadAll: () => void; onRefresh: () => void }) {
  ...
  return (
    <>
      <div className="content-heading module-heading notification-heading">
        <div>
          <p className="admin-eyebrow">رصد تشغيلي موحّد</p>
          <h1>مركز الإشعارات</h1>
          <span>تنبيهات الطلبات والمراسلات والعقود والعمالة والمستندات والمالية والصلاحيات في مكان واحد.</span>
        </div>
        <div className="heading-actions">
          ...
          {unread > 0 && (
            <button className="admin-primary" onClick={onReadAll}>
              <Icon name="check" />
              تعليم الكل كمقروء
            </button>
          )}
      ...
      <section className="metric-grid compact-metrics notification-metrics">
        <Metric label="غير مقروء" value={unread} note="تحتاج إلى مراجعة" />
        <Metric label="عاجل" value={critical} note="يتطلب إجراء سريعاً" />
        <Metric label="تحذيرات" value={warning} note="مواعيد أو نواقص" />
        <Metric label="إجمالي نشط" value={notifications.length} note="بعد استبعاد المحلول" />
      </section>
      <section className="panel notification-center">
        ...
        <div className="notification-list">
          {visible.length ? (
            visible.map((item) => (
              <article key={item.id} className={`${item.severity} ${item.readAt ? "read" : "unread"}`}>
                <button className="notification-main" onClick={() => onOpen(item)}>
                  <span className="notification-module">
                    <Icon name={item.module === "conversations" ? "conversations" : item.module === "workforce" ? "workforce" : item.module === "finance" ? "finance" : item.module === "legal" ? "legal" : item.module === "documents" ? "documents" : item.module === "users" ? "users" : item.module === "employees" ? "employees" : "bell"} />
                  </span>
                  <p>
                    <strong>{item.title}</strong>
                    <span>{item.message}</span>
                    <small>
                      {formatDate(item.createdAt, true)} · {item.source === "system-check" ? "فحص تلقائي" : "حدث تشغيلي"}
                    </small>
                  </p>
                  <b>فتح السجل ←</b>
                </button>
                <div className="notification-item-actions">
                  {!item.readAt && <button onClick={() => onRead(item.id)}>مقروء</button>}
                  <button onClick={() => onDismiss(item.id)}>إخفاء</button>
                </div>
              </article>
            ))
          ) : (
            <div className="notification-empty large">
              <Icon name="check" />
              <strong>لا توجد إشعارات ضمن هذا التصنيف</strong>
              <span>النظام يحدّث التنبيهات تلقائياً عند دخول البوابة وكل 45 ثانية أثناء العمل.</span>
            </div>
          )}
        </div>
      </section>
...
function ConversationCenter({ conversations, messages, businessHours, automation, query, setQuery, canManageSettings, onSelect, onRefresh, onOpenSettings }: { conversations: VisitorConversation[]; messages: VisitorMessage[]; businessHours: BusinessHours; automation: ChatAutomationConfig; query: string; setQuery: (value: string) => void; canManageSettings: boolean; onSelect: (id: string) => void; onRefresh: () => void; onOpenSettings: () => void }) {
  ...
  return (
    <>
      <div className="content-heading module-heading chat-heading">
        <div>
          <p className="admin-eyebrow">تواصل لحظي مع زوار الموقع</p>
          <h1>المحادثات المباشرة</h1>
          <span>استقبال فوري، وتصنيف تلقائي للاحتياج، وتحويل واضح إلى الموظف المخوّل.</span>
        </div>
        <div className="heading-actions">
          <button className="admin-secondary" onClick={onRefresh}>
            تحديث الآن
          ...
          {canManageSettings && (
            <button className="admin-primary" onClick={onOpenSettings}>
              إعدادات الرد الآلي
            </button>
          )}
        </div>
      </div>
      <section className="metric-grid compact-metrics chat-metrics">
        <Metric label="تنتظر الرد" value={conversations.filter((item) => item.status === "waiting").length} note="أولوية فريق الخدمة" />
        <Metric label="رسائل غير مقروءة" value={unread} note="من زوار الموقع" />
        <Metric label="محادثات جارية" value={conversations.filter((item) => item.status === "open").length} note="تم الرد عليها" />
        <Metric label="إجمالي المحادثات" value={conversations.length} note="سجل محفوظ وقابل للبحث" />
      </section>
      <section className={`business-hours-panel ${businessHours.isOpen ? "open" : "closed"}`}>
        <span>
          <Icon name="conversations" />
        </span>
        <div>
          <strong>{businessHours.isOpen ? "الفريق متاح الآن للمحادثة" : "الموقع حالياً خارج ساعات الدوام"}</strong>
          <p>
            الدوام من {businessHours.opensAt} إلى {businessHours.closesAt} بتوقيت مكة · {businessHours.isOpen ? "الرد البشري متاح" : `العودة ${businessHours.nextOpenLabel}`}
          </p>
        </div>
        <b>{automation.enabled ? `${automation.rules.filter((rule) => rule.enabled).length} مسارات آلية` : "الرد الآلي متوقف"}</b>
      </section>
      <section className="panel live-conversation-panel">
        <div className="panel-head">
          <div>
            <h2>صندوق المحادثات</h2>
            <p>تحديث تفاضلي تلقائي كل 12 ثانية أثناء العمل</p>
          </div>
          <span className="panel-count">{rows.length} محادثة</span>
        </div>
        <div className="table-tools">
          <label className="search-box">
            <Icon name="search" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث برقم المحادثة أو اسم الزائر أو الجوال" />
          </label>
        ...
        {rows.length ? (
          <div className="conversation-table-wrap">
            <table className="management-table conversation-table">
              <thead>
                <tr>
                  <th>المحادثة</th>
                  <th>الزائر</th>
              ...
              <tbody>
                {rows.map((item) => {
                  const latest = lastMessage(item.id);
                  const unreadCount = unreadFor(item.id);
                  return (
                    <tr key={item.id} onClick={() => onSelect(item.id)} className={unreadCount ? "unread" : ""}>
                      <td>
                        <strong dir="ltr">{item.trackingCode}</strong>
                        <small>{formatDate(item.createdAt, true)}</small>
                      </td>
                      <td>
                        <strong>{item.visitorName}</strong>
                        <small dir="ltr">{item.visitorMobile}</small>
                      </td>
                      <td>{item.subject}</td>
                      <td>
                        <strong>{latest?.senderType === "visitor" ? item.visitorName : latest?.senderType === "system" ? "الرد الآلي" : latest?.senderName || "—"}</strong>
                        <small>{latest?.body.slice(0, 70) || "لا توجد رسائل"}</small>
                      </td>
                      <td>{item.assignedTo ? <span dir="ltr">{item.assignedTo}</span> : "غير مسندة"}</td>
                      <td>
                        <span className={`conversation-status ${item.status}`}>{conversationStatusLabels[item.status] || item.status}</span>
                      </td>
                      <td>{unreadCount > 0 ? <b className="conversation-unread">{unreadCount}</b> : <button aria-label={`فتح ${item.trackingCode}`}>←</button>}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <EmptyRows label="ستظهر هنا أول محادثة يبدأها زائر الموقع." />
        )}
      </section>
    </>
  );
...
function ConversationDrawer({ conversation, messages, businessHours, canWrite, busy, onClose, onReply, onStatus }: { conversation: VisitorConversation; messages: VisitorMessage[]; businessHours: BusinessHours; canWrite: boolean; busy: string | null; onClose: () => void; onReply: (conversationId: string, form: HTMLFormElement) => Promise<void>; onStatus: (conversationId: string, status: "waiting" | "open" | "closed") => Promise<void> }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    void onReply(conversation.id, event.currentTarget);
  }
  return (
    <div className="drawer-layer">
      <button className="drawer-backdrop" aria-label="إغلاق المحادثة" onClick={onClose} />
      <aside className="request-drawer portal-chat-drawer" role="dialog" aria-modal="true" aria-label={`محادثة ${conversation.trackingCode}`}>
        <div className="drawer-head">
          <div>
            <span>{conversation.trackingCode}</span>
            <h2>{conversation.visitorName}</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        ...
        <div className="portal-chat-contact">
          <div>
            ...
            <p>
              ...
              <small dir="ltr">
                {conversation.visitorMobile} · {conversation.visitorEmail || "دون بريد"}
              </small>
            </p>
          </div>
        ...
        <div className={`portal-chat-hours ${businessHours.isOpen ? "open" : "closed"}`}>
          <strong>{businessHours.isOpen ? "ضمن ساعات الدوام" : "خارج ساعات الدوام"}</strong>
          <span>{businessHours.isOpen ? "يمكن الرد المباشر الآن." : "استلم الزائر الرد الآلي عند أول رسالة خارج الدوام."}</span>
        </div>
        <div className="portal-chat-messages" ref={messageBox}>
          {messages.length ? (
            messages.map((message) => (
              <article key={message.id} className={message.senderType}>
                <header>
                  <strong>{message.senderType === "visitor" ? conversation.visitorName : message.senderType === "system" ? "الرد الآلي" : message.senderName}</strong>
                  <time>{formatDate(message.createdAt, true)}</time>
                </header>
                <p>{message.body}</p>
                {message.senderType === "staff" && <small>{message.readByVisitorAt ? "قرأها الزائر" : "أُرسلت للزائر"}</small>}
              </article>
            ))
          ) : (
        ...
        {canWrite ? <form className="portal-chat-composer" onSubmit={submit}>
          ...
          <div>
            <small>سيُنسب الرد إلى حسابك ويُحفظ في سجل النشاط.</small>
            <button className="admin-primary" disabled={busy === `chat-reply-${conversation.id}`}>
...
function ChatSettingsModal({ businessHours, automation, busy, onClose, onSubmit }: { businessHours: BusinessHours; automation: ChatAutomationConfig; busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  return (
    <div className="modal-layer">
      <button className="drawer-backdrop" aria-label="إغلاق إعدادات الرد الآلي" onClick={onClose} />
      <section className="record-modal chat-settings-modal" role="dialog" aria-modal="true" aria-label="إعدادات نظام الرد الآلي">
        <div className="drawer-head">
          <div>
            <span>المحادثة المباشرة</span>
            <h2>نظام الرد الآلي وساعات الخدمة</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        ...
        <form onSubmit={submit}>
          <fieldset className="span-two automation-switches">
            <legend>حالة النظام</legend>
            <label>
              <input type="checkbox" name="automationEnabled" defaultChecked={automation.enabled} />
              <span>
                <strong>تشغيل المساعد الآلي</strong>
                <small>يعطّل هذا الخيار جميع الردود الآلية دفعة واحدة.</small>
              </span>
            </label>
            <label>
              <input type="checkbox" name="welcomeEnabled" defaultChecked={automation.welcomeEnabled} />
              <span>
                <strong>ترحيب خلال الدوام</strong>
                <small>إشعار فوري باستلام المحادثة.</small>
              </span>
            </label>
            <label>
              <input type="checkbox" name="afterHoursEnabled" defaultChecked={automation.afterHoursEnabled} />
              <span>
                <strong>رد خارج الدوام</strong>
                <small>يتضمن موعد العودة القادم تلقائياً.</small>
              </span>
            </label>
            <label>
              <input type="checkbox" name="intentRepliesEnabled" defaultChecked={automation.intentRepliesEnabled} />
              <span>
                <strong>توجيه بحسب نية الزائر</strong>
                <small>يطابق الكلمات ويرسل الإرشاد المناسب مرة واحدة.</small>
              </span>
            </label>
          </fieldset>
          <label className="span-two">
            رسالة الترحيب خلال الدوام
            <textarea name="welcomeReply" required minLength={10} maxLength={900} rows={4} defaultValue={automation.welcomeReply} />
          </label>
          <fieldset className="span-two">
            <legend>أيام العمل الرسمية</legend>
            <div className="working-days">
              {workingDayLabels.map((label, day) => (
                <label key={label}>
                  <input type="checkbox" name="workingDays" value={day} defaultChecked={businessHours.workingDays.includes(day)} />
                  <span>{label}</span>
                </label>
              ))}
            </div>
          ...
          <label>
            بداية الدوام
            <input name="opensAt" type="time" required defaultValue={businessHours.opensAt} />
          </label>
          <label>
            نهاية الدوام
            <input name="closesAt" type="time" required defaultValue={businessHours.closesAt} />
          </label>
          <label>
            إغلاقات استثنائية
            <textarea name="closedDates" rows={3} placeholder="2026-09-23، 2027-02-22" defaultValue={businessHours.closedDates.join("، ")} />
          </label>
          <label>
            أيام فتح استثنائية
            <textarea name="specialOpenDates" rows={3} placeholder="2026-08-15" defaultValue={businessHours.specialOpenDates.join("، ")} />
          </label>
          <label className="span-two">
            رسالة خارج الدوام
            <textarea name="autoReply" required minLength={10} maxLength={700} rows={5} defaultValue={businessHours.autoReply} />
          </label>
          <p className="form-hint span-two automation-placeholders">
            يمكن استخدام المتغيرات <code>{"{{name}}"}</code> و<code>{"{{trackingCode}}"}</code> و<code>{"{{nextOpen}}"}</code> و<code>{"{{subject}}"}</code>. يعتمد احتساب الموعد على توقيت مكة والإغلاقات الاستثنائية.
          </p>
          <section className="span-two automation-rules">
            <header>
              <div>
                <strong>مسارات التوجيه الذكي</strong>
                <span>يرسل كل مسار مرة واحدة فقط في المحادثة، حتى عند تكرار الرسالة.</span>
              </div>
              <b>{automation.rules.filter((rule) => rule.enabled).length} مفعّل</b>
            </header>
            {automation.rules.map((rule) => (
              <article key={rule.id}>
                <label className="automation-rule-toggle">
                  <input type="checkbox" name={`ruleEnabled:${rule.id}`} defaultChecked={rule.enabled} />
                  <span>{rule.label}</span>
                </label>
                <label>
                  الكلمات الدالة
                  <input name={`ruleKeywords:${rule.id}`} required maxLength={500} defaultValue={rule.keywords.join("، ")} />
                </label>
                <label className="wide">
                  الرد المخصص
                  <textarea name={`ruleResponse:${rule.id}`} required minLength={10} maxLength={900} rows={3} defaultValue={rule.response} />
                </label>
              </article>
            ))}
          </section>
          <div className="modal-actions span-two">
            ...
            <button className="admin-primary" disabled={busy}>
              {busy ? "جارٍ الحفظ..." : "حفظ النظام وتشغيله"}
            </button>
          </div>
        </form>
      </section>
...
function FinanceDocumentActions({ onIssue }: { onIssue: (type: string) => void }) {
  return (
    <section className="panel finance-document-actions">
      <div>
        <span>
          <Icon name="documents" />
        </span>
        <p>
          <strong>الإصدارات المالية الرسمية</strong>
          <small>تُحفظ تلقائياً في مركز المستندات وتُربط بالسجل المالي.</small>
        </p>
      </div>
      <div>
        <button onClick={() => onIssue("invoice")}>إصدار فاتورة</button>
        <button onClick={() => onIssue("receipt")}>سند قبض</button>
        <button onClick={() => onIssue("payment_voucher")}>سند صرف</button>
        <button onClick={() => onIssue("progress_claim")}>مستخلص عمالة</button>
      </div>
    </section>
  );
}
  ...
  return (
    <>
      ...
      {(expiringDocuments.length > 0 || expiredDocuments.length > 0) && (
        <section className="expiry-alert" role="status">
          <span>
            <Icon name="bell" />
          </span>
          <div>
            <strong>توجد مستندات تحتاج إلى متابعة</strong>
            <p>
              {expiredDocuments.length > 0 ? `${expiredDocuments.length} مستند منتهٍ، ` : ""}
              {expiringDocuments.length} مستند تنتهي صلاحيته خلال أقل من 30 يوماً.
            </p>
          </div>
          <div className="alert-docs">
            {[...expiredDocuments, ...expiringDocuments].slice(0, 3).map((item) => (
              <small key={item.id}>
                {item.title} · {expiryText(item.expiryDate)}
              </small>
            ))}
          </div>
        </section>
          ...
          {documents.map((item) => {
            ...
            return (
              <tr key={item.id}>
                ...
                <td>
                  <span className={`expiry-pill ${expiryClass}`}>{expiryText(item.expiryDate)}</span>
                </td>
                  ...
                  <div className="document-actions">
                    ...
                    {canShare && (
                      <button disabled={busy === `share-${item.id}`} onClick={() => void onShare(item.id)} aria-label={`مشاركة ${item.title}`}>
                        <Icon name="share" />
                        <span>{busy === `share-${item.id}` ? "جارٍ الإنشاء" : "مشاركة"}</span>
                      </button>
                    )}
                  </div>
                </td>
...
function PdfDownloadButton({ href, title, label, icon, inline = false }: { href: string; title: string; label: string; icon: IconName; inline?: boolean }) {
  ...
  async function openPdf() {
    ...
    try {
      ...
      if (!response.ok) {
        const data = (await readApiJson(response).catch(() => ({}))) as {
          error?: string;
        };
      ...
      setError(problem instanceof Error ? problem.message : "تعذّر تجهيز الملف");
    } finally {
      setLoading(false);
    }
  }
  return (
    <>
      <button type="button" disabled={loading} onClick={() => void openPdf()} aria-label={`${label} ${title}`}>
        <Icon name={icon} />
        <span>{loading ? "جارٍ التجهيز" : label}</span>
      </button>
      {error && (
        <div className="pdf-error-layer">
          <section className="pdf-error-dialog" role="alertdialog" aria-modal="true" aria-label="تعذّر تنزيل الملف">
            <span className="pdf-error-icon">!</span>
            <h3>الملف غير متاح للتنزيل</h3>
            <p>{error}</p>
            <button type="button" autoFocus onClick={() => setError("")}>
              حسنًا
            </button>
          </section>
        </div>
...
function CompanyAssetsPanel({ assets, canManage, busy, onUpload }: { assets: CompanyAsset[]; canManage: boolean; busy: string | null; onUpload: (slot: "stamp" | "signature", form: HTMLFormElement) => Promise<void> }) {
  return (
    <aside className="panel company-assets">
      <div className="panel-head">
        <div>
          <h2>الختم والتوقيع</h2>
          <p>أصول محمية تُدرج آلياً في كل PDF صادر</p>
        </div>
        <Icon name="stamp" />
      </div>
      <div className="asset-list">
        {(["stamp", "signature"] as const).map((slot) => {
          const asset = assets.find((item) => item.slot === slot);
          const label = slot === "stamp" ? "ختم الشركة" : "التوقيع المفوض";
          return (
            <form
              key={slot}
              onSubmit={(event) => {
                event.preventDefault();
                void onUpload(slot, event.currentTarget);
              }}
            >
              <div className={`asset-status ${asset ? "ready" : "missing"}`}>
                <span>{asset ? "✓" : "!"}</span>
                <p>
                  <strong>{label}</strong>
                  <small>{asset ? `${asset.fileName} · ${formatDate(asset.updatedAt, true)}` : "لم يُرفع بعد"}</small>
                </p>
                {asset && (
                  <a className="asset-preview" href={`/api/portal/company-assets?slot=${slot}`} target="_blank" rel="noreferrer">
                    معاينة
                  </a>
                )}
              </div>
              {canManage && (
                <>
                  <label className="asset-file">
                    <input type="file" name="file" accept=".png,.jpg,.jpeg,image/png,image/jpeg" required />
                    <span>
                      <Icon name="upload" />
                      {asset ? "استبدال الملف" : "رفع الملف"}
                    </span>
                  </label>
                  <button className="asset-submit" type="submit" disabled={busy === `asset-${slot}`}>
                    {busy === `asset-${slot}` ? "جارٍ الحفظ..." : "اعتماد"}
                  </button>
                </>
              )}
            </form>
...
function UploadDocumentModal({ busy, onClose, onSubmit }: { busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  return (
    <div className="modal-layer">
      <button className="drawer-backdrop" aria-label="إغلاق نافذة رفع المستند" onClick={onClose} />
      <section className="record-modal document-modal" role="dialog" aria-modal="true" aria-label="رفع مستند شركة">
        <div className="drawer-head">
          <div>
            <span>مركز المستندات</span>
            <h2>رفع مستند شركة</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        ...
        <form onSubmit={submit}>
          ...
          <label>
            حجز نظامي حتى
            <input name="lockedUntil" type="date" />
          </label>
          <label>
            الملف
          ...
          <div className="modal-actions span-two">
            ...
            <button className="admin-primary" type="submit" disabled={busy}>
              {busy ? "جارٍ الرفع..." : "رفع وحفظ المستند"}
            </button>
          </div>
        </form>
      </section>
...
function FinanceRecordModal({ busy, workers, contracts, assignments, onClose, onSubmit }: { busy: boolean; workers: WorkerRecord[]; contracts: WorkforceContract[]; assignments: ContractAssignment[]; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
    ...
    fetch("/api/portal/accounting", { cache: "no-store" })
      ...
      .then((payload: unknown) => {
        if (active) {
          ...
          setBanks((data.banks || []).filter((bank) => bank.status === "active"));
        }
      })
      .catch(() => {
        if (active) setBanks([]);
  ...
  return (
    <div className="modal-layer">
      <button className="drawer-backdrop" aria-label="إغلاق نافذة الحركة المالية" onClick={onClose} />
      <section className="record-modal finance-modal" role="dialog" aria-modal="true" aria-label="إضافة حركة مالية للعمالة">
        <div className="drawer-head">
          <div>
            <span>الإدارة المالية</span>
            <h2>إضافة حركة مالية للعمالة</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        ...
        <form onSubmit={submit}>
          ...
          {category === "worker_expense" && (
            <label>
              نوع المصروف
              <select name="subCategory" required defaultValue="">
                <option value="" disabled>
                  اختر نوع المصروف
                </option>
                {Object.entries(workforceExpenseLabels).map(([value, label]) => (
          ...
          <label>
            ...
            <input name="dueDate" required type="date" defaultValue={new Date().toISOString().slice(0, 10)} />
          </label>
          <label>
            طريقة الدفع
            <select name="paymentMethod" value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)}>
              ...
              <option value="payroll_file">ملف حماية الأجور</option>
              <option value="other">أخرى</option>
            </select>
          </label>
          {["bank_transfer", "cheque", "payroll_file"].includes(paymentMethod) && (
            <label className="span-two">
              الحساب البنكي
              <select name="bankAccountId" required defaultValue="">
                <option value="" disabled>
                  {banks.length ? "اختر الحساب البنكي" : "لا توجد حسابات بنكية نشطة"}
                </option>
                {banks.map((bank) => (
                  <option value={bank.id} key={bank.id}>
                    {bank.bankName} — {bank.accountName} — {bank.accountCode}
                  </option>
                ))}
              </select>
            </label>
          ...
          <label className="span-two">
            البيان
            <input name="description" required minLength={3} maxLength={240} placeholder="مثال: راتب شهر أغسطس للعامل أو مصروف تجديد إقامة" />
          </label>
          <label className="span-two">
            ملاحظات
            <textarea name="notes" maxLength={1000} rows={4} placeholder="تفاصيل إضافية أو رقم مرجع خارجي..." />
          </label>
          <p className="form-hint span-two">يظهر السجل في كشف العامل والعقد، ويمكن إصدار الفواتير والسندات الرسمية من لوحة المالية.</p>
          <div className="modal-actions span-two">
            <button type="button" onClick={onClose}>
              إلغاء
            </button>
            <button className="admin-primary" type="submit" disabled={busy}>
              {busy ? "جارٍ الحفظ..." : "حفظ الحركة المالية"}
            </button>
          </div>
        </form>
      </section>
...
function IssueDocumentModal({ initialType, initialQuoteId, conversionMode, canIssueContracts, canIssueFinance, busy, assetsReady, workers, contracts, requests, onClose, onSubmit }: { initialType: string; initialQuoteId: number | null; conversionMode: "as_is"|"modified"; canIssueContracts: boolean; canIssueFinance: boolean; busy: boolean; assetsReady: boolean; workers: WorkerRecord[]; contracts: WorkforceContract[]; requests: WorkforceRequest[]; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  const [representatives, setRepresentatives] = useState<
    Array<{
      ...
      fullName: string;
      status: string;
      representativeType: "sales" | "purchasing";
    }>
  >([]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/portal/sales-representatives", { cache: "no-store" })
      ...
      .then((data: unknown) => {
        ...
        setRepresentatives((parsed.representatives || []).filter((item) => item.status === "active"));
        setRepresentativeRequests((parsed.requests || []).filter((item) => item.status === "approved"));
      })
      .catch(() => {
        setRepresentatives([]);
  ...
  useEffect(() => {
    ...
    void fetch(`/api/portal/operations?limit=100${initialQuoteId ? `&quoteId=${initialQuoteId}` : ""}`, { cache: "no-store" })
      ...
      .then((raw: unknown) => {
        const data = raw as {
          quoteSources?: Array<{ quoteId: number; sourceRequestId: number | null; clientName: string | null; title: string | null }>;
          contractConversions?: Array<{ quoteId: number }>;
          quotes?: Array<{
            id: number;
            quoteCode: string;
            commercialTermsJson: string | null;
        ...
        const availableQuotes = (data.quotes || []).filter((quote) => quote.approvedBy && ["approved", "sent", "accepted"].includes(quote.status) && !data.contractConversions?.some(item => item.quoteId === quote.id));
        if (initialQuoteId && !availableQuotes.some(quote => quote.id === initialQuoteId)) throw new Error("عرض السعر غير معتمد أو سبق تحويله إلى عقد");
        setConvertibleQuotes(
          availableQuotes.map((quote) => {
              const source = data.quoteSources?.find(item => item.quoteId === quote.id);
              ...
              return {
                ...
                sourceRequestId: source?.sourceRequestId || opportunity?.sourceRequestId || null,
                quoteCode: quote.quoteCode,
                quantityMode: quote.quantityMode || "fixed",
                seasonType: quote.seasonType || "regular",
                paymentScheduleJson: quote.paymentScheduleJson || null,
                vatRateBps: quote.vatRateBps || 0,
                subtotalHalalas: quote.subtotalHalalas || 0,
                accommodationParty: quote.accommodationParty || "dali",
                transportParty: quote.transportParty || "dali",
                clientName: source?.clientName || client?.legalName || "",
                title: source?.title || opportunity?.title || quote.quoteCode,
                items: (data.quoteItems || [])
                  .filter((item) => item.quoteVersionId === quote.id)
                  .map((item) => ({
  ...
  type DraftProfession = {
    ...
    unitSalary?: number;
    actualSalary?: number;
    sponsorshipType: "dali" | "other";
    sponsorName: string;
    ajirContractStatus: "not_applicable" | "with_ajir" | "without_ajir";
  };
  type DraftPayment = {
    key: string;
    title: string;
  ...
  useEffect(() => {
    ...
    setContractAmount(String(quote.subtotalHalalas / 100));
    setContractVatEnabled(quote.vatRateBps > 0);
    setContractVatRate(String(quote.vatRateBps / 100));
    setAccommodationParty(quote.accommodationParty);
    setTransportParty(quote.transportParty);
    if (quote.items.length)
      setProfessions(
        quote.items.map((item, index) => ({
          key: `quote-profession-${index}`,
          profession: item.profession,
  ...
  const capacity = professions.map((item) => {
    ...
    const available = workers.filter((worker) => matchesAllocation(worker) && worker.status === "available").length;
    return {
      ...item,
      registered,
      available,
      selected: selectedWorkers[item.key]?.length || 0,
      registeredShortage: Math.max(0, item.requiredCount - registered),
      availableShortage: Math.max(0, item.requiredCount - available),
    };
  });
  ...
  function setProfessionCount(key: string, value: number) {
    ...
    setSelectedWorkers((items) => ({
      ...items,
      [key]: (items[key] || []).slice(0, requiredCount),
    }));
  }
  function setProfessionSalary(key: string, value: number) {
    setProfessions((items) => items.map((item) => (item.key === key ? { ...item, unitSalary: Math.max(0, value || 0) } : item)));
  }
  function setProfessionActualSalary(key: string, value: number) {
    setProfessions((items) => items.map((item) => (item.key === key ? { ...item, actualSalary: Math.max(0, value || 0) } : item)));
  }
  function setProfessionSponsorship(key: string, value: "dali" | "other") {
  ...
  function setProfessionAjir(key: string, value: "not_applicable" | "with_ajir" | "without_ajir") {
    setProfessions((items) => items.map((item) => (item.key === key ? { ...item, ajirContractStatus: value } : item)));
  }
  function setCustomProfession(key: string, value: string) {
    setProfessions((items) => items.map((item) => (item.key === key ? { ...item, customProfession: value } : item)));
  }
  function addProfession() {
    ...
    setProfessions((items) => [
      ...items,
      {
        key: `profession-${Date.now()}`,
        profession: next.label,
        customProfession: "",
        requiredCount: 1,
        unitSalary: 0,
        sponsorshipType: "dali",
  ...
  function addSponsorAllocation(source: DraftProfession) {
    setProfessions((items) => [
      ...items,
      {
        ...source,
        key: `allocation-${Date.now()}`,
        requiredCount: 1,
        sponsorshipType: "other",
        sponsorName: "",
        ajirContractStatus: "without_ajir",
      },
    ]);
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isContract) {
      ...
      const allocationKeys = professions.map((item) => [item.profession, item.sponsorshipType, item.sponsorName.trim(), item.ajirContractStatus].join("::"));
      if (new Set(allocationKeys).size !== allocationKeys.length) {
        showContractValidationError(2, "يوجد توزيع مكرر للمهنة والكفيل وحالة أجير؛ اجمع العدد في توزيع واحد.");
        return;
      }
      if (seasonType !== "regular" && quantityMode === "fixed" && (payments.length === 0 || Math.abs(payments.reduce((sum, item) => sum + item.percentage, 0) - 100) >= 0.01 || payments.some((item) => !item.title.trim() || !item.dueDate || item.percentage <= 0))) {
    ...
    const invalid = Array.from(form.elements).find((element) => (element instanceof HTMLInputElement || element instanceof HTMLSelectElement || element instanceof HTMLTextAreaElement) && element.type !== "hidden" && !element.checkValidity()) as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | undefined;
    if (invalid) {
  ...
  function validateAndSetStep(target: 1 | 2 | 3 | 4) {
    ...
    if (step === 1 && seasonType === "regular" && !annualEndDate) {
      ...
      if (start instanceof HTMLInputElement) {
        start.setCustomValidity("حدد تاريخ بداية العقد ليحسب النظام تاريخ النهاية السنوي.");
  ...
  const seasonalSubtotalHalalas = Math.max(0, Math.round((Number(contractAmount) || 0) * 100));
  const seasonalTotalHalalas = seasonalSubtotalHalalas + Math.round((seasonalSubtotalHalalas * annualVatRateBps) / 10000);
  const annualPercentages = annualInstallmentPercentages(ANNUAL_CONTRACT_MONTHS);
  const annualInstallments = Array.from({ length: ANNUAL_CONTRACT_MONTHS }, (_, index) => ({
    number: index + 1,
    dueDate: annualSchedule?.dueDates[index] || "",
    percentageBps: annualPercentages[index],
    subtotalHalalas: annualMonthlySubtotalHalalas,
    vatHalalas: annualMonthlyVatHalalas,
    amountHalalas: annualMonthlySubtotalHalalas + annualMonthlyVatHalalas,
  }));
  const serializedProfessions = JSON.stringify(
    professions.map((item) => ({
      ...
      ajirContractStatus: item.ajirContractStatus,
      workerIds: quantityMode === "open" ? [] : selectedWorkers[item.key] || [],
    })),
  );
  ...
  return (
    <div className="modal-layer">
      ...
      <section className="record-modal document-modal issue-modal" role="dialog" aria-modal="true" aria-label="إنشاء ملف PDF رسمي">
        ...
        {!assetsReady && (
          <div className="asset-required">
            <Icon name="stamp" />
            <p>
              <strong>الختم والتوقيع غير مكتملين</strong>
              <span>يجب أن يرفع مدير النظام الأصلين المعتمدين قبل الإصدار.</span>
            </p>
          </div>
        )}
        <form className={`contract-quantity-${quantityMode}`} onSubmit={submit} noValidate>
          ...
          <div className="document-type-control span-two">
            <label>
              نوع المستند
              <select
                name="documentType"
          ...
          {isContract && (
            <div className="contract-wizard-steps span-two">
              <button type="button" className={step === 1 ? "active" : "done"} onClick={() => validateAndSetStep(1)}>
                1 بيانات العقد
              </button>
              <button type="button" className={step === 2 ? "active" : step > 2 ? "done" : ""} onClick={() => validateAndSetStep(2)}>
                2 المهن والأعداد
              </button>
              <button type="button" className={step === 3 ? "active" : step > 3 ? "done" : ""} onClick={() => validateAndSetStep(3)}>
                3 اختيار العمالة
              </button>
              <button type="button" className={step === 4 ? "active" : ""} onClick={() => validateAndSetStep(4)}>
                4 الدفعات والمرفقات
              </button>
            </div>
          )}
          ...
          {isContract && (
            <>
              <input type="hidden" name="contractDirection" value={contractDirection} />
              <input type="hidden" name="contractClauses" value={serializedClauses} />
            </>
          )}
          ...
          {isContract && step === 1 && (
            <div className="contract-billing-controls span-two">
              <label>
                نوع التعاقد
                <select value={seasonType} onChange={(event) => setSeasonType(event.target.value as "regular" | "ramadan" | "hajj")}>
              ...
              {seasonType === "regular" && (
                <div className="billing-mode-summary success">
                  <b>موعد أول دفعة تلقائي</b>
                  <span>تستحق بعد شهر كامل من بداية العقد، ثم تتكرر الدفعات شهريًا.</span>
                </div>
              )}
              <p className="form-hint span-two">في العقود السنوية يحسب النظام تاريخ النهاية و12 دفعة شهرية من تاريخ البداية. في الحج ورمضان تُستخدم نسب جدول الدفعات ويجب أن يكون مجموعها 100٪.</p>
            </div>
          )}
          <div className={`issue-form-step span-two ${!isContract || step === 1 ? "visible" : ""}`}>
            {isContract && (
              <>
                <label className="span-two">
                  ...
                  <small>يغير صفات الأطراف والبنود ومسار الدفعات والمحاسبة تلقائياً.</small>
                </label>
                    ...
                    <select value={selectedQuoteId} onChange={applyQuote}>
                      ...
                      {convertibleQuotes.map((quote) => (
                        <option key={quote.id} value={quote.id}>
                          {quote.quoteCode} — {quote.title} — {quote.quantityMode === "open" ? "عدد مفتوح" : "عدد محدد"}
                        </option>
                      ))}
                    </select>
                  </label>
                  ...
                  <select name="sourceRequestId" value={selectedSourceRequestId} onChange={(event) => setSelectedSourceRequestId(event.target.value)} disabled={contractDirection === "dali_purchaser"}>
                    ...
                    {requests.map((request) => (
                      <option key={request.id} value={request.id}>
                        {request.trackingCode} — {request.companyName || request.fullName}
                      </option>
                    ))}
                  </select>
                  <small>تُعبأ بيانات الطلب تلقائياً ويمكن تعديلها.</small>
                </label>
                <label>
                    ...
                    {representatives
                      ...
                      .map((item) => (
                        <option key={item.id} value={item.id}>
                          {item.representativeCode} — {item.fullName}
                        </option>
                      ))}
                  </select>
                </label>
            ...
            <label>
              تاريخ الإصدار
              <input name="issueDate" required type="date" defaultValue={new Date().toISOString().slice(0, 10)} />
            </label>
            <label>
            ...
            <label>
              {isContract && contractDirection === "dali_purchaser" ? "الرقم الضريبي للمورّد" : "الرقم الضريبي للعميل"}
              <input name="clientVat" required={isContract} maxLength={30} dir="ltr" placeholder={isContract ? "إلزامي للعقد" : "مطلوب عند تفعيل الضريبة"} />
            </label>
            {isContract && (
              <label className="span-two">
                {contractDirection === "dali_purchaser" ? "العنوان الوطني للمورّد" : "العنوان الوطني للعميل"}
                <input name="clientAddress" required maxLength={240} placeholder="العنوان الوطني المسجل للجهة المتعاقدة" />
              </label>
            )}
            {!isContract && (
              <>
                ...
                <label>
                  ...
                  <select name="vatEnabled" defaultValue="true">
                ...
                <label>
                  نسبة الضريبة %
                  <input name="vatRate" type="number" min="0" max="100" step="0.01" defaultValue="15" dir="ltr" />
                </label>
              </>
            )}
            {isContract ? (
              <>
                <label>
                  موقع العمل
                  <input name="workSite" required maxLength={180} />
                </label>
                <label>
                  بداية العقد
                  <input name="startDate" required type="date" value={contractStartDate} onChange={(event) => setContractStartDate(event.target.value)} />
                </label>
                {seasonType === "regular" ? (
                  <div className="annual-contract-period">
                    <span>مدة العقد السنوي</span>
                    <strong>{annualEndDate ? formatDate(annualEndDate) : "تُحسب بعد تحديد تاريخ البداية"}</strong>
                    <small>يحسب النظام تاريخ النهاية تلقائيًا بعد 12 شهرًا ويظهره في العقد وملف PDF.</small>
                  </div>
                ) : (
                  <label>
                    نهاية العقد
              ...
              <>
                <label>
                  {documentType === "quotation" ? "صلاحية العرض حتى" : "تاريخ الاستحقاق / الانتهاء"}
                  <input name="expiryDate" type="date" required={documentType === "quotation"} />
                </label>
                {["invoice", "receipt", "payment_voucher", "progress_claim"].includes(documentType) && (
                  <label className="span-two">
                    العقد المرتبط
                    <select name="linkedContractId" defaultValue="">
          ...
          {isContract && (
            <div className={`issue-form-step span-two ${step === 2 ? "visible" : ""}`}>
              <div className="profession-builder-head">
                <div>
                  <strong>المهن وتوزيعات الكفالة</strong>
                  <small>أضف المهنة ثم وزّع عددها على دالي أو أكثر من كفيل. مجموع التوزيعات هو إجمالي المهنة.</small>
                </div>
                <button type="button" onClick={addProfession} disabled={new Set(professions.map((item) => item.profession)).size >= workforceProfessions.length}>
                  <Icon name="plus" /> إضافة مهنة
                </button>
              </div>
              <div className="profession-builder">
                {capacity.map((item) => (
                  <article key={item.key}>
                    <label>
                      المهنة
                      <SearchableCombobox name={`profession_${item.key}`} value={item.profession} options={workforceProfessions.map((option) => option.label).filter((label) => label === item.profession || !professions.some((other) => other.key !== item.key && other.profession === label))} onChange={(value) => setProfessionValue(item.key, value)} placeholder="ابحث عن المهنة" required />
                    </label>
                    <label>
                      عدد هذا التوزيع
                      <input type="number" min="1" max="100000" value={item.requiredCount} onChange={(event) => setProfessionCount(item.key, Number(event.target.value))} />
                    </label>
                    <div>
                      <span>
                        <b>{item.available}</b> متاح مطابق
                      </span>
                      <span>
                        <b>{professions.filter((row) => row.profession === item.profession).reduce((sum, row) => sum + row.requiredCount, 0)}</b> إجمالي المهنة
                      </span>
                    </div>
                    ...
                    {professions.length > 1 && (
                      <button className="remove-profession" type="button" onClick={() => removeProfession(item.key)}>
                        حذف التوزيع
                      </button>
                    )}
                  </article>
              ...
              <p className="form-hint">عند اختيار كفالة جهة أخرى تظهر العمالة المطابقة، وتُؤخذ أسماء كفلائهم تلقائياً من ملفاتهم.</p>
            </div>
          )}

          {isContract && step === 2 && (
            <div className="contract-profession-pricing span-two">
              <div className="profession-builder-head">
                <div>
                  <strong>الراتب والمهنة المخصصة</strong>
                  <small>راتب العامل الشهري إلزامي لكل مهنة حتى في العقد مفتوح العدد، ويُستخدم عند احتساب الفواتير الفعلية.</small>
                </div>
              </div>
              <div className="profession-builder">
                {professions.map((item) => (
                  <article key={`pricing-${item.key}`}>
                    {item.profession === "أخرى" && (
                      <label>
                        اكتب المهنة يدوياً
                        <input required minLength={2} maxLength={120} value={item.customProfession || ""} onChange={(event) => setCustomProfession(item.key, event.target.value)} placeholder="مثال: فني مضخات" />
                      </label>
                    )}
                    <label>
                    ...
                    <label>
                      ...
                      <small>يمكن تحديد حالة أجير مستقلة عن جهة الكفالة، بما في ذلك عمالة دالي.</small>
                    </label>
                    <p>
                      {item.profession === "أخرى" ? item.customProfession || "مهنة أخرى" : item.profession} · {quantityMode === "open" ? "عدد مفتوح" : `${item.requiredCount} عامل`} · {formatMoney(Math.round((item.unitSalary || 0) * 100))} للعامل/شهر
                    </p>
                  </article>
                ))}
              </div>
          ...
          {isContract && (
            <div className={`issue-form-step span-two ${step === 3 ? "visible" : ""}`}>
              <div className="selection-intro">
                <strong>اختيار العمالة المتاحة</strong>
                <p>اختيار الأسماء اختياري. يمكنك تخطي هذه الخطوة وإنشاء العقد ثم إضافة العمالة لاحقاً.</p>
              </div>
                    ...
                    <section key={item.key}>
                      <header>
                        <div>
                          ...
                          <small>
                            مطلوب {item.requiredCount} · مختار {selected.length}
                          </small>
                        </div>
                        <span className={selected.length === item.requiredCount ? "complete" : ""}>{item.requiredCount - selected.length} متبقٍ</span>
                      </header>
                      <div>
                        {candidates.length ? (
                          candidates.map((worker) => (
                            <label key={worker.id} className={selected.includes(worker.id) ? "selected" : ""}>
                              <input type="checkbox" checked={selected.includes(worker.id)} disabled={!selected.includes(worker.id) && selected.length >= item.requiredCount} onChange={() => toggleWorker(item.key, worker.id, item.requiredCount)} />
                              <span>{initials(worker.fullName)}</span>
                              <p>
                                <strong>{worker.fullName}</strong>
                                <small>
                                  {worker.workerNumber} · إقامة {worker.iqamaNumber || "غير مسجلة"} · {sponsorshipLabel(worker)}
                                </small>
                              </p>
                            </label>
                          ))
                        ) : (
                          <p className="empty-operational">لا توجد عمالة متاحة بهذه المهنة حالياً.</p>
                        )}
                      </div>
                    </section>
                  );
              ...
              {totalShortage > 0 && (
                <div className="contract-shortage-summary">
                  <Icon name="bell" />
                  <p>
                    <strong>يمكن إصدار العقد رغم العجز</strong>
                    <span>إجمالي العجز التشغيلي الحالي {totalShortage} عامل عبر المهن المطلوبة.</span>
                  </p>
                </div>
              )}
            </div>
          ...
          {isContract && (
            <div className={`issue-form-step contract-final-step span-two ${step === 4 ? "visible" : ""}`}>
              <section className="contract-final-card payment-plan-card">
                <header className="contract-final-heading">
                  ...
                  <div>
                    ...
                    <p>{quantityMode === "open" ? "تُحسب القيمة والضريبة عند إصدار كل فاتورة حسب العمالة الفعلية." : seasonType === "regular" ? "ينشئ النظام الدفعات الشهرية الاثنتي عشرة تلقائياً من رواتب المهن، وتُثبت مواعيدها عند اعتماد العقد." : "وزّع قيمة العقد على دفعات موسمية؛ يجب أن يكون مجموع النسب 100% قبل الإصدار."}</p>
                  </div>
                </header>
                <div className="contract-tax-fields">
                  <label>
                    إظهار جدول الدفعات في PDF
                    <select value={showPaymentSchedule ? "true" : "false"} onChange={(event) => setShowPaymentSchedule(event.target.value === "true")}>
                      <option value="true">نعم، إظهار الجدول</option>
                      <option value="false">لا، إخفاء الجدول</option>
                    </select>
                    <small>تبقى الدفعات محفوظة في النظام في الحالتين.</small>
                  </label>
                  <label>
                    السكن
                  ...
                  {quantityMode === "fixed" && seasonType !== "regular" && (
                    <label>
                      ...
                      <input name="amount" required type="number" min="0.01" max="1000000000" step="0.01" dir="ltr" value={contractAmount} onChange={(event) => setContractAmount(event.target.value)} readOnly={Boolean(selectedQuoteId)} />
                      <small>{selectedQuoteId ? "القيمة مطابقة لعرض السعر المرتبط" : "تُوزّع هذه القيمة على الدفعات أدناه"}</small>
                    </label>
                  )}
                  ...
                  {contractVatEnabled && (
                    <label>
                      نسبة الضريبة %
                      <input name="vatRate" required type="number" min="0.01" max="100" step="0.01" dir="ltr" value={contractVatRate} onChange={(event) => setContractVatRate(event.target.value)} readOnly={Boolean(selectedQuoteId)} />
                    </label>
                  )}
                  {!contractVatEnabled && <input type="hidden" name="vatRate" value="0" />}
                  {quantityMode === "fixed" && seasonType === "regular" && (
                    <div className="calculated-contract-value">
                      <strong>قيمة محسوبة تلقائياً</strong>
                      <span>عدد العمال × راتب العامل × 12 شهرًا، وتبدأ الدفعة الأولى بعد شهر من بداية العقد.</span>
                    </div>
                  )}
                </div>
                {quantityMode === "open" ? (
                  <div className="billing-mode-summary neutral">
                    <b>عقد بعدد مفتوح</b>
                    <span>لا تُطلب قيمة إجمالية أو نسب دفعات عند الإنشاء.</span>
                  </div>
                ) : seasonType === "regular" ? (
                  <div className="annual-payment-plan">
                    <div className="annual-payment-summary">
                      <div>
                        <span>قيمة الدفعة الشهرية</span>
                        <strong>{formatMoney(annualMonthlySubtotalHalalas + annualMonthlyVatHalalas)}</strong>
                        <small>{contractVatEnabled ? `تشمل ضريبة ${Number(contractVatRate) || 0}%` : "بدون ضريبة"}</small>
                      </div>
                      <div>
                        <span>إجمالي السنة</span>
                        <strong>{formatMoney((annualMonthlySubtotalHalalas + annualMonthlyVatHalalas) * ANNUAL_CONTRACT_MONTHS)}</strong>
                        <small>{ANNUAL_CONTRACT_MONTHS} دفعة شهرية متساوية</small>
                      </div>
                    </div>
                    <div className="annual-payment-installments">
                      {annualInstallments.map((installment) => (
                        <article key={installment.number}>
                          <span>{installment.number}</span>
                          <div>
                            <strong>الدفعة الشهرية {installment.number}</strong>
                            <small>{installment.dueDate ? `تستحق في ${installment.dueDate}` : `بعد ${installment.number} شهر من بداية العقد`}</small>
                          </div>
                          <b>{formatMoney(installment.amountHalalas)}</b>
                        </article>
                      ))}
                    </div>
                    <p>يحسب النظام تاريخ كل دفعة من بداية العقد، ثم يعكس الدفعات كاملة في المالية وملف PDF.</p>
                  </div>
                ) : (
                  <>
                    <div className="payment-total-bar">
                      <div>
                        ...
                        <strong>{payments.reduce((sum, item) => sum + item.percentage, 0).toFixed(2)}%</strong>
                      </div>
                      <meter
                        min="0"
                        max="100"
                        value={Math.min(
                          100,
                          payments.reduce((sum, item) => sum + item.percentage, 0),
                        )}
                      />
                      <span className={Math.abs(payments.reduce((sum, item) => sum + item.percentage, 0) - 100) < 0.01 ? "complete" : "incomplete"}>{Math.abs(payments.reduce((sum, item) => sum + item.percentage, 0) - 100) < 0.01 ? "مكتمل وجاهز" : "يجب إكمال المجموع إلى 100٪"}</span>
                    </div>
                    <div className="contract-payment-builder">
                      {payments.map((payment, index) => (
                        <article key={payment.key}>
                          ...
                          <b className="payment-calculated-amount">{formatMoney(Math.round((seasonalTotalHalalas * payment.percentage) / 100))}</b>
                          {payments.length > 1 && (
                            <button className="remove-payment" type="button" onClick={() => setPayments((items) => items.filter((item) => item.key !== payment.key))} aria-label={`حذف الدفعة ${index + 1}`}>
                              حذف
                            </button>
                          )}
                        </article>
                    ...
                    >
                      <Icon name="plus" /> إضافة دفعة جديدة
                    </button>
                  </>
                )}
              </section>
              ...
              <section className="contract-final-card attachments-card">
                <header className="contract-final-heading">
                  ...
                  <div>
                    <strong>مستندات العميل الإلزامية</strong>
                    <p>PDF أو JPG أو PNG. تُحفظ الملفات تلقائياً داخل ملف العميل وتبقى مرتبطة بالعقد.</p>
                  </div>
                  <b>3 ملفات مطلوبة</b>
                </header>
                <div className="client-document-files">
                  <label>
                    <span>
                      <Icon name="documents" />
                    </span>
...
function filterRecords<T>(records: T[], query: string) {
  const needle = query.trim().toLowerCase();
...
function EmployeeTable({ records, users, query, canWrite, canArchive, busy, onStatus, onUpdate, onDelete }: { records: EmployeeRecord[]; users: PortalUser[]; query: string; canWrite: boolean; canArchive: boolean; busy: string | null; onStatus: (id: number, status: string) => void; onUpdate: (id: number, data: Record<string, string>) => Promise<boolean>; onDelete: (id: number) => Promise<void> }) {
  ...
  if (!rows.length) return <EmptyRows label="أضف أول ملف موظف ليظهر هنا." />;
  return (
    <div className="employee-card-grid">
      {rows.map((item) => {
        const open = openId === item.id;
        return (
          <article className={`employee-profile-card ${open ? "open" : ""}`} key={item.id}>
            <header>
              <div className="employee-card-avatar" aria-hidden="true">
              ...
              <div>
                ...
                <small dir="ltr">{item.employeeNumber}</small>
              </div>
              <StatusControl entity="employees" id={item.id} value={item.status} canWrite={canWrite} busy={busy} onStatus={onStatus} />
            </header>
            ...
            >
              {open ? "إغلاق البطاقة" : "فتح واستعراض الملف"}
            </button>
            {open && (
              <div className="employee-card-details">
                <dl>
                  <div>
                    <dt>البريد الوظيفي</dt>
                ...
                <EmployeeProfileWorkspace employeeId={item.id} leaveBalanceDays={item.leaveBalanceDays} canWrite={canWrite} />
              </div>
            )}
          </article>
        );
...
function EmployeeComplianceEditor({ employee, employees, users, busy, onCancel, onSave }: { employee: EmployeeRecord; employees: EmployeeRecord[]; users: PortalUser[]; busy: boolean; onCancel: () => void; onSave: (data: Record<string, string>) => Promise<void> }) {
  ...
  return (
    ...
    >
      ...
      <label>
        رقم الجوال
        <input name="mobile" required type="tel" maxLength={20} defaultValue={employee.mobile} dir="ltr" />
      </label>
      <label>
        البريد الوظيفي
...
function FinanceTable({ records, workers, contracts, query, canWrite, busy, onStatus }: { records: FinanceRecord[]; workers: WorkerRecord[]; contracts: WorkforceContract[]; query: string; canWrite: boolean; busy: string | null; onStatus: (id: number, status: string) => void }) {
  const rows = filterRecords(records, query);
  if (!rows.length) return <EmptyRows label="أضف أول عملية مالية ليظهر السجل هنا." />;
  return (
    <div className="management-table-wrap">
      <table className="management-table finance-table">
        ...
        <tbody>
          {rows.map((item) => {
            ...
            return (
              <tr key={item.id}>
                <td dir="ltr">{item.referenceCode}</td>
                <td>
                  <strong>{financeLabels[item.category] ?? item.category}</strong>
                  <small>{item.subCategory ? workforceExpenseLabels[item.subCategory] || item.subCategory : ""}</small>
                </td>
                <td>
                  <strong>{worker?.fullName || contract?.clientName || "غير مرتبط"}</strong>
                  <small>{worker ? `${worker.profession} · ${worker.iqamaNumber}` : contract?.referenceCode || ""}</small>
                </td>
                <td>
                  <strong>{item.description}</strong>
                  <small>{contract ? contract.title : item.notes || ""}</small>
                </td>
                ...
                <td>
                  ...
                  <small>{item.paymentMethod ? paymentMethodLabels[item.paymentMethod] || item.paymentMethod : "غير محدد"}</small>
                </td>
...
function LegalTable({ records, query, canWrite, busy, onStatus }: { records: LegalRecord[]; query: string; canWrite: boolean; busy: string | null; onStatus: (id: number, status: string) => void }) {
  const [openId, setOpenId] = useState<number | null>(null);
  const rows = filterRecords(records, query);
  if (!rows.length) return <EmptyRows label="أضف أول عقد أو ملف قانوني ليظهر هنا." />;
  return (
    <div className="legal-record-card-grid">
      {rows.map((item) => {
        const summary = legalFileSummary(item.fileSnapshotJson),
          open = openId === item.id;
        return (
          <article className={`legal-record-card ${open ? "open" : ""}`} key={item.id}>
            <header>
              <div>
                <span>{legalLabels[item.category] ?? item.category}</span>
                <strong>{item.title}</strong>
                <small dir="ltr">{item.referenceCode}</small>
              </div>
              <StatusControl entity="legal" id={item.id} value={item.status} canWrite={canWrite} busy={busy} onStatus={onStatus} />
            </header>
            <div className="legal-record-glance">
              <span>
                <small>الطرف الآخر</small>
                <b>{item.counterparty}</b>
              </span>
              <span>
                <small>الانتهاء أو التجديد</small>
                <b className={daysUntil(item.expiryDate) <= 45 ? "date-alert" : ""}>{formatDate(item.expiryDate)}</b>
              </span>
            </div>
            <button className="legal-record-toggle" type="button" aria-expanded={open} onClick={() => setOpenId(open ? null : item.id)}>
              {open ? "إغلاق الملف" : "فتح الملف الكامل"}
            </button>
            {open && (
              <div className="legal-record-details">
                {item.referralReason && (
                  <p>
                    <strong>سبب الإحالة</strong>
                    <span>{item.referralReason}</span>
                  </p>
                )}
                {summary ? (
                  <div className="legal-file-stats">
                    <span>
                      <b>{summary.documents}</b> مستندات
                    </span>
                    <span>
                      <b>{summary.payments}</b> دفعات
                    </span>
                    <span>
                      <b>{summary.finances}</b> حركات مالية
                    </span>
                    <span>
                      <b>{summary.workers}</b> عمال
                    </span>
                  </div>
                ) : (
                  <p>ملف قانوني مسجل يدويًا ولا يحتوي لقطة عقد محالة.</p>
                )}
                <small>
                  أحيل بواسطة {item.referredBy || "النظام"} في {formatDate(item.referredAt, true)}
                </small>
              </div>
            )}
          </article>
...
function WorkforceOperations({ workers, attachments }: { workers: WorkerRecord[]; attachments: WorkerAttachment[] }) {
  ...
  const beneficiaries = Array.from(
    ...
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
  const professions = workforceProfessions
    .map((profession) => ({
  ...
  return (
    <section className="workforce-command-grid">
      <article className="panel allocation-panel">
        <div className="panel-head">
          <div>
            <h2>التوزيع التشغيلي الحالي</h2>
            <p>الجهات المستفيدة ونسبة إشغال القوى العاملة</p>
          </div>
        ...
        <div className="allocation-body">
          <div
            ...
            style={{
              background: `conic-gradient(var(--admin-red) ${utilization}%, #e8edf0 0)`,
            }}
          >
            <span>
          ...
          <div className="beneficiary-list">
            {beneficiaries.length ? (
              beneficiaries.map(([name, count]) => (
                <div key={name}>
                  <p>
                    <strong>{name}</strong>
                    <small>{count} عامل</small>
                  </p>
                  <span>
                    <i
                      style={{
                        width: `${Math.max(12, Math.round((count / Math.max(assigned, 1)) * 100))}%`,
                      }}
                    />
                  </span>
                </div>
              ...
              <div className="empty-operational">لا توجد عمالة مسندة إلى جهات حالياً.</div>
            )}
          </div>
        </div>
      </article>
      <article className="panel readiness-panel">
        <div className="panel-head">
          <div>
            <h2>الجاهزية حسب المهنة</h2>
            <p>المسجل والمتاح للعقود الجديدة</p>
          </div>
          <span className="readiness-score">
            {completeFiles}/{workers.length} مكتمل
          </span>
        </div>
        <div className="profession-readiness">
          {professions.length ? (
            professions.map((item) => (
              <div key={item.label}>
                <span>{item.label}</span>
                <p>
                  <strong>{item.available}</strong> متاح من {item.total}
                </p>
                <div>
                  <i
                    style={{
            ...
            <div className="empty-operational">أضف العمالة لتظهر مؤشرات الجاهزية.</div>
          )}
        </div>
      </article>
    </section>
...
function ContractOperations({ contracts, professions, assignments, onSelect }: { contracts: WorkforceContract[]; professions: ContractProfession[]; assignments: ContractAssignment[]; onSelect: (id: number) => void }) {
  ...
  return (
    <section className="panel contracts-panel">
      <div className="panel-head">
        <div>
          <h2>عقود توفير العمالة</h2>
          <p>المهن المطلوبة والعمالة المسندة والجهات المستفيدة</p>
        </div>
        <span className="panel-count">{contracts.filter((item) => item.status === "active").length} نشط</span>
      </div>
      {rows.length ? (
        <div className="management-table-wrap">
          <table className="management-table contracts-table">
            ...
            <tbody>
              {rows.map((contract) => {
                const contractProfessionRows = professions.filter((item) => item.contractId === contract.id);
                const activeAssignments = assignments.filter((item) => item.contractId === contract.id && item.status === "active");
                const required = contractProfessionRows.reduce((sum, item) => sum + item.requiredCount, 0);
                const assigned = activeAssignments.length;
                const labels = contractProfessionRows.map((item) => `${item.profession} (${item.requiredCount})`).join("، ");
                return (
                  <tr key={contract.id}>
                    <td>
                      <strong dir="ltr">{contract.referenceCode}</strong>
                      <small>{contract.title}</small>
                    </td>
                    <td>
                      <strong>{contract.clientName}</strong>
                      <small>{contract.workSite}</small>
                    </td>
                    <td>
                      <strong>{contractProfessionRows.length} مهنة</strong>
                      <small>{labels || "لم تُسجّل المهن"}</small>
                    </td>
                    <td>
                      <div className="contract-fill">
                        <span>
                          <i
                            style={{
                              width: `${required ? Math.min(100, Math.round((assigned / required) * 100)) : 0}%`,
                            }}
                          />
                        </span>
                        <small>
                          {assigned} من {required} · متبقٍ {Math.max(0, required - assigned)}
                        </small>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      <span className={`status-pill ${contract.status === "active" ? "status-contacted" : "status-closed"}`}>{contract.status === "active" ? "نشط" : contract.status}</span>
                    </td>
                    <td>
                      <button className="worker-view" onClick={() => onSelect(contract.id)}>
                        إدارة العمالة ←
                      </button>
                    </td>
                  </tr>
        ...
        <EmptyRows label="أنشئ عقد توفير عمالة ليظهر هنا مع المهن والأعداد." />
      )}
    </section>
  );
}
...
const contractStatusLabels: Record<string, string> = {
  ...
  terminated: "منهى",
  cancelled: "ملغى",
  superseded: "استبدل بإصدار أحدث",
};
const contractNextStatuses: Record<string, string[]> = {
  ...
  approved: ["active", "sent", "cancelled"],
    ...
    <div className="drawer-layer">
      ...
      <aside className="request-drawer contract-drawer contract-workforce-workspace" role="dialog" aria-modal="true" aria-label={`إدارة عمالة العقد ${contract.referenceCode}`}>
        <div className="drawer-head">
          <div>
            ...
            <h2>إدارة عمالة العقد</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        </div>
        <div className="contract-drawer-summary">
          <div>
          ...
          <div>
            <span>الحالة التعاقدية</span>
            <strong>{contractStatusLabels[contract.status] || contract.status}</strong>
            <small>الإصدار {contract.versionNumber || 1}</small>
          </div>
          <div>
        ...
        <div className="pdf-language-actions">
          <a className="contract-pdf-link" href={`/api/portal/documents/${contract.documentId}?language=ar`}>
            <Icon name="download" /> نسخة عربية
          </a>
          <a className="contract-pdf-link" href={`/api/portal/documents/${contract.documentId}?language=bilingual`}>
            <Icon name="download" /> نسخة عربي/English
          </a>
        </div>
        ...
        {canWrite && !["active", "suspended", "signed", "expired", "terminated", "cancelled", "superseded"].includes(contract.status) && (
          <div className="document-actions">
            ...
            {contract.status === "draft" && !contract.approvedBy && (
              <button className="danger-action" disabled={busy === `contract-delete-${contract.id}`} onClick={() => void onDelete(contract)}>
                حذف مسودة العقد
              </button>
            )}
          </div>
        )}
        {canWrite && (contractNextStatuses[contract.status]?.length || 0) > 0 && (
          <section className="drawer-section contract-lifecycle">
            <h3>دورة اعتماد العقد</h3>
            <p>اعتماد العقد للمالك أو مشرف النظام، وبعد الاعتماد يستطيع صاحب صلاحية التشغيل تفعيله فتبدأ آثاره التشغيلية والمالية.</p>
            <div>
              <select value={nextStatus} onChange={(event) => setNextStatus(event.target.value)}>
                <option value="">اختر الحالة التالية</option>
                {contractNextStatuses[contract.status]
                  .filter((status) => (status === "approved" ? isOwner : isAdmin || !["signed", "terminated", "cancelled", "superseded"].includes(status)))
                  .map((status) => (
                    <option value={status} key={status}>
              ...
              {!["cancelled", "terminated"].includes(nextStatus) && <textarea value={statusReason} onChange={(event) => setStatusReason(event.target.value)} rows={2} maxLength={1000} placeholder="سبب القرار إلزامي للتعليق" />}
              <button
                className="admin-primary"
                disabled={!nextStatus || busy === `contract-status-${contract.id}`}
                onClick={() => {
                  if (["cancelled", "terminated"].includes(nextStatus)) {
                    setShowCancellation(true);
                    return;
                  }
                  void onStatus(contract.id, nextStatus, statusReason).then(() => {
                    setNextStatus("");
                    setStatusReason("");
                  });
                }}
              >
                {busy === `contract-status-${contract.id}` ? "جارٍ الحفظ..." : "حفظ انتقال الحالة"}
              </button>
            </div>
          </section>
        )}
          ...
          <fieldset className="drawer-section contract-absence-panel" disabled={!canRecordAbsence || absenceHistoryLoading || Boolean(absenceError) || contract.status !== "active" || Boolean(busy)} aria-busy={absenceHistoryLoading}>
            ...
            <div>
                ...
                <select value={absenceWorkerId} onChange={(event) => { setAbsenceWorkerId(event.target.value); setReplacementWorkerId(""); }}>
                  ...
                  {assignments
                    ...
                    .map((item) => {
                      ...
                      return (
                        <option key={item.id} value={item.workerId}>
                          {worker?.fullName || `عامل ${item.workerId}`}
                        </option>
                      );
                    })}
                </select>
              ...
              <label>
                ...
                <small>عند اختيار بديل لا يخصم من دفعة العميل.</small>
              </label>
              <label>
                عدد المتغيبين
                <input type="number" min="1" value={absenceWorkerId ? "1" : absenceCount} disabled={Boolean(absenceWorkerId)} onChange={(event) => setAbsenceCount(event.target.value)} />
              </label>
              <label>
                بداية الغياب
                <input
      ...
      {showCancellation && (
        <ContractCancellationDialog
          contract={contract}
          busy={busy === `contract-status-${contract.id}`}
          onClose={() => setShowCancellation(false)}
          onConfirm={async (reason, reasonCode) => {
            await onStatus(contract.id, nextStatus, reason, reasonCode);
...
function RecordModal({ entity, users, linkedEmployeeEmails, busy, onClose, onSubmit }: { entity: Exclude<RecordEntity, "finance" | "workforce">; users: PortalUser[]; linkedEmployeeEmails: Set<string>; busy: boolean; onClose: () => void; onSubmit: (entity: RecordEntity, form: HTMLFormElement) => Promise<void> }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    void onSubmit(entity, event.currentTarget);
  }
  return (
    <div className="modal-layer">
      <button className="drawer-backdrop" aria-label="إغلاق النافذة" onClick={onClose} />
      <section className="record-modal" role="dialog" aria-modal="true" aria-label={titles[entity]}>
        <div className="drawer-head">
          <div>
            <span>سجل جديد</span>
            <h2>{titles[entity]}</h2>
        ...
        <form onSubmit={submit}>
          {entity === "employees" && (
            <>
              <label className="span-two">
                مستخدم الموظف
                <select name="portalUserEmail" required defaultValue="">
                  <option value="" disabled>
                    اختر مستخدمًا نشطًا غير مرتبط بموظف
                  </option>
                  {users
                    .filter((user) => user.status === "active" && !linkedEmployeeEmails.has(user.email))
                    .map((user) => (
              ...
              <label>
                المسمى الوظيفي
                <input name="jobTitle" required maxLength={100} />
              </label>
              <label>
                الإدارة أو القسم
              ...
              <label>
                رقم الجوال
                <input name="mobile" required type="tel" maxLength={20} dir="ltr" />
              </label>
              <label>
                البريد الوظيفي
                <input name="email" type="email" maxLength={160} dir="ltr" />
              </label>
              <label>
                تاريخ الالتحاق
              ...
              {employeeResidency === "resident" && (
                <>
                  ...
                  <label>
                    تاريخ انتهاء رخصة العمل
                    <input name="workPermitExpiry" required type="date" />
                  </label>
                </>
              )}
              <label>
                الراتب الأساسي
                <input name="baseSalary" required type="number" min="0" step="0.01" defaultValue="0" dir="ltr" />
              </label>
              <label>
                بدل السكن
                <input name="housingAllowance" type="number" min="0" step="0.01" defaultValue="0" dir="ltr" />
              </label>
              <label>
                بدل النقل
                <input name="transportAllowance" type="number" min="0" step="0.01" defaultValue="0" dir="ltr" />
              </label>
              <label>
                بدلات أخرى
                <input name="otherAllowance" type="number" min="0" step="0.01" defaultValue="0" dir="ltr" />
              </label>
              <label>
                رقم الآيبان — اختياري
                <input name="iban" value={iban} onChange={(event) => setIban(formatSaudiIban(event.target.value))} inputMode="numeric" maxLength={29} placeholder="SA00 0000 0000 0000 0000 0000" dir="ltr" />
                <small>يبقى SA ثابتًا، وتضاف مسافة تلقائيًا بعد كل أربع خانات.</small>
              </label>
              <label>
                اسم البنك — تلقائي
                <input name="bankName" value={detectedBank} readOnly placeholder={iban.length > 2 ? "سيظهر بعد اكتمال رمز البنك" : "يُعبأ من الآيبان"} />
              </label>
              ...
              <label className="file-drop span-two">
                ...
                <input name="employmentContract" type="file" required accept="application/pdf,image/png,image/jpeg" />
              </label>
            </>
          )}
          {entity === "legal" && (
            <>
              ...
              <label>
                ...
                <input name="counterparty" required maxLength={160} />
              </label>
              <label>
                تاريخ الانتهاء أو التجديد
                <input name="expiryDate" type="date" />
...
function SearchableCombobox({ name, value, options, onChange, placeholder, required = false }: { name: string; value: string; options: readonly string[]; onChange: (value: string) => void; placeholder: string; required?: boolean }) {
  ...
  const filtered = options.filter((option) => hasExactSelection || !value.trim() || matchesWorkforceProfession(option, value)).slice(0, 40);
  ...
  return (
    <div className="searchable-combobox">
      <div>
        ...
        />
        <button type="button" aria-label="عرض الخيارات" onMouseDown={(event) => event.preventDefault()} onClick={() => setOpen((visible) => !visible)}>
          ⌄
        </button>
      </div>
      {open && (
        <div className="combobox-options" id={listId} role="listbox">
          {filtered.length ? (
            ...
            <p>لا توجد خيارات مطابقة. اكتب جزءاً من الاسم للبحث.</p>
          )}
        </div>
      )}
    </div>
...
function WorkerModal({ busy, onClose, onSubmit }: { busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  return (
    <div className="modal-layer">
      <button className="drawer-backdrop" aria-label="إغلاق نافذة إضافة عامل" onClick={onClose} />
      <section className="record-modal worker-modal" role="dialog" aria-modal="true" aria-label="إنشاء ملف عامل">
        <div className="drawer-head">
          <div>
            <span>شؤون العمالة</span>
            <h2>إنشاء ملف عامل متكامل</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        </div>
        <div className="worker-modal-steps">
          <span className="active">1 البيانات</span>
          <span className="active">2 متطلبات المهنة</span>
          <span className="active">3 الجاهزية للعقود</span>
        </div>
        <form onSubmit={submit}>
          <input type="hidden" name="workerCount" value={workerCount} />
          <p className="form-section-title span-two">الإعداد المشترك للدفعة</p>
          <label>
            عدد العمال المراد إضافتهم
            <input type="number" min="1" max="20" value={workerCount} onChange={(event) => setWorkerCount(Math.max(1, Math.min(20, Number(event.target.value) || 1)))} />
          </label>
          <label>
            الجنسية
            <SearchableCombobox name="nationality" value={nationality} options={workforceNationalities} onChange={setNationality} placeholder="اكتب للبحث عن الجنسية" required />
          </label>
          <label className="span-two">
            المهنة
            <SearchableCombobox name="profession" value={profession} options={workforceProfessions.map((item) => item.label)} onChange={setProfession} placeholder="اكتب للبحث عن المهنة" required />
          </label>
          {Array.from({ length: workerCount }, (_, index) => (
            <div className="bulk-worker-entry span-two" key={index}>
              <h3>بيانات العامل {index + 1}</h3>
              <div>
                <label>
                  رقم العامل
                  <input name={`workerNumber:${index}`} required maxLength={30} placeholder={`WRK-${String(index + 1).padStart(3, "0")}`} dir="ltr" />
                </label>
                <label>
                  رقم الإقامة
                  <input name={`iqamaNumber:${index}`} required inputMode="numeric" pattern="[0-9]{10}" maxLength={10} placeholder="10 أرقام" dir="ltr" />
                </label>
                <label>
                  الاسم الكامل
                  <input name={`fullName:${index}`} required maxLength={120} />
                </label>
                <label>
                  رقم الجوال
                  <input name={`mobile:${index}`} required type="tel" maxLength={20} dir="ltr" />
                </label>
                <label>
                  الراتب الشهري (ريال)
                  <input name={`monthlySalary:${index}`} required type="number" min="0.01" step="0.01" />
                </label>
                <label>
                  جهة الكفالة
                  <select
                    name={`sponsorshipType:${index}`}
                    value={workerSponsorship[index] || "dali"}
                    onChange={(event) =>
                      setWorkerSponsorship((items) => ({
                        ...items,
                        [index]: event.target.value as "dali" | "other",
                      }))
                    }
                  >
                    <option value="dali">على كفالة شركة دالي</option>
                ...
                {workerSponsorship[index] === "other" && (
                  <label>
                    اسم الكفيل
                    <input name={`sponsorName:${index}`} required minLength={2} maxLength={160} />
                  </label>
                )}
                <label>
                  اسم البنك
                  <select name={`bankName:${index}`} required defaultValue="">
                    <option value="" disabled>
                      اختر البنك
                    </option>
                    {saudiBanks.map((bank) => (
                ...
                <label>
                  رقم الآيبان السعودي
                  <input name={`iban:${index}`} required pattern="SA[0-9]{22}" minLength={24} maxLength={24} placeholder="SA0000000000000000000000" dir="ltr" />
                </label>
                <label>
                  تاريخ انتهاء الإقامة
                  <input name={`iqamaExpiry:${index}`} required type="date" />
                </label>
                <label>
                  تاريخ انتهاء التأمين الطبي
                  <input name={`medicalInsuranceExpiry:${index}`} required type="date" />
                </label>
                <label className="file-drop requirement-file">
                  صورة الإقامة — إلزامية
                  <input name={`iqamaDocument:${index}`} type="file" required accept="application/pdf,image/png,image/jpeg" />
                </label>
                <label className="file-drop requirement-file">
                  صورة العامل — إلزامية
                  <input name={`photo:${index}`} type="file" required accept="image/png,image/jpeg" />
                </label>
                <label className="file-drop requirement-file">
                  شهادة الآيبان — إلزامية
                  <input name={`ibanCertificate:${index}`} type="file" required accept="application/pdf,image/png,image/jpeg" />
                </label>
                {requirements.map((requirement) => (
                  <label className="file-drop" key={requirement.code}>
                    {requirement.label} — اختياري
                    <input name={`requirement:${requirement.code}:${index}`} type="file" accept="application/pdf,image/png,image/jpeg" />
                  </label>
                ))}
                <label className="file-drop">
                  مرفقات إضافية اختيارية
                  <input name={`extraCertificates:${index}`} type="file" multiple accept="application/pdf,image/png,image/jpeg" />
                </label>
              </div>
            </div>
          ))}
          <p className="form-hint span-two">لن يُنشأ الملف قبل إرفاق الصورة وجميع المستندات المطلوبة للمهنة. يُسجّل العامل متاحاً، ثم تُحدّد الجهة المستفيدة تلقائياً عند اختياره في عقد نشط.</p>
          <div className="modal-actions span-two">
            <button type="button" onClick={onClose}>
              إلغاء
            </button>
            <button className="admin-primary" type="submit" disabled={busy}>
              {busy ? "جارٍ إنشاء الملف..." : "إنشاء ملف العامل"}
            </button>
          </div>
        </form>
      </section>
...
function WorkerDrawer({ worker, attachments, contracts, contractAssignments, canWrite, canArchive, canViewFinance, busy, onClose, onUploadAttachment }: { worker: WorkerRecord; attachments: WorkerAttachment[]; contracts: WorkforceContract[]; contractAssignments: ContractAssignment[]; canWrite: boolean; canArchive: boolean; canViewFinance: boolean; busy: string | null; onClose: () => void; onUploadAttachment: (workerId: number, form: HTMLFormElement) => Promise<void> }) {
  ...
  async function archiveWorker() {
    ...
    const response = await fetch("/api/portal/workers", {
      method: "DELETE",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ id: worker.id, reason }),
    });
  ...
  return (
    <div className="drawer-layer">
      <button className="drawer-backdrop" aria-label="إغلاق ملف العامل" onClick={onClose} />
      <aside className="request-drawer worker-drawer" role="dialog" aria-modal="true" aria-label={`ملف العامل ${worker.fullName}`}>
        <div className="drawer-head">
          <div>
        ...
        <div className="worker-profile-head">
          {photo ? <Image unoptimized src={`/api/portal/workers/attachments/${photo.id}?inline=1`} alt={`صورة ${worker.fullName}`} width={96} height={96} /> : <span>{initials(worker.fullName)}</span>}
          <div>
            <h3>{worker.fullName}</h3>
            <p>
              {worker.profession} · {worker.nationality}
            </p>
            <p>{sponsorshipLabel(worker)}</p>
            <small dir="ltr">إقامة: {worker.iqamaNumber || "غير مسجل"}</small>
          </div>
          <b className={`status-pill ${statusClass(worker.status)}`}>{recordStatus.workforce[worker.status] || worker.status}</b>
        </div>
        <div className="profile-completion">
          ...
          <small>{profile.missing.length ? `متبقٍ ${profile.missing.length} مستند مهني` : "جميع متطلبات المهنة مكتملة"}</small>
        </div>
        <div className="drawer-section worker-facts">
          <h3>البيانات التشغيلية والبنكية</h3>
          <dl>
            <div>
              <dt>الجوال</dt>
              <dd dir="ltr">{worker.mobile || "غير مسجل"}</dd>
            </div>
            <div>
              <dt>البنك</dt>
              <dd>{worker.bankName || "غير مسجل"}</dd>
            </div>
            <div>
              <dt>رقم الآيبان</dt>
            ...
            <div>
              <dt>انتهاء التأمين الطبي</dt>
              <dd className={daysUntil(worker.medicalInsuranceExpiry) <= 30 ? "date-alert" : ""}>{formatDate(worker.medicalInsuranceExpiry)}</dd>
            </div>
            <div>
              <dt>الجهة المستفيدة</dt>
              <dd>{activeContract?.clientName || worker.beneficiaryName || "غير مسند"}</dd>
            </div>
            <div>
              <dt>العقد النشط</dt>
              <dd>{activeContract?.referenceCode || "لا يوجد عقد نشط"}</dd>
            </div>
            <div>
              <dt>موقع العمل</dt>
              <dd>{activeContract?.workSite || worker.clientSite}</dd>
            </div>
            <div>
              <dt>بداية الإسناد</dt>
              <dd>{formatDate(activeAssignment?.assignedAt || worker.assignmentStartDate)}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="drawer-section">
          ...
          <div className="worker-file-list">
            {attachments.map((item) => (
              <a key={item.id} href={`/api/portal/workers/attachments/${item.id}?inline=1`} target="_blank" rel="noreferrer">
                <span>
                  <Icon name={item.documentType === "photo" ? "employees" : "documents"} />
                </span>
                <p>
                  <strong>{item.title}</strong>
                  <small>
                    {item.fileName} · {formatBytes(item.sizeBytes)}
                    {item.expiryDate ? ` · ينتهي ${formatDate(item.expiryDate)}` : " · دون انتهاء"}
                  </small>
                </p>
                <span>معاينة</span>
              </a>
          ...
          {!attachments.length && <div className="empty-operational">لا توجد مرفقات في هذا الملف.</div>}
          {canWrite && (
            <form className="attachment-upload" onSubmit={submitAttachment}>
              <strong>إضافة مرفق إلى ملف العامل</strong>
              <label>
                نوع المستند
                <select name="requirementCode" defaultValue="">
                  <option value="">مرفق إضافي</option>
              ...
              <label>
                اسم المرفق
                <input name="title" required maxLength={160} placeholder="مثال: بطاقة صحية" />
              </label>
              <label>
                تاريخ انتهاء المرفق
                <input name="expiryDate" type="date" />
              ...
              <label>
                الملف
                <input name="file" type="file" required accept="application/pdf,image/png,image/jpeg" />
              </label>
              <button className="admin-primary" disabled={busy === `worker-attachment-${worker.id}`}>
                {busy === `worker-attachment-${worker.id}` ? "جارٍ الرفع..." : "رفع المرفق"}
              </button>
            </form>
          )}
        </div>
        <div className="drawer-section">
          <h3>مصدر الجهة المستفيدة</h3>
          <div className="contract-source-note">
            <Icon name="documents" />
            <p>
              <strong>{activeContract ? "الإسناد مرتبط بعقد نشط" : "العامل متاح للعقود"}</strong>
          ...
          {!canWrite && <p className="readonly-note">صلاحية حسابك تتيح الاطلاع والتنزيل فقط.</p>}
        </div>
      </aside>
    </div>
  );
  ...
  return (
    <div className="request-table-wrap">
      <table className="request-table">
        ...
        <tbody>
          {requests.map((item) => {
            const status = requestStatuses[safeRequestStatus(item.status)];
            return (
              <tr key={item.id} onClick={() => onSelect(item.id)}>
                <td>
                  <b>{item.trackingCode}</b>
                </td>
                <td>
                  <strong>{item.companyName || item.fullName}</strong>
                  <small>{item.companyName ? `${item.fullName} · ${item.mobile}` : item.mobile}</small>
                </td>
                <td>
                  <span>{item.specialization}</span>
                  {item.requestType === "quotation" && <small className="quote-request-kind">طلب عرض سعر</small>}
                </td>
                ...
                <td>
                  <button aria-label={`عرض طلب ${item.trackingCode}`}>←</button>
                </td>
              </tr>
            );
          })}
...
function StructuredQuoteRequest({ request }: { request: WorkforceRequest }) {
  ...
  try {
    items = JSON.parse(request.quotationItemsJson) as typeof items;
  } catch {}
  try {
    terms = JSON.parse(request.quotationTermsJson || "{}") as Record<string, string | null>;
  } catch {}
  const party = (value: string | null | undefined) => (value === "client" ? "العميل" : value === "dali" ? "شركة دالي" : "لا ينطبق");
  return (
    <>
      <div className="drawer-section">
        <h3>بيانات المنشأة التعاقدية</h3>
        <dl>
          <div>
            <dt>السجل التجاري</dt>
            <dd>{request.clientCr || "غير مسجل"}</dd>
          </div>
          <div>
            <dt>الرقم الضريبي</dt>
            <dd>{request.clientVat || "غير مسجل"}</dd>
          </div>
          <div>
            <dt>العنوان الوطني</dt>
            <dd>{request.clientAddress || "غير مسجل"}</dd>
          </div>
          <div>
            <dt>صفة الممثل</dt>
            <dd>{request.representativeTitle || "غير مسجلة"}</dd>
          </div>
          <div>
            <dt>نوع النشاط</dt>
          ...
          <div>
            <dt>نطاق العدد</dt>
            <dd>{request.quantityMode === "open" ? "عدد مفتوح" : "عدد محدد"}</dd>
          </div>
        </dl>
      </div>
      <div className="drawer-section">
        <h3>بنود العرض المطلوبة</h3>
        <div className="request-quote-items">
          {items.map((item, index) => (
            <article key={`${item.description}-${index}`}>
              <strong>{item.description || `البند ${index + 1}`}</strong>
              <span>
                {request.quantityMode === "open" ? "عدد مفتوح" : `${item.quantity || 0} ${item.unit || "وحدة"}`} · {item.durationMonths || 0} شهر
              </span>
              {item.sponsorshipType && <small>{item.sponsorshipType === "dali" ? "على كفالة شركة دالي" : `على كفالة ${item.sponsorName || "جهة أخرى"} — ${item.ajirContractStatus === "with_ajir" ? "بعقد أجير" : "بدون عقد أجير"}`}</small>}
      ...
      <div className="drawer-section">
        <h3>شروط التشغيل المقترحة</h3>
        <dl>
          <div>
            <dt>تاريخ النهاية</dt>
            <dd>{formatDate(terms.endDate)}</dd>
          </div>
          <div>
            <dt>ساعات العمل</dt>
            <dd>{terms.workingHours || "غير محددة"}</dd>
          </div>
          <div>
            <dt>الراحة الأسبوعية</dt>
            <dd>{terms.weeklyOff || "غير محددة"}</dd>
          </div>
          <div>
            <dt>السكن</dt>
            <dd>{party(terms.accommodationParty)}</dd>
          </div>
          <div>
            <dt>النقل</dt>
            <dd>{party(terms.transportParty)}</dd>
          </div>
          <div>
            <dt>شروط الدفع</dt>
            <dd>{terms.paymentTerms || "تحدد في العرض"}</dd>
          </div>
        </dl>
        {terms.specialTerms && <p className="request-details">{terms.specialTerms}</p>}
      </div>
    </>
  );
}
function RequestDrawer({ canConvert, canApprove, onApproval, onConvert, request, replies, emailConfigured, canWrite, statusBusy, replyBusy, onClose, onStatus, onReply }: { canConvert: boolean; canApprove: boolean; onApproval: (id: number, action: "approve" | "reject" | "request-changes") => void; onConvert: (id: number) => void; request: WorkforceRequest; replies: WorkforceRequestReply[]; emailConfigured: boolean; canWrite: boolean; statusBusy: boolean; replyBusy: boolean; onClose: () => void; onStatus: (id: number, status: RequestStatus) => void; onReply: (id: number, form: HTMLFormElement) => Promise<void> }) {
  ...
  const deliveryLabels: Record<string, string> = {
    ...
    pending: "قيد الإرسال",
    failed: "فشل الإرسال",
    configuration_required: "تحتاج تهيئة البريد",
  };
  return (
    <div className="drawer-layer">
      <button className="drawer-backdrop" aria-label="إغلاق تفاصيل الطلب" onClick={onClose} />
      <aside className="request-drawer visitor-request-drawer" role="dialog" aria-modal="true" aria-label="تفاصيل طلب الزائر">
        <div className="drawer-head">
          <div>
            <span>{request.trackingCode}</span>
            <h2>{request.requestType === "quotation" ? "طلب عرض سعر" : "طلب الزائر والردود"}</h2>
          </div>
          <button onClick={onClose} aria-label="إغلاق">
            <Icon name="close" />
          </button>
        </div>
        {request.requestType === "quotation" && <div className="drawer-section"><h3>اعتماد طلب عرض السعر</h3><p>{request.approvalStatus === "approved" ? "معتمد وقابل للتحويل إلى عرض سعر" : request.approvalStatus === "rejected" ? "مرفوض" : "بانتظار اعتماد المالك أو مشرف النظام"}</p>{request.approvalReason && <p>{request.approvalReason}</p>}{canApprove && request.approvalStatus === "pending" && <div className="record-actions"><button disabled={statusBusy} onClick={() => onApproval(request.id, "approve")}>اعتماد طلب عرض السعر</button><button disabled={statusBusy} onClick={() => onApproval(request.id, "reject")}>رفض بدون تعديلات</button><button disabled={statusBusy} onClick={() => onApproval(request.id, "request-changes")}>إعادة للتعديل</button></div>}{canConvert && request.approvalStatus === "approved" && <button className="admin-primary" onClick={() => onConvert(request.id)}>تحويل الطلب إلى عرض سعر</button>}</div>}
        <div className="drawer-status">
          <span>حالة الطلب</span>
          {canWrite ? (
            <select value={safeRequestStatus(request.status)} disabled={statusBusy} onChange={(event) => onStatus(request.id, event.target.value as RequestStatus)}>
              {Object.entries(requestStatuses).map(([value, meta]) => (
                <option value={value} key={value}>
                  {meta.label}
                </option>
              ))}
            ...
            <span className={`status-pill ${requestStatuses[safeRequestStatus(request.status)].className}`}>{requestStatuses[safeRequestStatus(request.status)].label}</span>
          )}
        </div>
        <div className="drawer-section">
          <h3>بيانات الزائر</h3>
          <dl>
            ...
            <div>
              <dt>الشركة أو الجهة</dt>
              <dd>{request.companyName || "غير محددة"}</dd>
            </div>
            <div>
              <dt>رقم الجوال</dt>
              <dd dir="ltr">{request.mobile}</dd>
            </div>
            <div>
        ...
        <div className="drawer-section">
          <h3>{request.requestType === "quotation" ? "بيانات عرض السعر" : "موضوع الطلب"}</h3>
          <dl>
            <div>
              <dt>نوع الاحتياج</dt>
              <dd>{request.specialization}</dd>
            ...
            <div>
              <dt>موقع العمل</dt>
              <dd>{request.workSite || "غير محدد"}</dd>
            </div>
            {request.requestType === "quotation" && (
              <>
                <div>
                  <dt>العدد التقريبي</dt>
                  <dd>{request.quantityMode === "open" ? "مفتوح" : request.requestedCount || "غير محدد"}</dd>
                </div>
                <div>
                  <dt>تاريخ البدء</dt>
                  <dd>{formatDate(request.requiredStartDate)}</dd>
                </div>
                <div>
                  <dt>مدة التعاقد</dt>
                  <dd>{request.duration || "غير محددة"}</dd>
                </div>
                <div>
                  <dt>التواصل المفضل</dt>
                  <dd>{request.preferredContact === "phone" ? "اتصال هاتفي" : request.preferredContact === "email" ? "بريد إلكتروني" : "الجوال أو البريد"}</dd>
                </div>
              </>
            )}
            <div>
        ...
        {canWrite && (
          <div className="drawer-section reply-composer">
            <div className="reply-section-title">
              ...
              <div>
                <h3>الرد عبر البريد الإلكتروني</h3>
                <p>سيُحفظ النص وحالة التسليم في سجل الطلب، وتتحول الحالة تلقائياً إلى «تم التواصل» بعد نجاح الإرسال.</p>
              </div>
            </div>
            {!emailConfigured && (
              <div className="email-configuration-alert">
                <strong>خدمة البريد غير مهيأة</strong>
                <span>يجب على مدير النظام إضافة مفتاح خدمة الإرسال وبريد المرسل قبل تفعيل زر الإرسال.</span>
              </div>
            )}
            <form onSubmit={submitReply}>
              <label>
                عنوان الرسالة
                <input name="subject" required minLength={3} maxLength={180} defaultValue={`رد شركة دالي للتشغيل والصيانة على طلبكم ${request.trackingCode}`} />
              </label>
              <label>
                نص الرد
                <textarea name="body" required minLength={2} maxLength={10000} rows={8} placeholder="اكتب الرد الذي سيصل إلى بريد الزائر كما هو..." />
              </label>
              <button className="admin-primary" disabled={replyBusy || !emailConfigured}>
                {replyBusy ? "جارٍ الإرسال..." : "إرسال الرد وحفظه"}
                <Icon name="mail" />
              </button>
            </form>
          </div>
        )}
        <div className="drawer-section reply-history">
          <div className="reply-section-title">
            <span>
              <Icon name="documents" />
            </span>
            <div>
              <h3>سجل المراسلات</h3>
              <p>{replies.length} رسالة مرتبطة بهذا الطلب</p>
            </div>
          </div>
          {replies.length ? (
                ...
                <article key={reply.id}>
                  <header>
                    <p>
                      ...
                      <small>
                        بواسطة {reply.senderName} · {formatDate(reply.sentAt || reply.createdAt, true)}
                      </small>
                    </p>
                    <span className={`reply-status ${reply.deliveryStatus}`}>{deliveryLabels[reply.deliveryStatus] || reply.deliveryStatus}</span>
                  </header>
                  <p>{reply.body}</p>
                  <footer>
                    <span>
                      إلى: <b dir="ltr">{reply.recipientEmail}</b>
                    </span>
                    {reply.deliveryStatus === "sent" && (
                      <span>
                        <Icon name="check" /> موثّق في سجل التسليم
                      </span>
                    )}
                  </footer>
                </article>
            ...
            <div className="empty-operational">لم يُرسل رد على هذا الطلب بعد.</div>
          )}
        </div>
        <div className="drawer-actions">
          <a href={`tel:${request.mobile}`}>اتصال بالعميل</a>
          <a href={`mailto:${request.email}`}>فتح البريد الخارجي</a>
        </div>
      </aside>
    </div>
  );
...
function activityLabel(action: string) {
  ...
  const labels: Record<string, string> = {
    "request-status-updated": "تحديث حالة طلب",
    "user-access-updated": "تحديث صلاحية مستخدم",
    "employee-created": "إضافة موظف",
    "financial-record-created": "إضافة سجل مالي",
    "legal-record-created": "إضافة ملف قانوني",
    "worker-created": "إضافة عامل",
    "worker-profile-created": "إنشاء ملف عامل متكامل",
    "worker-assignment-updated": "تحديث إسناد عامل",
    "worker-certificate-uploaded": "إضافة شهادة عامل",
    "company-document-uploaded": "رفع مستند شركة",
    "official-pdf-issued": "إصدار ملف PDF رسمي",
    "workforce-contract-created": "إنشاء عقد توفير عمالة",
    "contract-worker-assigned": "إسناد عامل إلى عقد",
    "contract-worker-released": "إنهاء إسناد عامل",
    "document-share-created": "إنشاء رابط مشاركة",
    "document-downloaded": "تنزيل مستند",
    "company-stamp-updated": "تحديث ختم الشركة",
    "company-signature-updated": "تحديث توقيع الشركة",
    "live-chat-replied": "الرد على محادثة زائر",
    "live-chat-closed": "إغلاق محادثة زائر",
    "live-chat-status-updated": "تحديث حالة محادثة",
    "business-hours-updated": "تحديث ساعات الدوام",
  };
