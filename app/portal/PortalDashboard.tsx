...
type WorkerRecord = {
  ...
  mobile: string | null; beneficiaryName: string | null; clientSite: string; assignmentStartDate: string | null;
  iqamaExpiry: string | null; status: string; createdAt: string; updatedAt: string;
};
type WorkerAttachment = {
  id: number; workerId: number; documentType: string; requirementCode: string | null; title: string;
  fileName: string; contentType: string; sizeBytes: number; createdBy: string; createdAt: string;
};
...
type WorkforceContract = {
  ...
  details: string; status: string; versionNumber: number; parentContractId: number | null; approvedBy: string | null; approvedAt: string | null;
...
type ContractProfession = { id: number; contractId: number; profession: string; requiredCount: number; createdAt: string };
...
function DocumentCenter({ documents, assets, query, setQuery, canManageDocuments, canManageAssets, busy, expiringDocuments, expiredDocuments, onUpload, onIssue, onShare, onUploadAsset }: {
  documents: CompanyDocument[]; assets: CompanyAsset[]; query: string; setQuery: (value: string) => void;
  ...
  onUpload: () => void; onIssue: () => void; onShare: (id: number) => Promise<void>;
  ...
  return <>
    <div className="content-heading module-heading documents-heading"><div><p className="admin-eyebrow">الحفظ والإصدار الرسمي</p><h1>مركز المستندات</h1><span>إدارة ملفات الشركة، ومشاركة النسخ، وإصدار العقود والمستندات الرسمية.</span></div>{canManageDocuments && <div className="heading-actions"><button className="admin-secondary" onClick={onUpload}><Icon name="upload"/>رفع مستند</button><button className="admin-primary" onClick={onIssue}><Icon name="plus"/>إنشاء PDF</button></div>}</div>
...
function DocumentTable({ documents, canShare, busy, onShare }: { documents: CompanyDocument[]; canShare: boolean; busy: string | null; onShare: (id: number) => Promise<void> }) {
  ...
  return <div className="management-table-wrap"><table className="management-table documents-table"><thead><tr><th>المرجع</th><th>المستند</th><th>التصنيف</th><th>الجهة</th><th>الانتهاء</th><th>الحجم</th><th>الإجراءات</th></tr></thead><tbody>{documents.map((item) => { const days = daysUntil(item.expiryDate); const expiryClass = days < 0 ? "expired" : days <= 30 ? "expiring" : ""; return <tr key={item.id}><td dir="ltr"><b>{item.referenceCode}</b></td><td><strong>{item.title}</strong><small>{item.source === "generated" ? issuedTypeLabels[item.documentType || ""] || "PDF صادر" : item.fileName}{item.lockedUntil ? ` · حجز نظامي حتى ${formatDate(item.lockedUntil)}` : item.retentionUntil ? ` · مراجعة الاحتفاظ ${formatDate(item.retentionUntil)}` : ""}</small></td><td><span className={`doc-kind ${item.source === "generated" ? "generated" : ""}`}>{item.source === "generated" ? "صادر من النظام" : documentCategoryLabels[item.category] || item.category}</span></td><td>{item.counterparty || "—"}</td><td><span className={`expiry-pill ${expiryClass}`}>{expiryText(item.expiryDate)}</span></td><td>{formatBytes(item.sizeBytes)}</td><td><div className="document-actions"><a href={`/api/portal/documents/${item.id}?inline=1`} target="_blank" rel="noreferrer" aria-label={`معاينة ${item.title}`}><Icon name="documents"/><span>معاينة</span></a><a href={`/api/portal/documents/${item.id}`} aria-label={`تنزيل ${item.title}`}><Icon name="download"/><span>تنزيل</span></a>{canShare && <button disabled={busy === `share-${item.id}`} onClick={() => void onShare(item.id)} aria-label={`مشاركة ${item.title}`}><Icon name="share"/><span>{busy === `share-${item.id}` ? "جارٍ الإنشاء" : "مشاركة"}</span></button>}</div></td></tr>; })}</tbody></table></div>;
}
...
function IssueDocumentModal({ initialType, busy, assetsReady, workers, contracts, requests, onClose, onSubmit }: { initialType: string; busy: boolean; assetsReady: boolean; workers: WorkerRecord[]; contracts: WorkforceContract[]; requests: WorkforceRequest[]; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  useEffect(()=>{window.setTimeout(()=>{const form=document.querySelector<HTMLFormElement>(".issue-modal form");const amount=form?.elements.namedItem("amount");if(amount instanceof HTMLInputElement){amount.disabled=seasonType==="regular";amount.required=seasonType!=="regular";}},0);},[seasonType]);
  ...
  function addProfession() {
    ...
    if (!next) return;
    setProfessions((items) => [...items, { key: `profession-${Date.now()}`, profession: next.label, customProfession: "", requiredCount: 1, unitSalary: 0 }]);
  }
  ...
  function applyQuote(event: React.ChangeEvent<HTMLSelectElement>){const id=event.target.value;setSelectedQuoteId(id);const quote=convertibleQuotes.find(item=>String(item.id)===id);if(!quote)return;setQuantityMode(quote.quantityMode);if(quote.items.length)setProfessions(quote.items.map((item,index)=>({key:`quote-profession-${index}`,profession:item.profession,requiredCount:quote.quantityMode==="open"?0:Math.max(1,item.quantity)})));window.setTimeout(()=>{const form=event.target.form;if(!form)return;const client=form.elements.namedItem("clientName");const title=form.elements.namedItem("title");const vat=form.elements.namedItem("vatRate");const amount=form.elements.namedItem("amount");if(client instanceof HTMLInputElement)client.value=quote.clientName;if(title instanceof HTMLInputElement)title.value=`عقد ناتج عن ${quote.quoteCode} — ${quote.title}`;if(vat instanceof HTMLInputElement)vat.value=String(quote.vatRateBps/100);if(amount instanceof HTMLInputElement)amount.value=String(quote.subtotalHalalas/100);},0);}
  const serializedProfessions = JSON.stringify(professions.map((item) => ({ profession: item.profession === "أخرى" ? (item.customProfession || "").trim() : item.profession, requiredCount: quantityMode==="open"?0:item.requiredCount, unitSalary:item.unitSalary || 0, workerIds: quantityMode==="open"?[]:(selectedWorkers[item.key] || []) })));
  ...
  return <div className="modal-layer"><button className="drawer-backdrop" aria-label="إغلاق نافذة إصدار المستند" onClick={onClose}/><section className="record-modal document-modal issue-modal" role="dialog" aria-modal="true" aria-label="إنشاء ملف PDF رسمي"><div className="drawer-head"><div><span>الإصدار الرسمي</span><h2>{isContract ? "إنشاء عقد توفير عمالة" : "إنشاء ملف PDF"}</h2></div><button onClick={onClose} aria-label="إغلاق"><Icon name="close"/></button></div>
    ...
    <form className={`contract-quantity-${quantityMode}`} onSubmit={submit}>
      ...
      {isContract&&step===1&&<div className="contract-billing-controls span-two"><label>نوع التعاقد<select name="seasonType" value={seasonType} onChange={event=>setSeasonType(event.target.value as "regular"|"ramadan"|"hajj")}><option value="regular">عقد عادي — فوترة شهرية</option><option value="ramadan">موسم رمضان — دفعات بالنسب</option><option value="hajj">موسم الحج — دفعات بالنسب</option></select></label>{seasonType==="regular"&&<label>تاريخ استحقاق أول دفعة<input name="firstPaymentDueDate" type="date" required defaultValue={new Date().toISOString().slice(0,10)}/></label>}<p className="form-hint span-two">في العقد العادي تُنشأ الدفعات شهرياً من رواتب المهن. في الحج ورمضان تُستخدم نسب جدول الدفعات ويجب أن يكون مجموعها 100٪.</p></div>}
      ...
      {isContract && step === 2 && <div className="contract-profession-pricing span-two"><div className="profession-builder-head"><div><strong>الراتب والمهنة المخصصة</strong><small>الراتب شهري للعامل الواحد، ويُستخدم تلقائياً في إنشاء الاستحقاقات والفواتير المحاسبية.</small></div></div><div className="profession-builder">{professions.map((item)=><article key={`pricing-${item.key}`}>{item.profession==="أخرى"&&<label>اكتب المهنة يدوياً<input required minLength={2} maxLength={120} value={item.customProfession||""} onChange={event=>setCustomProfession(item.key,event.target.value)} placeholder="مثال: فني مضخات"/></label>}<label>راتب العامل الشهري (ريال)<input required={quantityMode==="fixed"} type="number" min={quantityMode==="fixed"?0.01:0} step="0.01" value={item.unitSalary||""} onChange={event=>setProfessionSalary(item.key,Number(event.target.value))}/></label><p>{item.profession==="أخرى"?(item.customProfession||"مهنة أخرى"):item.profession} · {quantityMode==="open"?"عدد مفتوح":`${item.requiredCount} عامل`} · {formatMoney(Math.round((item.unitSalary||0)*100))} للعامل/شهر</p></article>)}</div></div>}
...
function EmployeeTable({ records, query, canWrite, busy, onStatus }: { records: EmployeeRecord[]; query: string; canWrite: boolean; busy: string | null; onStatus: (id: number, status: string) => void }) {
  const rows = filterRecords(records, query); if (!rows.length) return <EmptyRows label="أضف أول ملف موظف ليظهر هنا."/>;
  return <div className="management-table-wrap"><table className="management-table"><thead><tr><th>الرقم الوظيفي</th><th>الموظف</th><th>المسمى</th><th>الإدارة</th><th>تاريخ الالتحاق</th><th>الحالة</th></tr></thead><tbody>{rows.map((item) => <tr key={item.id}><td dir="ltr">{item.employeeNumber}</td><td><strong>{item.fullName}</strong><small dir="ltr">{item.mobile}</small></td><td>{item.jobTitle}</td><td>{item.department}</td><td>{formatDate(item.hireDate)}</td><td><StatusControl entity="employees" id={item.id} value={item.status} canWrite={canWrite} busy={busy} onStatus={onStatus}/></td></tr>)}</tbody></table></div>;
}
...
function WorkerDrawer({ worker, attachments, contracts, contractAssignments, canWrite, busy, onClose, onUploadAttachment }: {
  ...
  const missingOptions = profile.missing;
