...
type WorkerRecord = {
  ...
  mobile: string | null; iban: string | null; bankName: string | null; beneficiaryName: string | null; clientSite: string; assignmentStartDate: string | null;
...
function workerRequirementStatus(worker: WorkerRecord, attachments: WorkerAttachment[]) {
  ...
  const total = 7;
  const complete = (hasPhoto ? 1 : 0) + (hasIqamaCopy ? 1 : 0) + (hasIbanCertificate ? 1 : 0) + (worker.iqamaNumber ? 1 : 0) + (worker.iban ? 1 : 0) + (worker.iqamaExpiry ? 1 : 0) + (worker.medicalInsuranceExpiry ? 1 : 0);
  ...
  async function createWorker(form: HTMLFormElement) {
    ...
    try {
      ...
      for(let index=0;index<count;index++){
        ...
        for(const name of ["workerNumber","iqamaNumber","fullName","mobile","iban","bankName","iqamaExpiry","medicalInsuranceExpiry","photo","iqamaDocument","ibanCertificate"])payload.set(name,source.get(`${name}:${index}`) as FormDataEntryValue);
