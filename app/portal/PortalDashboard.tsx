...
const financeLabels: Record<string, string> = {
  worker_salary: "راتب عامل", worker_advance: "سلفة عامل", worker_deduction: "خصم عامل", worker_violation: "مخالفة عامل", worker_expense: "مصروف عمالة",
...
export default function PortalDashboard({ currentUser, initialRequests, initialRequestReplies, initialNotifications, initialUsers, initialActivity, initialEmployees, initialFinance, initialLegal, initialWorkers, initialWorkerAttachments, initialDocuments, initialAssets, initialContracts, initialContractProfessions, initialContractAssignments, initialConversations, initialConversationMessages, initialBusinessHours, initialChatAutomation, initialWebsiteContent, canAccessWebsite, canManageWebsite, canAccessConstruction, canManageChatSettings, canManageDocuments, canManageAssets, emailConfigured, signOutPath }: {
  ...
  useEffect(() => {
    ...
    return () => { window.clearTimeout(timer); window.removeEventListener("dali-server-changes", refreshWithoutReload); };
  }, [router]);
  ...
  async function updateContractStatus(contractId: number, status: string, reason: string, reasonCode?: "late_payment" | "other") {
    ...
    try {
      ...
      const response = await fetch(`/api/portal/contracts/${contractId}/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status, reason, reasonCode, stampId }) });
  ...
  async function recordContractAbsence(contractId: number, contractProfessionId: number, absenceDate: string, absenceEndDate: string, workerId: number | null, replacementWorkerId: number | null, absentCount: number, notes: string) {
    ...
    try {
      const response = await fetch(`/api/portal/contracts/${contractId}/attendance`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ contractProfessionId, absenceDate, absenceEndDate, workerId, replacementWorkerId, absentCount, notes }),
      });
      const result = await readApiJson(response) as { absence?: { deductionHalalas: number }; error?: string };
  ...
  async function editContract(contract: WorkforceContract, changes: {clientName:string;title:string;workSite:string;startDate:string;endDate?:string}) {
    setBusy(`contract-edit-${contract.id}`);
    try {
      const response = await fetch(`/api/portal/contracts/${contract.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(changes) });
    ...
    } catch (error) { notify(error instanceof Error ? error.message : "تعذّر تعديل العقد."); throw error; }
    finally { setBusy(null); }
  }
  ...
  return <main className="admin-shell">
    ...
    {documentModal === "issue" && issuePreset === "quotation" && <QuotationIssueModal onClose={() => { setDocumentModal(null); setIssueReturnView(null); }} onCreated={(message) => { notify(message); setOperationsTab("quotes"); changeView(issueReturnView === "contractual-documents" ? "contractual-documents" : "operations"); setIssueReturnView(null); }}/>}
...
function CreateUserModal({ busy, onClose, onSubmit }: { busy: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  useEffect(()=>{let active=true;void fetch("/api/portal/role-definitions",{cache:"no-store"}).then(async response=>{const data=await readApiJson(response)as{roles?:Array<{roleKey:string;labelAr:string;description:string|null;active:boolean}>;error?:string};if(!response.ok)throw new Error(data.error||"تعذر تحميل الأدوار");if(active){const assignable=new Set(["system_owner","system_admin","hr_officer","accountant","government_relations_officer","administrative_assistant","lawyer","workforce_supervisor"]);const available=(data.roles||[]).filter(role=>role.active&&assignable.has(role.roleKey));setRoles(available);setSelectedRoles(current=>current.filter(key=>available.some(role=>role.roleKey===key)))}}).catch(error=>{if(active)setLoadError(error instanceof Error?error.message:"تعذر تحميل الأدوار")});return()=>{active=false}},[]);
...
function FinanceRecordModal({ busy, workers, contracts, onClose, onSubmit }: { busy: boolean; workers: WorkerRecord[]; contracts: WorkforceContract[]; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  return <div className="modal-layer"><button className="drawer-backdrop" aria-label="إغلاق نافذة الحركة المالية" onClick={onClose}/><section className="record-modal finance-modal" role="dialog" aria-modal="true" aria-label="إضافة حركة مالية للعمالة"><div className="drawer-head"><div><span>الإدارة المالية</span><h2>إضافة حركة مالية للعمالة</h2></div><button onClick={onClose} aria-label="إغلاق"><Icon name="close"/></button></div><form onSubmit={submit}>
    <label>نوع الحركة<select name="category" value={category} onChange={(event) => setCategory(event.target.value)}><option value="worker_salary">راتب عامل</option><option value="worker_advance">سلفة عامل</option><option value="worker_deduction">خصم عامل</option><option value="worker_violation">مخالفة عامل</option><option value="worker_expense">مصروف خاص بالعمالة</option><option value="workforce_invoice">فاتورة عمالة</option><option value="receipt_voucher">سند قبض</option><option value="payment_voucher">سند صرف</option><option value="progress_claim">مستخلص عمالة</option></select></label><label>المبلغ بالريال<input name="amount" required type="number" min="0.01" max="1000000000" step="0.01" dir="ltr"/></label>
...
function IssueDocumentModal({ initialType, initialQuoteId, busy, assetsReady, workers, contracts, requests, onClose, onSubmit }: { initialType: string; initialQuoteId:number|null; busy: boolean; assetsReady: boolean; workers: WorkerRecord[]; contracts: WorkforceContract[]; requests: WorkforceRequest[]; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  type ConvertibleQuote={id:number;quoteCode:string;quantityMode:"fixed"|"open";seasonType:"regular"|"ramadan"|"hajj";paymentScheduleJson:string|null;vatRateBps:number;subtotalHalalas:number;accommodationParty:"dali"|"counterparty";transportParty:"dali"|"counterparty";clientName:string;title:string;items:Array<{profession:string;quantity:number;unitPriceHalalas?:number;sponsorshipType?:"dali"|"other"|null;sponsorName?:string|null;ajirContractStatus?:"not_applicable"|"with_ajir"|"without_ajir"|null}>};
  ...
  useEffect(()=>{if(initialType!=="workforce_contract")return;void fetch("/api/portal/operations?limit=100",{cache:"no-store"}).then(response=>response.ok?response.json():Promise.reject()).then((raw:unknown)=>{const data=raw as {quotes?:Array<{id:number;quoteCode:string;opportunityId:number;status:string;quantityMode:"fixed"|"open";seasonType:"regular"|"ramadan"|"hajj";paymentScheduleJson:string|null;vatRateBps:number;subtotalHalalas:number;accommodationParty?:"dali"|"counterparty";transportParty?:"dali"|"counterparty"}>;quoteItems?:Array<{quoteVersionId:number;profession:string;quantity:number;unitPriceHalalas:number;sponsorshipType?:"dali"|"other"|null;sponsorName?:string|null;ajirContractStatus?:"not_applicable"|"with_ajir"|"without_ajir"|null}>;opportunities?:Array<{id:number;clientId:number|null;title:string}>;clients?:Array<{id:number;legalName:string}>};setConvertibleQuotes((data.quotes||[]).filter(quote=>["approved","sent","accepted"].includes(quote.status)).map(quote=>{const opportunity=(data.opportunities||[]).find(item=>item.id===quote.opportunityId);const client=(data.clients||[]).find(item=>item.id===opportunity?.clientId);return{id:quote.id,quoteCode:quote.quoteCode,quantityMode:quote.quantityMode||"fixed",seasonType:quote.seasonType||"regular",paymentScheduleJson:quote.paymentScheduleJson||null,vatRateBps:quote.vatRateBps||0,subtotalHalalas:quote.subtotalHalalas||0,accommodationParty:quote.accommodationParty||"dali",transportParty:quote.transportParty||"dali",clientName:client?.legalName||"",title:opportunity?.title||quote.quoteCode,items:(data.quoteItems||[]).filter(item=>item.quoteVersionId===quote.id).map(item=>({profession:item.profession,quantity:item.quantity,unitPriceHalalas:item.unitPriceHalalas,sponsorshipType:item.sponsorshipType,sponsorName:item.sponsorName,ajirContractStatus:item.ajirContractStatus}))};}));}).catch(()=>setConvertibleQuotes([]));},[initialType]);
  type DraftProfession = { key: string; profession: string; customProfession?: string; requiredCount: number; unitSalary?: number; actualSalary?: number; sponsorshipType:"dali"|"other"; sponsorName:string; ajirContractStatus:"not_applicable"|"with_ajir"|"without_ajir" };
  ...
  function setProfessionActualSalary(key: string, value: number) { setProfessions((items) => items.map((item) => item.key === key ? { ...item, actualSalary: Math.max(0, value || 0) } : item)); }
  function setProfessionSponsorship(key:string, value:"dali"|"other") { setProfessions(items=>items.map(item=>item.key===key?{...item,sponsorshipType:value,sponsorName:value==="dali"?"":item.sponsorName,ajirContractStatus:item.ajirContractStatus||"not_applicable"}:item)); }
  ...
  function setProfessionAjir(key:string,value:"not_applicable"|"with_ajir"|"without_ajir"){setProfessions(items=>items.map(item=>item.key===key?{...item,ajirContractStatus:value}:item));}
  ...
  function addSponsorAllocation(source: DraftProfession) {
    setProfessions((items) => [...items, { ...source, key: `allocation-${Date.now()}`, requiredCount: 1, sponsorshipType: "other", sponsorName: "", ajirContractStatus: "without_ajir" }]);
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isContract) {
      const invalidProfession = professions.find((item) =>
        ...
        || !Number.isFinite(item.actualSalary) || (item.actualSalary || 0) <= 0
        || !["not_applicable", "with_ajir", "without_ajir"].includes(item.ajirContractStatus)
        || (item.sponsorshipType === "other" && (item.sponsorName || "").trim().length < 2)
      );
      ...
      if(new Set(allocationKeys).size!==allocationKeys.length){showContractValidationError(2,"يوجد توزيع مكرر للمهنة والكفيل وحالة أجير؛ اجمع العدد في توزيع واحد.");return;}
  ...
  const serializedProfessions = JSON.stringify(professions.map((item) => ({ profession: item.profession === "أخرى" ? (item.customProfession || "").trim() : item.profession, requiredCount: quantityMode==="open"?0:item.requiredCount, unitSalary:item.unitSalary || 0, actualSalary:item.actualSalary || item.unitSalary || 0, sponsorshipType:item.sponsorshipType, sponsorName:item.sponsorshipType==="other"?item.sponsorName:null, ajirContractStatus:item.ajirContractStatus, workerIds: quantityMode==="open"?[]:(selectedWorkers[item.key] || []) })));
  ...
  return <div className="modal-layer"><button className="drawer-backdrop" aria-label="إغلاق نافذة إصدار المستند" onClick={onClose}/><section className="record-modal document-modal issue-modal" role="dialog" aria-modal="true" aria-label="إنشاء ملف PDF رسمي"><div className="drawer-head"><div><span>الإصدار الرسمي</span><h2>{isContract ? "إنشاء عقد توفير عمالة" : "إنشاء ملف PDF"}</h2></div><button onClick={onClose} aria-label="إغلاق"><Icon name="close"/></button></div>
    ...
    <form className={`contract-quantity-${quantityMode}`} onSubmit={submit} noValidate>
      ...
      {isContract&&<><input type="hidden" name="showPaymentSchedule" value={showPaymentSchedule?"true":"false"}/><input type="hidden" name="accommodationParty" value={accommodationParty==="dali"?"توفره دالي":"يوفره الطرف الثاني"}/><input type="hidden" name="transportParty" value={transportParty==="dali"?"توفره دالي":"يوفره الطرف الآخر"}/></>}
      ...
      {isContract && <div className={`issue-form-step span-two ${step === 2 ? "visible" : ""}`}><div className="profession-builder-head"><div><strong>المهن وتوزيعات الكفالة</strong><small>أضف المهنة ثم وزّع عددها على دالي أو أكثر من كفيل. مجموع التوزيعات هو إجمالي المهنة.</small></div><button type="button" onClick={addProfession} disabled={new Set(professions.map(item=>item.profession)).size >= workforceProfessions.length}><Icon name="plus"/> إضافة مهنة</button></div><datalist id="contract-sponsor-options">{sponsorOptions.map(name=><option value={name} key={name}/>)}</datalist><div className="profession-builder">{capacity.map((item) => <article key={item.key}><label>المهنة<SearchableCombobox name={`profession_${item.key}`} value={item.profession} options={workforceProfessions.map((option) => option.label).filter((label) => label === item.profession || !professions.some((other) => other.key !== item.key && other.profession === label))} onChange={(value) => setProfessionValue(item.key, value)} placeholder="ابحث عن المهنة" required/></label><label>عدد هذا التوزيع<input type="number" min="1" max="100000" value={item.requiredCount} onChange={(event) => setProfessionCount(item.key, Number(event.target.value))}/></label><div><span><b>{item.available}</b> متاح مطابق</span><span><b>{professions.filter(row=>row.profession===item.profession).reduce((sum,row)=>sum+row.requiredCount,0)}</b> إجمالي المهنة</span></div><p className={item.registeredShortage || item.availableShortage ? "shortage" : "ready"}>{item.registeredShortage ? `أقل من هذا التوزيع في السجلات بفارق ${item.registeredShortage}` : item.availableShortage ? `عجز تشغيلي في التوزيع: ${item.availableShortage}` : "العدد المطابق متاح حالياً"}</p><button className="quote-add-line" type="button" onClick={()=>addSponsorAllocation(item)} disabled={Boolean(selectedQuoteId)}>+ توزيع كفيل آخر</button>{professions.length > 1 && <button className="remove-profession" type="button" onClick={() => removeProfession(item.key)}>حذف التوزيع</button>}</article>)}</div><p className="form-hint">تظهر أسماء الكفلاء تلقائياً من ملفات العمال المسجلين، ويمكن كتابة كفيل جديد إذا لم يكن له عامل مسجل بعد.</p></div>}

      {isContract && step === 2 && <div className="contract-profession-pricing span-two"><div className="profession-builder-head"><div><strong>الراتب والمهنة المخصصة</strong><small>راتب العامل الشهري إلزامي لكل مهنة حتى في العقد مفتوح العدد، ويُستخدم عند احتساب الفواتير الفعلية.</small></div></div><div className="profession-builder">{professions.map((item)=><article key={`pricing-${item.key}`}>{item.profession==="أخرى"&&<label>اكتب المهنة يدوياً<input required minLength={2} maxLength={120} value={item.customProfession||""} onChange={event=>setCustomProfession(item.key,event.target.value)} placeholder="مثال: فني مضخات"/></label>}<label>سعر العامل على العميل شهريًا (ريال)<input required type="number" min="0.01" max="1000000" step="0.01" value={item.unitSalary||""} onChange={event=>setProfessionSalary(item.key,Number(event.target.value))}/></label><label>الراتب الفعلي للعامل شهريًا (ريال)<input required type="number" min="0.01" max="1000000" step="0.01" value={item.actualSalary||""} onChange={event=>setProfessionActualSalary(item.key,Number(event.target.value))}/><small>لا ينشأ استحقاق الراتب إلا للعامل المسند إلى عقد فعّال.</small></label><label>جهة الكفالة<select value={item.sponsorshipType} onChange={event=>setProfessionSponsorship(item.key,event.target.value as "dali"|"other")} disabled={Boolean(selectedQuoteId)}><option value="dali">على كفالة شركة دالي</option><option value="other">على كفالة جهة أخرى</option></select></label>{item.sponsorshipType==="other"&&<label>اسم الكفيل<input required minLength={2} maxLength={160} value={item.sponsorName} onChange={event=>setProfessionSponsorName(item.key,event.target.value)} readOnly={Boolean(selectedQuoteId)}/></label>}<label>حالة عقد أجير<select required value={item.ajirContractStatus} onChange={event=>setProfessionAjir(item.key,event.target.value as "not_applicable"|"with_ajir"|"without_ajir")} disabled={Boolean(selectedQuoteId)}><option value="not_applicable">لا ينطبق</option><option value="with_ajir">بعقد أجير</option><option value="without_ajir">بدون عقد أجير</option></select><small>يمكن تحديد حالة أجير مستقلة عن جهة الكفالة، بما في ذلك عمالة دالي.</small></label><p>{item.profession==="أخرى"?(item.customProfession||"مهنة أخرى"):item.profession} · {quantityMode==="open"?"عدد مفتوح":`${item.requiredCount} عامل`} · {formatMoney(Math.round((item.unitSalary||0)*100))} للعامل/شهر</p></article>)}</div></div>}

      {isContract && <div className={`issue-form-step span-two ${step === 3 ? "visible" : ""}`}><div className="selection-intro"><strong>اختيار العمالة المتاحة</strong><p>اختيار الأسماء اختياري. يمكنك تخطي هذه الخطوة وإنشاء العقد ثم إضافة العمالة لاحقاً.</p></div><div className="worker-selection-groups">{capacity.map((item) => { const candidates = workers.filter((worker) => worker.profession === item.profession && worker.status === "available" && worker.sponsorshipType === item.sponsorshipType && (item.sponsorshipType !== "other" || worker.sponsorName === item.sponsorName)); const selected = selectedWorkers[item.key] || []; return <section key={item.key}><header><div><strong>{item.profession}</strong><small>مطلوب {item.requiredCount} · مختار {selected.length}</small></div><span className={selected.length === item.requiredCount ? "complete" : ""}>{item.requiredCount - selected.length} متبقٍ</span></header><div>{candidates.length ? candidates.map((worker) => <label key={worker.id} className={selected.includes(worker.id) ? "selected" : ""}><input type="checkbox" checked={selected.includes(worker.id)} disabled={!selected.includes(worker.id) && selected.length >= item.requiredCount} onChange={() => toggleWorker(item.key, worker.id, item.requiredCount)}/><span>{initials(worker.fullName)}</span><p><strong>{worker.fullName}</strong><small>{worker.workerNumber} · إقامة {worker.iqamaNumber || "غير مسجلة"} · {sponsorshipLabel(worker)}</small></p></label>) : <p className="empty-operational">لا توجد عمالة متاحة بهذه المهنة حالياً.</p>}</div></section>; })}</div>{totalShortage > 0 && <div className="contract-shortage-summary"><Icon name="bell"/><p><strong>يمكن إصدار العقد رغم العجز</strong><span>إجمالي العجز التشغيلي الحالي {totalShortage} عامل عبر المهن المطلوبة.</span></p></div>}</div>}
      ...
      {isContract&&<div className={`issue-form-step contract-final-step span-two ${step===4?"visible":""}`}>
        <section className="contract-final-card payment-plan-card">
          ...
          <div className="contract-tax-fields"><label>إظهار جدول الدفعات في PDF<select value={showPaymentSchedule?"true":"false"} onChange={event=>setShowPaymentSchedule(event.target.value==="true")}><option value="true">نعم، إظهار الجدول</option><option value="false">لا، إخفاء الجدول</option></select><small>تبقى الدفعات محفوظة في النظام في الحالتين.</small></label><label>السكن<select value={accommodationParty} onChange={event=>setAccommodationParty(event.target.value as "dali"|"counterparty")} disabled={Boolean(selectedQuoteId)}><option value="dali">توفره دالي</option><option value="counterparty">يوفره الطرف الثاني</option></select></label><label>النقل<select value={transportParty} onChange={event=>setTransportParty(event.target.value as "dali"|"counterparty")} disabled={Boolean(selectedQuoteId)}><option value="dali">توفره دالي</option><option value="counterparty">يوفره الطرف الآخر</option></select></label>
...
function EmployeeTable({ records, query, canWrite, busy, onStatus, onUpdate, onDelete }: { records: EmployeeRecord[]; query: string; canWrite: boolean; busy: string | null; onStatus: (id: number, status: string) => void; onUpdate: (id: number, data: Record<string, string>) => Promise<void>; onDelete: (id: number) => Promise<void> }) {
  ...
  return <div className="employee-card-grid">{rows.map(item=>{const open=openId===item.id;return <article className={`employee-profile-card ${open?"open":""}`} key={item.id}><header><div className="employee-card-avatar" aria-hidden="true">{item.fullName.trim().slice(0,1)}</div><div><strong>{item.fullName}</strong><span>{item.jobTitle} · {item.department}</span><small dir="ltr">{item.employeeNumber}</small></div><StatusControl entity="employees" id={item.id} value={item.status} canWrite={canWrite} busy={busy} onStatus={onStatus}/></header><div className="employee-card-glance"><span><small>الجوال</small><b dir="ltr">{item.mobile}</b></span><span><small>الإقامة</small><b className={daysUntil(item.iqamaExpiry)<29?"date-alert":""}>{formatDate(item.iqamaExpiry)}</b></span></div><button className="employee-card-toggle" type="button" aria-expanded={open} onClick={()=>{setOpenId(open?null:item.id);if(open)setEditingId(null)}}>{open?"إغلاق البطاقة":"فتح واستعراض الملف"}</button>{open&&<div className="employee-card-details"><dl><div><dt>البريد الوظيفي</dt><dd dir="ltr">{item.email||"غير مسجل"}</dd></div><div><dt>جهة الكفالة</dt><dd>{item.sponsorshipType==="dali"?"على كفالة دالي":item.sponsorName||"كفالة أخرى"}</dd></div><div><dt>انتهاء عقد العمل</dt><dd>{item.sponsorshipType==="dali"?formatDate(item.contractEndDate):"غير مطلوب"}</dd></div><div><dt>انتهاء رخصة العمل</dt><dd>{item.sponsorshipType==="dali"?formatDate(item.workPermitExpiry):"غير مطلوب"}</dd></div></dl>{canWrite&&<div className="employee-row-actions"><button onClick={()=>setEditingId(editingId===item.id?null:item.id)}>{editingId===item.id?"إغلاق الإدارة":"إدارة وتحديث"}</button><button className="danger-action" disabled={busy===`employee-delete-${item.id}`} onClick={()=>void onDelete(item.id)}>{busy===`employee-delete-${item.id}`?"جارٍ الحذف":"حذف الموظف"}</button></div>}{editingId===item.id&&<EmployeeComplianceEditor employee={item} busy={busy===`employee-update-${item.id}`} onCancel={()=>setEditingId(null)} onSave={async data=>{await onUpdate(item.id,data);setEditingId(null)}}/>}<EmployeeProfileWorkspace employeeId={item.id} leaveBalanceDays={item.leaveBalanceDays} canWrite={canWrite}/></div>}</article>})}</div>;
}
...
function LegalTable({ records, query, canWrite, busy, onStatus }: { records: LegalRecord[]; query: string; canWrite: boolean; busy: string | null; onStatus: (id: number, status: string) => void }) {
  ...
  return <div className="legal-record-card-grid">{rows.map(item=>{const summary=legalFileSummary(item.fileSnapshotJson),open=openId===item.id;return <article className={`legal-record-card ${open?"open":""}`} key={item.id}><header><div><span>{legalLabels[item.category]??item.category}</span><strong>{item.title}</strong><small dir="ltr">{item.referenceCode}</small></div><StatusControl entity="legal" id={item.id} value={item.status} canWrite={canWrite} busy={busy} onStatus={onStatus}/></header><div className="legal-record-glance"><span><small>الطرف الآخر</small><b>{item.counterparty}</b></span><span><small>الانتهاء أو التجديد</small><b className={daysUntil(item.expiryDate)<=45?"date-alert":""}>{formatDate(item.expiryDate)}</b></span></div><button className="legal-record-toggle" type="button" aria-expanded={open} onClick={()=>setOpenId(open?null:item.id)}>{open?"إغلاق الملف":"فتح الملف الكامل"}</button>{open&&<div className="legal-record-details">{item.referralReason&&<p><strong>سبب الإحالة</strong><span>{item.referralReason}</span></p>}{summary?<div className="legal-file-stats"><span><b>{summary.documents}</b> مستندات</span><span><b>{summary.payments}</b> دفعات</span><span><b>{summary.finances}</b> حركات مالية</span><span><b>{summary.workers}</b> عمال</span></div>:<p>ملف قانوني مسجل يدويًا ولا يحتوي لقطة عقد محالة.</p>}<small>أحيل بواسطة {item.referredBy||"النظام"} في {formatDate(item.referredAt,true)}</small></div>}</article>})}</div>;
}
...
function ContractDrawer({ contract, professions, assignments, workers, canWrite, isAdmin, isOwner, busy, onClose, onAssign, onRelease, onStatus, onEdit, onDelete, onRecordAbsence }: {
  ...
  onEdit: (contract: WorkforceContract, changes: {clientName:string;title:string;workSite:string;startDate:string;endDate?:string}) => Promise<void>;
  ...
  function submitContractEdit(event:FormEvent<HTMLFormElement>){event.preventDefault();const fd=new FormData(event.currentTarget);const changes={clientName:String(fd.get("clientName")||""),title:String(fd.get("title")||""),workSite:String(fd.get("workSite")||""),startDate:String(fd.get("startDate")||""),...(contract.seasonType!=="regular"?{endDate:String(fd.get("endDate")||"")}:{})};void onEdit(contract,changes).then(()=>setShowEditForm(false));}
  return <div className="drawer-layer"><button className="drawer-backdrop" aria-label="إغلاق إدارة العقد" onClick={onClose}/><aside className="request-drawer contract-drawer" role="dialog" aria-modal="true" aria-label={`إدارة عمالة العقد ${contract.referenceCode}`}><div className="drawer-head"><div><span dir="ltr">{contract.referenceCode}</span><h2>إدارة عمالة العقد</h2></div><button onClick={onClose} aria-label="إغلاق"><Icon name="close"/></button></div>
    ...
    <div className="pdf-language-actions"><a className="contract-pdf-link" href={`/api/portal/documents/${contract.documentId}?language=ar`}><Icon name="download"/> نسخة عربية</a><a className="contract-pdf-link" href={`/api/portal/documents/${contract.documentId}?language=bilingual`}><Icon name="download"/> نسخة عربي/English</a></div>{!contract.approvedBy && <p className="readonly-note">نسخة مسودة غير معتمدة.</p>}{showEditForm&&<section className="drawer-section"><h3>تعديل بيانات العقد</h3><form className="feature-form" onSubmit={submitContractEdit}><label>العميل أو الجهة<input name="clientName" required defaultValue={contract.clientName}/></label><label>عنوان العقد<input name="title" required defaultValue={contract.title}/></label><label>موقع العمل<input name="workSite" required defaultValue={contract.workSite}/></label><label>تاريخ البداية<input name="startDate" type="date" required defaultValue={contract.startDate}/></label>{contract.seasonType!=="regular"&&<label>تاريخ النهاية<input name="endDate" type="date" required defaultValue={contract.endDate}/></label>}<div className="modal-actions span-two"><button type="button" onClick={()=>setShowEditForm(false)}>إلغاء</button><button className="admin-primary" disabled={busy===`contract-edit-${contract.id}`}>حفظ التعديلات</button></div></form></section>}
    {canWrite && !["active","suspended","expired","terminated","superseded"].includes(contract.status) && <div className="document-actions"><button disabled={busy === `contract-edit-${contract.id}`} onClick={() => setShowEditForm(true)}>تعديل بيانات العقد</button>{contract.status === "draft" && !contract.approvedBy && <button className="danger-action" disabled={busy === `contract-delete-${contract.id}`} onClick={() => void onDelete(contract)}>حذف مسودة العقد</button>}</div>}
    {canWrite && (contractNextStatuses[contract.status]?.length || 0) > 0 && <section className="drawer-section contract-lifecycle"><h3>دورة اعتماد العقد</h3><p>اعتماد العقد للمالك أو مشرف النظام، وبعد الاعتماد يستطيع صاحب صلاحية التشغيل تفعيله فتبدأ آثاره التشغيلية والمالية.</p><div><select value={nextStatus} onChange={(event) => setNextStatus(event.target.value)}><option value="">اختر الحالة التالية</option>{contractNextStatuses[contract.status].filter((status) => status === "approved" ? isOwner : isAdmin || !["signed","terminated","cancelled","superseded"].includes(status)).map((status) => <option value={status} key={status}>{contractStatusLabels[status]}</option>)}</select>{!["cancelled","terminated"].includes(nextStatus)&&<textarea value={statusReason} onChange={(event) => setStatusReason(event.target.value)} rows={2} maxLength={1000} placeholder="سبب القرار إلزامي للتعليق"/>}<button className="admin-primary" disabled={!nextStatus || busy === `contract-status-${contract.id}`} onClick={() => {if(["cancelled","terminated"].includes(nextStatus)){setShowCancellation(true);return}void onStatus(contract.id, nextStatus, statusReason).then(() => { setNextStatus(""); setStatusReason(""); })}}>{busy === `contract-status-${contract.id}` ? "جارٍ الحفظ..." : "حفظ انتقال الحالة"}</button></div></section>}
    {isOwner && contract.status === "active" && <section className="drawer-section contract-absence-panel"><h3>تسجيل غياب العمالة وخصم اليومية</h3><p>اختر عاملًا محددًا، أو اترك العامل فارغًا وسجّل عدد المتغيبين من المهنة. اليومية = الراتب الفعلي ÷ 30، ويحسب النظام الأيام تلقائياً دون يوم الجمعة ثم يخصمها من دفعة شهر الغياب.</p><div><label>المهنة<select value={absenceProfessionId} onChange={(event)=>{setAbsenceProfessionId(event.target.value);setAbsenceWorkerId("");}}><option value="">اختر المهنة</option>{professions.map(item=><option key={item.id} value={item.id}>{item.profession}</option>)}</select></label><label>العامل — اختياري<select value={absenceWorkerId} onChange={(event)=>setAbsenceWorkerId(event.target.value)}><option value="">تسجيل بالعدد والمهنة</option>{activeAssignments.filter(item=>String(item.contractProfessionId)===absenceProfessionId).map(item=>{const worker=workers.find(row=>row.id===item.workerId);return <option key={item.id} value={item.workerId}>{worker?.fullName||`عامل ${item.workerId}`}</option>})}</select></label><label>عامل بديل — اختياري<select value={replacementWorkerId} disabled={!absenceWorkerId} onChange={(event)=>setReplacementWorkerId(event.target.value)}><option value="">دون بديل — تخصم الدفعة</option>{workers.filter(worker=>worker.status==="available"&&String(worker.id)!==absenceWorkerId&&worker.profession===professions.find(item=>String(item.id)===absenceProfessionId)?.profession).map(worker=><option key={worker.id} value={worker.id}>{worker.fullName} — {sponsorshipLabel(worker)}</option>)}</select><small>عند اختيار بديل لا يخصم من دفعة العميل.</small></label><label>عدد المتغيبين<input type="number" min="1" value={absenceWorkerId?"1":absenceCount} disabled={Boolean(absenceWorkerId)} onChange={(event)=>setAbsenceCount(event.target.value)}/></label><label>بداية الغياب<input type="date" value={absenceDate} min={contract.startDate} max={contract.endDate} onChange={(event)=>{setAbsenceDate(event.target.value);setAbsenceEndDate(event.target.value)}}/></label><label>نهاية الغياب<input type="date" value={absenceEndDate} min={absenceDate||contract.startDate} max={contract.endDate} onChange={(event)=>setAbsenceEndDate(event.target.value)}/><small>لا تُحتسب أيام الجمعة تلقائياً.</small></label><label className="span-two">ملاحظات<input value={absenceNotes} maxLength={1000} onChange={(event)=>setAbsenceNotes(event.target.value)} placeholder="سبب الغياب أو مرجع إثبات الحضور"/></label><button className="admin-primary span-two" disabled={!absenceProfessionId||!absenceDate||busy===`contract-absence-${contract.id}`} onClick={()=>void onRecordAbsence(contract.id,Number(absenceProfessionId),absenceDate,absenceEndDate,absenceWorkerId?Number(absenceWorkerId):null,replacementWorkerId?Number(replacementWorkerId):null,absenceWorkerId?1:Number(absenceCount),absenceNotes)}>{busy===`contract-absence-${contract.id}`?"جارٍ تسجيل الخصم...":"تسجيل الغياب وخصم اليومية"}</button></div></section>}
  ...
  </aside>{showCancellation&&<ContractCancellationDialog contract={contract} busy={busy===`contract-status-${contract.id}`} onClose={()=>setShowCancellation(false)} onConfirm={async(reason,reasonCode)=>{await onStatus(contract.id,nextStatus,reason,reasonCode);setShowCancellation(false);setNextStatus("");setStatusReason("")}}/>}</div>;
}
