...
function IssueDocumentModal({ initialType, busy, assetsReady, workers, contracts, requests, onClose, onSubmit }: { initialType: string; busy: boolean; assetsReady: boolean; workers: WorkerRecord[]; contracts: WorkforceContract[]; requests: WorkforceRequest[]; onClose: () => void; onSubmit: (form: HTMLFormElement) => Promise<void> }) {
  ...
  function applyQuote(event: React.ChangeEvent<HTMLSelectElement>){const id=event.target.value;setSelectedQuoteId(id);const quote=convertibleQuotes.find(item=>String(item.id)===id);if(!quote)return;setQuantityMode(quote.quantityMode);setContractAmount(String(quote.subtotalHalalas/100));setContractVatEnabled(quote.vatRateBps>0);setContractVatRate(String(quote.vatRateBps/100));if(quote.items.length)setProfessions(quote.items.map((item,index)=>({key:`quote-profession-${index}`,profession:item.profession,requiredCount:quote.quantityMode==="open"?0:Math.max(1,item.quantity),unitSalary:(item.unitPriceHalalas||0)/100})));window.setTimeout(()=>{const form=event.target.form;if(!form)return;const client=form.elements.namedItem("clientName");if(client instanceof HTMLInputElement)client.value=quote.clientName;},0);}
  ...
  return <div className="modal-layer"><button className="drawer-backdrop" aria-label="إغلاق نافذة إصدار المستند" onClick={onClose}/><section className="record-modal document-modal issue-modal" role="dialog" aria-modal="true" aria-label="إنشاء ملف PDF رسمي"><div className="drawer-head"><div><span>الإصدار الرسمي</span><h2>{isContract ? "إنشاء عقد توفير عمالة" : "إنشاء ملف PDF"}</h2></div><button onClick={onClose} aria-label="إغلاق"><Icon name="close"/></button></div>
    ...
    <form className={`contract-quantity-${quantityMode}`} onSubmit={submit}>
      ...
      {isContract&&step===1&&<div className="contract-billing-controls span-two"><label>نوع التعاقد<select value={seasonType} onChange={event=>setSeasonType(event.target.value as "regular"|"ramadan"|"hajj")}><option value="regular">عقود سنوية بدفعات شهرية</option><option value="ramadan">موسم رمضان — دفعات بالنسب</option><option value="hajj">موسم الحج — دفعات بالنسب</option></select></label>{seasonType==="regular"&&<label>تاريخ استحقاق أول دفعة<input type="date" required value={firstPaymentDueDate} onChange={event=>setFirstPaymentDueDate(event.target.value)}/></label>}<p className="form-hint span-two">في العقود السنوية تُنشأ الدفعات شهرياً من رواتب المهن. في الحج ورمضان تُستخدم نسب جدول الدفعات ويجب أن يكون مجموعها 100٪.</p></div>}
      ...
      {isContract&&<div className={`issue-form-step contract-final-step span-two ${step===4?"visible":""}`}>
        <section className="contract-final-card payment-plan-card">
          ...
          <div className="contract-tax-fields">
            ...
            {quantityMode==="fixed"&&seasonType==="regular"&&<div className="calculated-contract-value"><strong>قيمة محسوبة تلقائياً</strong><span>عدد العمال × راتب العامل × أشهر الاستحقاق، ثم تضاف الضريبة المختارة.</span></div>}
          </div>
