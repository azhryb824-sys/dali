-- Reconcile legacy databases whose migration history advanced without applying
-- 0030_video_interviews_and_operational_roles.sql. This migration is additive,
-- idempotent, and preserves all existing conversation and call records.
...
REVOKE ALL ON TABLE public.portal_user_presence, public.video_interviews, public.video_interview_transfers FROM PUBLIC;

DO $dali_video_acl$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    EXECUTE 'REVOKE ALL ON TABLE public.portal_user_presence, public.video_interviews, public.video_interview_transfers FROM anon';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    EXECUTE 'REVOKE ALL ON TABLE public.portal_user_presence, public.video_interviews, public.video_interview_transfers FROM authenticated';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'dali_app') THEN
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.portal_user_presence, public.video_interviews, public.video_interview_transfers TO dali_app';
    EXECUTE 'GRANT USAGE, SELECT ON SEQUENCE public.video_interview_transfers_id_seq TO dali_app';
    EXECUTE 'DROP POLICY IF EXISTS portal_user_presence_server_access ON public.portal_user_presence';
    EXECUTE 'DROP POLICY IF EXISTS video_interviews_server_access ON public.video_interviews';
    EXECUTE 'DROP POLICY IF EXISTS video_interview_transfers_server_access ON public.video_interview_transfers';
    EXECUTE 'CREATE POLICY portal_user_presence_server_access ON public.portal_user_presence AS PERMISSIVE FOR ALL TO dali_app USING (true) WITH CHECK (true)';
    EXECUTE 'CREATE POLICY video_interviews_server_access ON public.video_interviews AS PERMISSIVE FOR ALL TO dali_app USING (true) WITH CHECK (true)';
    EXECUTE 'CREATE POLICY video_interview_transfers_server_access ON public.video_interview_transfers AS PERMISSIVE FOR ALL TO dali_app USING (true) WITH CHECK (true)';
  END IF;
END
$dali_video_acl$;
