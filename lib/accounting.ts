...
export async function resolvePostingRule(
  eventType: string,
  fallback: { debitCode: string; creditCode: string; taxCode?: string },
) {
  const db = getDb();
  const rule = await db.query.accountingPostingRules.findFirst({
    where: and(
      eq(accountingPostingRules.eventType, eventType),
  ...
  const codes = [
      fallback.debitCode,
      fallback.creditCode,
      ...(fallback.taxCode ? [fallback.taxCode] : []),
    ],
    accounts = await db
    ...
    byCode = new Map(accounts.map((account) => [account.code, account.id])),
    debitAccountId = byCode.get(fallback.debitCode),
    creditAccountId = byCode.get(fallback.creditCode),
    taxAccountId = fallback.taxCode
      ? byCode.get(fallback.taxCode) || null
      : null;
  if (!debitAccountId || !creditAccountId)
    throw new Error(
      `قاعدة الترحيل ${eventType} أو حساباتها الاحتياطية غير مهيأة`,
    );
  return { debitAccountId, creditAccountId, taxAccountId };
}

export function validateBalancedJournal(lines: JournalLineInput[]) {
  if (lines.length < 2)
    throw new Error("يجب أن يحتوي القيد على طرفين على الأقل");
  let debit = 0;
  let credit = 0;
  for (const line of lines) {
    const lineDebit = Number(line.debitHalalas || 0);
    const lineCredit = Number(line.creditHalalas || 0);
    if (!Number.isSafeInteger(line.accountId) || line.accountId < 1)
      throw new Error("الحساب المحاسبي غير صحيح");
    if (
      !Number.isSafeInteger(lineDebit) ||
      !Number.isSafeInteger(lineCredit) ||
      lineDebit < 0 ||
      lineCredit < 0
    )
      throw new Error("قيمة بند القيد غير صحيحة");
    if (lineDebit > 0 === lineCredit > 0)
      throw new Error("كل بند يجب أن يكون مدينًا أو دائنًا فقط");
    debit += lineDebit;
    credit += lineCredit;
  }
...
export async function createReversalDraft(
  ...
  const existing = await db.query.journalEntries.findFirst({
    where: and(
      eq(journalEntries.reversalOfId, entryId),
      inArray(journalEntries.status, ["draft", "approved", "posted"]),
    ),
  });
  ...
  const result = await createDraftJournal({
    ...
    lines: lines.map((line) => ({
      ...
      description: `عكس: ${line.description || original.description}`.slice(
        0,
        300,
      ),
  ...
  await emitPortalNotification({
    eventType: "journal-reversal-created",
    title: "قيد عكسي بانتظار الاعتماد",
    message: `${result.entry.entryNumber} — عكس ${original.entryNumber}.`,
    severity: "warning",
    module: "finance",
    entityType: "journal-entry",
...
export async function postJournal(entryId: number, actorEmail: string) {
  ...
  if (entry.createdBy.trim().toLowerCase() === actorEmail.trim().toLowerCase())
    throw new Error("لا يمكن لمنشئ القيد ترحيله");
  const period = await db.query.fiscalPeriods.findFirst({
  ...
  if (
    posted.sourceType === "financial-record" &&
    posted.sourceId &&
    /^\d+$/.test(posted.sourceId)
  ) {
    await db
      ...
      .where(
        and(
          eq(financialRecords.id, Number(posted.sourceId)),
          eq(financialRecords.journalEntryId, posted.id),
        ),
      );
  }
  if (posted.reversalOfId) {
    await db
      ...
      .where(
        and(
          eq(journalEntries.id, posted.reversalOfId),
  ...
  await emitPortalNotification({
    ...
    title: "تم ترحيل قيد محاسبي",
    message: `${posted.entryNumber} — ${posted.description}.`,
    severity: "success",
    module: "finance",
    entityType: "journal-entry",
    entityId: posted.id,
...
export async function approveJournal(entryId: number, actorEmail: string) {
  ...
  if (!entry || entry.status !== "draft")
    throw new Error("لا يمكن اعتماد القيد في حالته الحالية");
  if (entry.createdBy.trim().toLowerCase() === actorEmail.trim().toLowerCase())
    throw new Error("يجب أن يعتمد القيد مستخدم آخر تطبيقًا لفصل المهام");
  const now = new Date().toISOString();
  const [approved] = await db
    ...
    .where(
      and(eq(journalEntries.id, entry.id), eq(journalEntries.status, "draft")),
    )
    .returning();
  ...
  await emitPortalNotification({
    ...
    title: "قيد محاسبي جاهز للترحيل",
    message: `${approved.entryNumber} — ${approved.description}.`,
    severity: "info",
    module: "finance",
    entityType: "journal-entry",
    entityId: approved.id,
