...
const requiredColumns = {
  ...
  legal_records: ["assigned_lawyer_id", "assigned_lawyer_email", "court_case_number", "court_name", "claim_amount_halalas", "judgment_amount_halalas", "litigation_stage"],
  legal_lawyers: ["full_name", "license_number", "mobile", "portal_user_email", "status"],
  legal_external_shares: ["legal_record_id", "attachment_id", "lawyer_id", "token_hash", "shared_by", "shared_at", "expires_at"],
...
const requiredTables = [
  ...
  "legal_hearings", "legal_evidence_custody", "legal_submissions", "legal_settlements", "legal_lawyers", "legal_external_shares",
...
try {
  ...
  for (const name of ["employees_manager_fk", "employee_termination_status_check", "legal_judgment_payments_status_check", "legal_records_assigned_lawyer_fk", "legal_external_shares_channel_check", "bank_statement_direction_check"]) {
  ...
  if (tableSet.has("legal_lawyers") && tableSet.has("portal_users") && tableSet.has("portal_access_scopes")) {
    const [{ count }] = await sql`
      select count(*)::int as count
      from public.legal_lawyers lawyer
      left join public.portal_users users on lower(users.email) = lower(lawyer.portal_user_email)
      where lawyer.portal_user_email is not null
        and (users.email is null or users.status <> 'active' or not exists (
          select 1 from public.portal_access_scopes scope
          where lower(scope.user_email) = lower(lawyer.portal_user_email)
            and scope.active = true and scope.functional_role in ('legal_supervisor','lawyer')
        ))`;
    if (count) failures.push(`INVALID_LINKED_LEGAL_LAWYERS:${count}`);
  }
  if (tableSet.has("legal_external_shares") && tableSet.has("legal_lawyers")) {
    const [{ count }] = await sql`
      select count(*)::int as count
      from public.legal_external_shares share
      join public.legal_lawyers lawyer on lawyer.id = share.lawyer_id
      where lawyer.portal_user_email is not null or nullif(trim(lawyer.mobile), '') is null`;
    if (count) failures.push(`INVALID_EXTERNAL_LEGAL_SHARES:${count}`);
  }
