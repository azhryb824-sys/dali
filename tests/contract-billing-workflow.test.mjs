...
test("contracts require a balanced payment schedule and preserve mandatory client attachments",async()=>{
  ...
  assert.match(ui,/nationalAddressFile/);assert.match(ui,/العنوان الوطني للعميل/);assert.match(ui,/required=\{!savedQuoteFiles\.some/);assert.match(route,/storedQuoteAttachment/);
...
test("contract professions include hospitality and trades with enforced custom profession",async()=>{
  ...
  assert.match(ui,/اكتب المهنة يدوياً/);assert.match(ui,/راتب العامل الشهري/);assert.match(ui,/ContractWorkforceBoard/);
...
test("seasonal and annual payment schedules flow through quotes contracts finance and bilingual PDFs",async()=>{
  ...
  assert.match(contractUi,/تعديل موعد الدفعة/);assert.match(contractUi,/PDF عربي\/English/);assert.match(contractEditUi,/annualContractEndDate\(startDate\)/);assert.match(contractEditUi,/readOnly=\{season === "regular"\}/);
...
test("contract and quotation edit delete and cancellation actions preserve financial and legal history",async()=>{
  ...
  assert.match(statusApi,/contract-cancellation-referred-legal/);assert.match(await source("lib/contract-cancellation.ts"),/category: "case"/);
  assert.match(statusApi,/applyContractCancellation\(tx/);
...
test("contract cancellation transfers a complete immutable client case snapshot to legal",async()=>{
  const[status,schema,migration,portal]=await Promise.all([source("lib/contract-cancellation.ts"),source("db/schema.ts"),source("drizzle-pg/0024_legal_client_case_file.sql"),source("app/portal/PortalDashboard.tsx")]);
  for(const section of ["documents","payments","finances","professions","assignments","workers"])assert.match(status,new RegExp(section));
  assert.match(status,/fileSnapshotJson: JSON\.stringify/);assert.match(status,/originalPayments: payments/);
...
test("due installments auto invoice once, support secure WhatsApp sharing, and feed finance and legal command centers",async()=>{
  ...
  assert.match(paymentsApi,/issueDueContractInvoice/);assert.match(paymentsApi,/clientMobiles/);assert.match(dashboard,/WhatsAppFileShareDialog/);assert.match(whatsapp,/https:\/\/wa\.me\//);assert.match(dashboard,/source=\{\{ documentId: sharingInvoiceId \}\}/);
