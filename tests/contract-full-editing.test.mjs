...
test("contract edit dialog covers commercial, operational and profession data",async()=>{
  ...
  assert.match(dialog,/المهن والأعداد والأسعار/);assert.match(dialog,/unitSalaryHalalas/);assert.match(dialog,/actualSalaryHalalas/);
...
test("sponsorship is contract-level while Ajir is an independent contract allocation",async()=>{
  ...
  assert.match(dialog,/جهة الكفالة/);
  assert.match(dialog,/حالة عقد أجير/);
  assert.match(dialog,/ajirContractStatus/);
