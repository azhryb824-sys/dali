...
test("directional workforce contracts use editable bilingual clauses and conditional jurisdiction",async()=>{
  ...
  assert.match(api,/contractDirection/);assert.match(api,/parseWorkforceContractClauses/);assert.match(api,/db\.insert\(contractClauses\)/);assert.match(api,/db\.query\.suppliers/);assert.match(api,/representativeRequestId/);
