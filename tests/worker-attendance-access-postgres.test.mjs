...
before(async () => {
  ...
  await build({ stdin: { contents: `export * as schema from './db/schema.ts'; export * as attendance from './app/api/portal/contracts/[id]/attendance/route.ts'; export {canManageWorkerAttendance} from './lib/worker-attendance-access.ts'; export * from 'qa-attendance-state';`, resolveDir: process.cwd() }, outfile, bundle: true, platform: "node", format: "esm", packages: "external", logLevel: "silent", plugins: [{ name: "isolated-attendance-boundaries", setup(b) {
    ...
    b.onLoad({ filter: /.*/, namespace: "attendance-state" }, () => ({ contents: `let db, actor; export const audits=[], notifications=[]; export function getDb(){return db} export function getSqlClient(){throw new Error('Unexpected raw SQL dependency in attendance fixture')} export function setDb(v){db=v} export function setActor(v){actor=v;audits.length=0;notifications.length=0} export async function requirePortalApiRole(roles){return actor?.authorized && actor.status==='active' && roles.includes(actor.role) ? actor : null} export async function hasPortalPermission(a,r,v){return a.functionalPermissions.includes('*')||a.functionalPermissions.includes(r+'.'+v)} export async function auditPortalAction(v){audits.push(v)} export async function emitPortalNotification(v){notifications.push(v)} ` }));
  } }] });
